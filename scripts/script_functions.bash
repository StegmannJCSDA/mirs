...
#
#  12/01/2014     Dr. Tanvir islam Extend to include GPM/GMI. 
#                      (CIRA@NOAA/NESDIS/STAR)
#  11/22/2016     Junye Chen       Extend to include JPSS-1(N20)/ATMS. 
#                      (ESSIC/UMD@NOAA/NESDIS/STAR)
#  05/01/2018     Shuyan Liu and Ryan Honeyager Many changes for DAP (Shuyan).
#                 (ESSIC/UMD,IMSG@NOAA/NESDIS/STAR)   Rewrote mirs2nc, added unit tests (Ryan).
#  04/01/2019     Ryan Honeyager             Improvements to mirs2nc to beter handle DAP versioning.
#                      (IMSG@NOAA/NESDIS/STAR)
#  06/21/2019     Ryan Honeyager             Extending scripts for CMake compatability and
#                      (IMSG@NOAA/NESDIS/STAR)     automatic Valgrind testing.
#  10/28/2020     Yong-Keun Lee(CISESS/UMD)  metopSGA1 MWS experimental version
#
####################################################################################################
...
#
# Description:
#       Figures out if we are running Valgrind or another
#       tool on the MiRS pipeline.
#
#
# Outputs:
#      - cmd_preamble - A command that is prepended to the actual program invocations.
#
#===============================================================
function preamble() {
   # Mildly hidden variable - if SCRIPT_TESTING_PREAMBLE is defined, use it.
   # Otherwise, set cmd_preamble to an empty string.
   # Needed for Valgrind testing.
   if [ -z ${SCRIPT_TESTING_PREAMBLE+x} ]; then
      cmd_preamble=""
   else
      cmd_preamble="${SCRIPT_TESTING_PREAMBLE}"
   fi
   }
...
function rdr2tdr() {
    ...
    if [[ "$sensor" == "amsua" ]] ; then
      ...
      ext1="NPR.TDRN*"
      ext2="*"
    elif [[ "$sensor" == "atms" && "${satId}" == "npp" ]] ; then
      suffix="npp"
      ext1="*ATMS*"
      ext2="*"
    elif [[ "$sensor" == "atms" && "${satId}" == "n20" ]] ; then
      suffix="n20"
      ext1="*ATMS*"
      ext2="*"
    elif [[ "$sensor" == "atms" && "${satId}" == "n21" ]] ; then
      suffix="n21"
      ext1="*ATMS*"
      ext2="*"
      ...
      ext1="MT1SAPSL1A*"
      ext2="*h5"
    elif [[ "$sensor" == "gmi" ]] ; then
      suffix="gpm"
      ext1="1C-R.GPM*"
      ext2="*5"
    elif [[ "$sensor" == "mws" ]] ; then
      suffix="metopSGA1"
      ext1="MWS"
      ext2="*"
    fi    
    ...
    if [[ "${satId}" == "n18" || "${satId}" == "metopA" || "${satId}" == "n19" || "${satId}" == "metopB" || "${satId}" == "metopC" ]] ; then 
      ...
      echo "${cmd_preamble} ${binDir}/rdr2tdr_${satId}_${sensor} < ${rdr2tdr_cntrl}"
      ${cmd_preamble} ${binDir}/rdr2tdr_${satId}_${sensor} < ${rdr2tdr_cntrl}
      checkStatus $? "rdr2tdr"
	...
	CreatNamList ${rdr2tdr_cntrl}_calib '&ContrlCalibTDR' "$@"
    	...
    	DoMake ${rdr2tdrSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/calib_${satId}_ssmis < ${rdr2tdr_cntrl}_calib"
    	${cmd_preamble} ${binDir}/calib_${satId}_ssmis < ${rdr2tdr_cntrl}_calib
	...
	set 	"rdrfileList='${rdrList}'"	\
    	...
    	DoMake ${rdr2tdrSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/rdr2tdr_${satId}_ssmis < ${rdr2tdr_cntrl}_grid"
    	${cmd_preamble} ${binDir}/rdr2tdr_${satId}_ssmis < ${rdr2tdr_cntrl}_grid
	checkStatus $? "rdr2tdr: rdr2tdr_${satId}_ssmis"
	
    elif [[ "${satId}" == "f18" || "${satId}" == "f17" || "${satId}" == "f19" ]] ; then
	...
	set 	"rdrfileList='${rdrList}'"	\
    	...
    	CreatNamList ${rdr2tdr_cntrl} '&ContrlRDR2TDR' "$@"
    	DoMake ${rdr2tdrSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/rdr2tdr_${satId}_ssmis < ${rdr2tdr_cntrl}"
    	${cmd_preamble} ${binDir}/rdr2tdr_${satId}_ssmis < ${rdr2tdr_cntrl}
	checkStatus $? "rdr2tdr: rdr2tdr_${satId}_ssmis"
	...
	if [[ ${rdrType} -eq 1 ]] ; then
    	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "TATMS_npp*" "${ext2}"
	  geoList=${rdrList/npp_atms_rdrFiles_/npp_atms_geoFiles_}
	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${geoList} "GATMO_npp*" "${ext2}"
	elif [[ ${rdrType} -eq 2 ]] ; then  
    	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "SATMS_npp*" "${ext2}"
	...
	#echo "${logFile}"         >> ${rdr2tdr_cntrl}
    	...
    	DoMake ${rdr2tdrSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/rdr2tdr_npp_atms < ${rdr2tdr_cntrl}"
	${cmd_preamble} ${binDir}/rdr2tdr_npp_atms < ${rdr2tdr_cntrl}
	...
	# assume RDR and GEO files are in the same directory, otherwise, need a geoDir
	if [[ ${rdrType} -eq 1 ]] ; then
    	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "TATMS_j01*" "${ext2}"
	  geoList=${rdrList/n20_atms_rdrFiles_/n20_atms_geoFiles_}
	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${geoList} "GATMO_j01*" "${ext2}"
	elif [[ ${rdrType} -eq 2 ]] ; then  
    	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "SATMS_j01*" "${ext2}"
	elif [[ ${rdrType} -eq 3 ]] ; then  
    	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "SATMS_j01*" "${ext2}"
	  geoList=${rdrList/n20_atms_rdrFiles_/n20_atms_geoFiles_}
	  ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${geoList} "GATMO_j01*" "${ext2}"
   ...
   DoMake ${rdr2tdrSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/rdr2tdr_n20_atms < ${rdr2tdr_cntrl}"
	${cmd_preamble} ${binDir}/rdr2tdr_n20_atms < ${rdr2tdr_cntrl}
   #
   #---change tdr file name from "j01" to "n20", liusy+
   #
   rename j01 n20 ${tdrDir}/*
   rename j01 n20 ${nedtPath}/NEDT*.dat
   #
        ...
        # assume RDR and GEO files are in the same directory, otherwise, need a geoDir
        if [[ ${rdrType} -eq 1 ]] ; then
          ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "TATMS_j02*" "${ext2}"
          geoList=${rdrList/n21_atms_rdrFiles_/n21_atms_geoFiles_}
          ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${geoList} "GATMO_j02*" "${ext2}"
        elif [[ ${rdrType} -eq 2 ]] ; then
          ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "SATMS_j02*" "${ext2}"
        elif [[ ${rdrType} -eq 3 ]] ; then
          ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "SATMS_j02*" "${ext2}"
          geoList=${rdrList/n21_atms_rdrFiles_/n21_atms_geoFiles_}
          ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${geoList} "GATMO_j02*" "${ext2}"
   ...
   DoMake ${rdr2tdrSrc} ${makeOrNot}
        echo "${cmd_preamble} ${binDir}/rdr2tdr_n21_atms < ${rdr2tdr_cntrl}"
        ${cmd_preamble} ${binDir}/rdr2tdr_n21_atms < ${rdr2tdr_cntrl}
   #
   #---change tdr file name from "j02" to "n21", liusy+
   #
   rename j02 n21 ${tdrDir}/*
   rename j02 n21 ${nedtPath}/NEDT*.dat
   #
	...
	echo "${cmd_preamble} ${binDir}/rdr2tdr_${satId}_${sensor} < ${rdr2tdr_cntrl}"
    	${cmd_preamble} ${binDir}/rdr2tdr_${satId}_${sensor} < ${rdr2tdr_cntrl}
	checkStatus $? "rdr2tdr: rdr2tdr_gpm_gmi"

    elif [[ "${satId}" == "metopSGA1" ]] ; then
     	
	DirExistEst "RDR-${suffix}" ${rdrDir}     #---Test existence of RDR directory
    	ConstructList ${processingMode} ${rdrDir} ${orbitInfo} ${rdrList} "MWS_metopSGA1*" "${ext2}"
	DataExistEst "${suffix}" "RDR" ${rdrList} #---Test existence of RDR files
	...
	echo "${instrFile}"  >> ${rdr2tdr_cntrl}
    	...
    	DoMake ${rdr2tdrSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/rdr2tdr_metopSGA1_mws < ${rdr2tdr_cntrl}"
	${cmd_preamble} ${binDir}/rdr2tdr_metopSGA1_mws < ${rdr2tdr_cntrl}

    rename eps metopSGA1 ${tdrDir}/*
    rename eps metopSGA1 ${nedtPath}/NEDT*.dat
    fi
    
    echo "End of step rdr2tdr 1"    
}
...
function mergeNedt() {
	...
	echo "${cmd_preamble} ${binDir}/mergeNEDT < ${mergeNedt_cntrl}"
    	${cmd_preamble} ${binDir}/mergeNEDT < ${mergeNedt_cntrl}
    elif [[ "${satId}" == "f16" || "${satId}" == "f17" || "${satId}" == "f18" || "${satId}" == "f19" ]] ; then
    	...
    	cp ${nedtNominalFile} ${nedtBefFMFile}
#    elif [[ "${satId}" == "npp" || "${satId}" == "n20" || "${satId}" == "n21" || "${satId}" == metopSGA1 ]] ; then
    elif [[ "${satId}" == "npp" || "${satId}" == "n20" || "${satId}" == "n21" ]] ; then
	   ...
	   if [[ ${nfile} -eq 0 ]] ; then
         ...
         set  "listFile='${fileList}'"     \
	      ...
	      echo "${cmd_preamble} ${binDir}/mergeNedt_npp_atms < ${mergeNedt_cntrl}"
	      ${cmd_preamble} ${binDir}/mergeNedt_npp_atms < ${mergeNedt_cntrl}

         yyyymmdd=`basename ${nedtGranuleDir}`
         yyyy=`echo ${yyyymmdd} | cut -c1-4`
         mm=`echo ${yyyymmdd} | cut -c6-7`
         dd=`echo ${yyyymmdd} | cut -c9-10`

	      if [[ "${nedtPath}" != "${nedtGranuleDir}" && "${nedtPath}/" != "${nedtGranuleDir}" ]] ; then
                mv -f ${nedtPath}/NEDT_*${yyyy}${mm}${dd}*.dat ${nedtGranuleDir}
              fi
	   fi
#   fi
    elif [[ "${satId}" == "metopSGA1" ]] ; then
        ...
        local nedtPath=`dirname ${nedtBefFMFile}`
        local controlPath=`dirname ${mergeNedt_cntrl}`
        local fileList=${controlPath}/NEDT_metopSGA1_mws_list
        ls -1 ${nedtPath}/NEDT_*dat > ${fileList} 2> /dev/null
        local nfile=`wc -l ${fileList} | awk '{print $1}' 2> /dev/null`
        if [[ ${nfile} -eq 0 ]] ; then
            echo "NO NEDT_* files under ${nedtPath}, use nominal nedt file"
            ...
            CreatNamList ${mergeNedt_cntrl} '&MergeNEDTCntrl' "$@"
            DoMake ${mergeNedtSrc} ${makeOrNot}
            echo "${cmd_preamble} ${binDir}/mergeNedt_metopSGA1_mws < ${mergeNedt_cntrl}"
            ${cmd_preamble} ${binDir}/mergeNedt_metopSGA1_mws < ${mergeNedt_cntrl}
            if [[ "${nedtPath}" != "${nedtGranuleDir}" && "${nedtPath}/" != "${nedtGranuleDir}" ]] ; then
...
function tdr2sdr() {
    ...
    echo "${cmd_preamble} ${binDir}/tdr2sdr < ${tdr2sdr_cntrl}"
    ${cmd_preamble} ${binDir}/tdr2sdr < ${tdr2sdr_cntrl}
    checkStatus $? "tdr2sdr"
...
function fm() {
    ...
    if [[ "${satId}" == "n18" || "${satId}" == "metopA" || "${satId}" == "n19" || "${satId}" == "metopB" || "${satId}" == "metopC" ]] ; then
	   ...
	   echo "${cmd_preamble} ${binDir}/fm_${satId} < ${fm_cntrl}"
    	${cmd_preamble} ${binDir}/fm_${satId} < ${fm_cntrl}
	   checkStatus $? "fm"
      ...
      DataExistEst "SSM/IS-UAS" "SDR" ${sdrList_ssmis_uas}
	   ...
	   DataExistEst "${satId}^^" "ATMS" ${sdrList_amsua}
	   ...
	   for line in `cat ${fileList}`
        ...
        ConstructList ${processingMode} ${sdrDir_amsua} ${OrbitInfo} ${sdrList_amsua} "SDR_*" "*"
        DataExistEst "metopSGA1" "MWS" ${sdrList_amsua}
        set     "sdrfileList='${sdrList_amsua}'"         \
                "pathFMSDR='${fmsdrDir}/'"               \
                "FMresol=${iFMtype}"                     \
                "norbits2process=${norbits2process}"     \
        ...
        local nedtPath=`dirname ${nedtFileIn}`/${rdirExt}
        local controlPath=`dirname ${modifyNedt_cntrl}`
        local fileList=${controlPath}/metopSGA1_mws_nedt_list
      ...
      set 	"sdrfileList_lr='${sdrList_lr}'"                      \
	   ...
	   #---modify nedt values after the footprint matching process (noise amplification/reduction)
    	...
    	ConstructList ${processingMode} ${sdrDir_amsua} ${OrbitInfo} ${sdrList_amsua} "SDR_*" "*"
	   DataExistEst "GPM" "GMI" ${sdrList_amsua}
    	set 	"sdrfileList='${sdrList_amsua}'"	         \
		      "pathFMSDR='${fmsdrDir}/'"		            \
	         "FMresol=${iFMtype}"                      \
		      "norbits2process=${norbits2process}"	   \
...
# 	- satId: Satellite ID ( n18/n19/f16/npp/n20/trmm, etc )
...
#    echo "${cmd_preamble} ${binDir}/prepNWP < ${namelist}"
    ${cmd_preamble} ${binDir}/prepNWP < ${namelist}
...
#       - imsSfcUse: Flag for using (1) or not (0) the IMS snow/ice analysis
#       - imsCollocDir: IMS collocated analysis directory
# 	- processingMode: Processing mode (0:Orbit mode,  1:Daily mode)
# 	- OrbitInfo: Orbit information
#
#
...
function nwp() {
    ...
    local imsSfcUse=${22}
    local imsCollocDir=${23}

    local imsColloc4nwpList=${24}

    local processingMode=${25}
    local orbitInfo=${26}
    ...
    imsCollocFile=${imsCollocDir}/'missing'
    echo "processingMode="${processingMode}
    echo "imsCollocDir="${imsCollocDir}
    if [ ${processingMode} == 0 ]
	 then
	   ls -1 ${imsCollocDir}IMScol*${OrbitInfo}* > ${imsColloc4nwpList}
    else
	   ls -1 ${imsCollocDir}/IMScol* > ${imsColloc4nwpList}
    fi
    echo "imsColloc4nwpList="${imsColloc4nwpList}
    ...
    # way too many granules for NPP/ATMS, GPM/GMI, and N20/ATMS
    local CPUuse=1
    if [[ ${sensor_id} -eq 6 || ${sensor_id} -eq 10 || ${sensor_id} -eq 20 || ${sensor_id} -eq 22 || ${sensor_id} -eq 23 ]] ; then
      CPUuse=0
    fi
    ...
    if [[ ${CPUuse} -eq 0 ]] ; then  # use only one CPU
      ...
      set "RadFileList='${fmsdrList4nwp}'"                        \
          ...
          "imsSfcUse=${imsSfcUse}"                                \
          "imsCollocFilesList='${imsColloc4nwpList}'"             \
	  "Coeff_Path='${CRTM_Coeff_Path}'"                       \
          "BiasFile='${biasFileToUse}'"                           \
          "TuningFile='${tune1File}'"
      CreatNamList ${grid2nwp_cntrl}_${nwpstr} '&ControlNWP' "$@"
      echo "${cmd_preamble} ${binDir}/colocNWPwRad < ${grid2nwp_cntrl}_${nwpstr}"
      ${cmd_preamble} ${binDir}/colocNWPwRad < ${grid2nwp_cntrl}_${nwpstr}
	...
	sed -n "$ifile,$ifile p" ${imsColloc4nwpList} > ${imsColloc4nwpList}_${ifile}
	set      "RadFileList='${fmsdrList4nwp}_${ifile}'"               \
            ...
            "nwp_source=${nwpDataSrc}"                              \
            "imsSfcUse=${imsSfcUse}"                                \
            "imsCollocFilesList='${imsColloc4nwpList}_${ifile}'"   \
            "Coeff_Path='${CRTM_Coeff_Path}'"                       \
            "BiasFile='${biasFileToUse}'"                           \
            "TuningFile='${tune1File}'"
	CreatNamList ${grid2nwp_cntrl}_${nwpstr}_${ifile} '&ControlNWP' "$@"
	DoMake ${nwpGener_analys} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/colocNWPwRad < ${grid2nwp_cntrl}_${nwpstr}_${ifile} &"
	${cmd_preamble} ${binDir}/colocNWPwRad < ${grid2nwp_cntrl}_${nwpstr}_${ifile} &
	checkStatus $? "nwp: ${nwpstr}"
	let ifile=ifile+1
      done
      wait
...
function fwd() {
    ...
    if [[ ${satId} == 'npp' || ${satId} == 'n20'|| ${satId} == 'n21' ]] ; then
    ...
    do
	 ...
	 set "CntrlConfig_fwd%GeophInputFile='${nwpfile}'" "CntrlConfig_fwd%Coeff_Path='${CRTM_Coeff_Path}'"      \
      ...
      echo "${cmd_preamble} ${binDir}/fwd < ${fwd_cntrl}_${ifile}_${nwpstr} &"
      ${cmd_preamble} ${binDir}/fwd < ${fwd_cntrl}_${ifile}_${nwpstr} &
...
function biasGen() {
    ...
    echo "${cmd_preamble} ${binDir}/Calib_generic_rad  < ${biasGen_cntrl}_${nwpstr}"
    ${cmd_preamble} ${binDir}/Calib_generic_rad  < ${biasGen_cntrl}_${nwpstr}
...
function chopp() {
    ...
    echo "${cmd_preamble} ${binDir}/chopp < ${chopp_cntrl}"
    ${cmd_preamble} ${binDir}/chopp < ${chopp_cntrl}
    checkStatus $? "chopp"
...
function applyRegress() {
    ...
    echo "${cmd_preamble} ${binDir}/ApplyRegress < ${regress_cntrl}"
    ${cmd_preamble} ${binDir}/ApplyRegress < ${regress_cntrl}
...
function fmsdr2edr() {
    ...
    local imsSfcUse=$1 ; shift
    local imsCollocDir=$1 ; shift
    local imsColloc4retrList=$1 ; shift
    ...
    local rdirExt=$1; shift
 
    local TBbias_NNModelFile=$1; shift
    local TPW_NNModelFile=$1; shift
    local Tskin_NNModelFile=$1
 
    echo "=============== RUNNING FMSDR->EDR STEP ======================="
    DoMake ${fmsdr2edr} ${makeOrNot}

    if [[ ${iSensor_ID} == 6 || ${iSensor_ID} == 20 || ${iSensor_ID} == 23 ]] ; then
    ...
    #----list of IMS collocation data if requested
    if [ ${imsSfcUse} == 1 ] 
    then 
	    if [ ${processingMode} == 0 ]
	    then
	       ls -1 ${imsCollocDir}IMScol*${OrbitInfo}* > ${imsColloc4retrList}
	    else
	       ls -1 ${imsCollocDir}/IMScol* > ${imsColloc4retrList}
	    fi

	echo "imsColloc4retrList="${imsColloc4retrList}

	xarrIms=(`cat ${imsColloc4retrList}`)
        DataExistEst "IMS colloc" "for retrieval" ${imsColloc4retrList}
        #nfilesExt=$?
	local nfilesIms=`wc -l ${imsColloc4retrList} | awk '{print $1}'`
        #----Consistency checks
	    if [ ${nfiles} -ne ${nfilesIms} ]
	    then
	       op_msg "Warning: Number of files inconsistent between Radiances and IMS collocation data: ${nfiles} ${nfilesIms}"
	       exit 1
	    fi
    fi
    ...
    local ncpu=40  # how many CPUs will be used here, check /proc/cpuinfo to get number of cpus
    #if [[ ${nfiles} -lt 32 ]] ; then # for operational use at NDE 
    if [[ ${nfiles} -gt 50 ]] ; then # for use in STAR
      ...
      do
         ...
         checking=1
         while [ ${checking} == 1 ] ; do
            npcount=`ps -C 1dvar --no-headers | wc -l`
            let npdiff=ncpu-npcount
            if [[ $npdiff -gt 1 ]] ; then
               let top=bot+npdiff
               checking=0
            else
	...
	for (( ifile=${bot}; ifile<${top}; ifile++ )); do  # start ifile inner loop
	  ...
	  if [ ${imsSfcUse} == 1 ] 
	  then
	      imsfile=${xarrIms[${ifile}]}
	  fi

	  #get NEDT file at ATMS granule level
	  if [[ ${iSensor_ID} == 6 || ${iSensor_ID} == 20 || ${iSensor_ID} == 23 || ${iSensor_ID} == 22 ]] ; then
	  ...
	  set "CntrlConfig%AlgSN=${algSN}" "CntrlConfig%SensorID=${iSensor_ID}"                               \
         ...
         "CntrlConfig%CovBkgFileAtm(2)='${CovBkgFile2Atm}'"                                        	   \
	      ...
	      "CntrlConfig%useImsSfc=${imsSfcUse}"                                                            \
	      "CntrlConfig%imsCollocFile='${imsfile}'"                                                        \
         ...
         "CntrlConfig%TBbias_NNModelFile='${TBbias_NNModelFile}'"                                        \
         "CntrlConfig%TPW_NNModelFile='${TPW_NNModelFile}'"                                              \
         "CntrlConfig%Tskin_NNModelFile='${Tskin_NNModelFile}'" 

	  CreatNamList ${fmsdr2edr_cntrl}_${jfile} '&ContrlP' "$@"

	echo "${cmd_preamble} ${binDir}/1dvar < ${fmsdr2edr_cntrl}_${jfile} &"
	  ${cmd_preamble} ${binDir}/1dvar < ${fmsdr2edr_cntrl}_${jfile} &
  ...
  if [[ "${satId}" == "metopSGA1" ]] ; then  
	...
	if [[ ${iSensor_ID} == 22 ]] ; then
	    fileName=`basename "${radfile}"`
	    timeInfo=`echo ${fileName} | cut -f3-7 -d "_"`
	    nedtFile=`ls ${nedtGranPath}/NEDT_*_${timeInfo}_*_aftFM.dat 2> /dev/null`
            echo ${nedtFile}
	    #nedtFile=${nedtGranPath}/NEDT_SATMS_${timeInfo}_ops_aftFM.dat
	    if [[ -z ${nedtFile} ]] ; then  
	        nedtFileAfterFM=${aveNedtFile}
	...
	set "CntrlConfig%AlgSN=${algSN}" "CntrlConfig%SensorID=${iSensor_ID}"                                 \
       ...
       "CntrlConfig%FileModelErr(2)='${ModelErrFile2ToUse}'"                                             \
	    ...
	    "CntrlConfig%MonitorFile='${MonitorFile}'" "CntrlConfig%Topogr='${Topogr}'"                       \
	    "CntrlConfig%Coeff_Path='${CRTM_Coeff_Path}'" "CntrlConfig%LogFile='${Logfile}'"		      \
            "CntrlConfig%TBbias_NNModelFile='${TBbias_NNModelFile}'" "CntrlConfig%TPW_NNModelFile='${TPW_NNModelFile}'"       \
            "CntrlConfig%Tskin_NNModelFile='${Tskin_NNModelFile}'"
	CreatNamList ${fmsdr2edr_cntrl}_${jfile} '&ContrlP' "$@"
...
function mergeEdr() {
    ...
    do
      ...
      echo "${cmd_preamble} ${binDir}/mergeEDR < ${mergeEDR_cntrl}_${ifile}"
      ${cmd_preamble} ${binDir}/mergeEDR < ${mergeEDR_cntrl}_${ifile}
...
#
# Description:
#	Prepare the SWE climatology files, by collocating with the FMSDRs
#
#
# Input Variables:
# 	- fmsdrDir: FMSDR data directory
# 	- extResol: Extension resolution
# 	- fmsdrList4PrepSwe: FMSDR data file list for preparing SWE climatology
# 	- prepSweClimodir: SWE climatology files output directory
# 	- SweClimoMeanFile: SWE gridded mean climatology file
# 	- SweClimoStdevFile: SWE gridded std dev climatology file
# 	- Logfile: Log file
# 	- prepswe_cntrl: PrepSweClimo control file
# 	- prepSweSrc: source file for prepSwe
# 	- makeOrNot: Switches between making (1) the executables on the fly or not (0)
# 	- binDir: Bin directory where all executables reside
# 	- processingMode: Processing mode (0:Orbit mode,  1:Daily mode)
# 	- OrbitInfo: Orbit information
...
#
#===============================================================
function prepSweClimo() {
    preamble
    if [[ $# -ne 18 ]]
    then
	op_msg "Error: function prepSweClimo: check number of arguments: $#"
	exit 1      
    fi
    local fmsdrDir=$1
    local extResol=$2
    local fmsdrList4PrepSwe=$3
    local prepSweClimodir=$4
    local SweClimoMeanFile=$5
    local SweClimoStdevFile=$6
    local Logfile=${7}
    local prepswe_cntrl=${8}
    local prepSweSrc=${9}
    local makeOrNot=${10}
    local binDir=${11}
    local processingMode=${12}
    local OrbitInfo=${13}
    local retrOnOrbitOrSubOrbit=${14}
    local fmsdrDir_Chopp=${15}
    local iSensor_ID=${16}
    local nOrbits2Process=${17}
    local algSN=${18}
...
#    fi
	...
	rm -f ${prepSweClimodir}DEPswecl*${OrbitInfo}* #purge files to avoid confusion in post-processing stage
    else
	
	rm -f ${prepSweClimodir}/DEPswecl* #purge files to avoid confusion in post-processing stage
    fi
    ConstructList ${processingMode} ${fmsdrDir0} ${OrbitInfo} ${fmsdrList4PrepSwe} "${pref}" "*${extResol}*"
    DataExistEst "FM-SDR" "for prepSWE" ${fmsdrList4PrepSwe}
    set  "ListRadMeasFiles='${fmsdrList4PrepSwe}'" 		\
	      "pathDep='${prepSweClimodir}/'" 			         \
	      "LogFile='${Logfile}'" 					            \
	      "sweClimoFile='${SweClimoMeanFile}'"			   \
	      "sweStdevFile='${SweClimoStdevFile}'"			   \
         "sensor_id=${iSensor_ID}" 				            \
	      "nOrbits2Process=${nOrbits2Process}"			   \
	      "AlgSN=${algSN}"
    CreatNamList ${prepswe_cntrl} '&ContrlPrepSwe' "$@"
    DoMake ${prepSweSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/prepSweClimo < ${prepswe_cntrl}"
    ${cmd_preamble} ${binDir}/prepSweClimo < ${prepswe_cntrl}
    checkStatus $? "prepSweClimo"
    echo "End of step prepSweClimo" 
}
...
#
# Description:
#	Prepare the ims collocation files, by collocating IMS snow/ice analysis with the FMSDRs
#
#
# Input Variables:
# 	- fmsdrDir: FMSDR data directory
# 	- extResol: Extension resolution
# 	- fmsdrList4imsColloc: FMSDR data file list for collocating with IMS
# 	- imsCollocdir: collocated IMS files output directory
#       - imsAnalysisPath: directory containing IMS analysis file
#       - date: processing date
#       - ndayLagIms: number of day lag between processing date and valid date of IMS analysis
# 	- Logfile: Log file
# 	- imscolloc_cntrl: imsColloc control file
# 	- imsCollocSrc: source file for imsColloc
# 	- makeOrNot: Switches between making (1) the executables on the fly or not (0)
# 	- binDir: Bin directory where all executables reside
# 	- processingMode: Processing mode (0:Orbit mode,  1:Daily mode)
# 	- OrbitInfo: Orbit information
...
#
#===============================================================
function imsCollocNwp() {
    preamble
    if [[ $# -ne 18 ]]
    then
	op_msg "Error: function imsCollocNwp: check number of arguments: $#"
	exit 1      
    fi
    local fmsdrDir=$1
    local extResol=$2
    local fmsdrList4imsColloc=$3
    local imsCollocdir=$4
    local imsFile=$5
    local ndayLagIms=${6}
    local Logfile=${7}
    local imscolloc_cntrl=${8}
    local imsCollocSrc=${9}
    local makeOrNot=${10}
    local binDir=${11}
    local processingMode=${12}
    local OrbitInfo=${13}
...
#	pref="CHP*"
#    fi
# always assume no chopping for the NWP collocation application
    fmsdrDir0=${fmsdrDir}
    pref=${ext1}
    if [ ${processingMode} == 0 ]
	then
	
	rm -f ${imsCollocdir}IMScol*${OrbitInfo}* #purge files to avoid confusion in post-processing stage
    else
	
	rm -f ${imsCollocdir}/IMScol* #purge files to avoid confusion in post-processing stage
    fi

    ConstructList ${processingMode} ${fmsdrDir0} ${OrbitInfo} ${fmsdrList4imsColloc} "${pref}" "*${extResol}*"
    DataExistEst "FM-SDR" "for imsColloc" ${fmsdrList4imsColloc}
    set  "ListRadMeasFiles='${fmsdrList4imsColloc}'" 		\
	      "pathDep='${imsCollocdir}/'" 			            \
	      "LogFile='${Logfile}'" 					            \
	      "imsFile='${imsFile}'"			                  \
         "maxDayLag=${ndayLagIms}" 				            \
         "sensor_id=${iSensor_ID}" 				            \
	      "nOrbits2Process=${nOrbits2Process}"			   \
	      "AlgSN=${algSN}"
    CreatNamList ${imscolloc_cntrl} '&ContrlImsColloc' "$@"
    DoMake ${imsCollocSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/imsColloc < ${imscolloc_cntrl}"
    ${cmd_preamble} ${binDir}/imsColloc < ${imscolloc_cntrl}
    checkStatus $? "imsColloc"
    echo "End of step imsCollocNwp" 
}
...
function imsColloc() {
    ...
    if [[ $# -ne 18 ]]
    then
	op_msg "Error: function imsColloc: check number of arguments: $#"
	exit 1      
    fi
    local fmsdrDir=$1
    local extResol=$2
...
#   Calling SCS will build assumed name of IMS data file 
#   and pass to this function as an argument (${imsFile})
#   Note: IMS archive file naming convention is such that
#   analysis for julian day jjjj is saved in
#   files with julian day stamp jjj+1
#   E.g. IMS analysis valid for 2015-01-03 (julian day 003)
#   is saved in file ims2015004_4km_v1.3.asc

#   Therefore, 
#   If user intends to use data valid on the actual
#   day of the retrieval (for research/testing/off-line calculations):
#   set ndayLagIms=0 in PCF
#   If user intends to use previous day's data (closer to operational confguration)
#   set ndayLagIms=-1 in PCF
#   This function will build the filename appropriately
...
#
# Description:
#	Prepare the forest fraction collocation files, by collocating VIIRS forest fraction with the FMSDRs
#
#
# Input Variables:
# 	- fmsdrDir: FMSDR data directory
# 	- extResol: Extension resolution
# 	- fmsdrList4ffColloc: FMSDR data file list for collocating with gridded forest fraction
# 	- ffCollocdir: collocated FF files output directory
#       - ffFile: Forest fraction file
#       - ffLatLonFile: Lat/Lon file corresponding to gridded FF
# 	- Logfile: Log file
# 	- ffcolloc_cntrl: ffColloc control file
# 	- ffCollocSrc: source file for ffColloc
# 	- makeOrNot: Switches between making (1) the executables on the fly or not (0)
# 	- binDir: Bin directory where all executables reside
# 	- processingMode: Processing mode (0:Orbit mode,  1:Daily mode)
# 	- OrbitInfo: Orbit information
...
function ffColloc() {
    ...
    if [[ $# -ne 18 ]]
    then
	op_msg "Error: function ffColloc: check number of arguments: $#"
	exit 1      
    fi
    local fmsdrDir=$1
    local extResol=$2
    local fmsdrList4ffColloc=$3
    local ffCollocdir=$4
    local ffFile=$5
    local ffLatLonFile=$6
    local Logfile=${7}
    local ffcolloc_cntrl=${8}
    local ffCollocSrc=${9}
    local makeOrNot=${10}
    local binDir=${11}
    local processingMode=${12}
...
#    fi
	...
	rm -f ${ffCollocdir}FFcol*${OrbitInfo}* #purge files to avoid confusion in post-processing stage
    else
	
	rm -f ${ffCollocdir}/FFcol* #purge files to avoid confusion in post-processing stage
    fi
    ConstructList ${processingMode} ${fmsdrDir0} ${OrbitInfo} ${fmsdrList4ffColloc} "${pref}" "*${extResol}*"
    DataExistEst "FM-SDR" "for ffColloc" ${fmsdrList4ffColloc}
    set  "ListRadMeasFiles='${fmsdrList4ffColloc}'" 		\
	      "pathDep='${ffCollocdir}/'" 			            \
	      "LogFile='${Logfile}'" 					            \
	      "forestFracFile='${ffFile}'"			            \
	      "forestFracLatLonFile='${ffLatLonFile}'"			\
         "sensor_id=${iSensor_ID}" 				            \
	      "nOrbits2Process=${nOrbits2Process}"			   \
	      "AlgSN=${algSN}"
    CreatNamList ${ffcolloc_cntrl} '&ContrlFFColloc' "$@"
    DoMake ${ffCollocSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/viirsForFracColloc < ${ffcolloc_cntrl}"
    ${cmd_preamble} ${binDir}/viirsForFracColloc < ${ffcolloc_cntrl}
    checkStatus $? "viirsForFracColloc"
    echo "End of step ffColloc" 
}
...
# 	- prepSweClimodir: SWE Climo Dir for vipp
# 	- prepSweClimoList4VIPP: Base name for list of SWE climo files
# 	- sweClimoUse: Flag to use (1) or not use (0) SWE climo
# 	- pathDEPOut: Output path for DEP
# 	- ffCollocdir: Forest fraction collocation directory
# 	- ffCollocList4VIPP: Base name for list of FF collocation files
# 	- ffUse: Flag to use (1) or not use (0) Forest Fraction
# 	- LogFile: Log file
# 	- norbits2process: Number of orbits to process
...
# 	- binDir: Bin directory where all executables reside
# 	- seaIceCatalog: Emissivity catalog for sea ice
# 	- snowCatalog: Emissivity catalog for snow
#
#
# Outputs:
#      - None
...
function vipp() {
    ...
    local edrDir4VIPP=$2
    local prepSweClimodir=$3
    local prepSweClimoList4VIPP=$4
    local sweClimoUse=$5
    local ffCollocdir=$6
    local ffCollocList4VIPP=$7
    local ffUse=$8
    local pathDEPOut=$9
    local LogFile=${10}
    local norbits2process=${11}
    local nprofs2process=${12}
    local sensor_id=${13}
    local processingMode=${14}
    local orbitInfo=${15}
    local extResol=${16}
    ...
    local vipp_src=${18}
    local binDir=${19}
    local seaIceCatalog=${20}
    local snowCatalog=${21}

    echo "=============== RUNNING VIPP STEP ======================="
    rm -f ${pathDEPOut}/DEP_*  #purge DEP files that may already exist
    ConstructList ${processingMode} ${edrDir4VIPP}  ${orbitInfo} ${edrList4VIPP} "EDR*" "*"${extResol}".ORB"
    echo "edrList4VIPP="${edrList4VIPP}
    if [ ${sweClimoUse} == 1 ]
	then
	ConstructList ${processingMode} ${prepSweClimodir} ${orbitInfo} ${prepSweClimoList4VIPP} "DEPswecl*" "*"${extResol}"*"
	DataExistEst "prepSweClimo" "for vipp" ${prepSweClimoList4VIPP}	
    fi
    if [ ${ffUse} == 1 ]
	then
	ConstructList ${processingMode} ${ffCollocdir} ${orbitInfo} ${ffCollocList4VIPP} "FFcol*" "*"${extResol}"*"
	DataExistEst "ffColloc" "for vipp" ${ffCollocList4VIPP}
    fi
	...
	"sensor_id=${sensor_id}"                \
	"sweClimoFilesList='${prepSweClimoList4VIPP}'" \
	"useSweClimo=${sweClimoUse}" \
	"ffCollocFilesList='${ffCollocList4VIPP}'" \
	"useFF=${ffUse}"
    CreatNamList ${vipp_cntrl} '&VIPPNMLST' "$@"
    DoMake ${vipp_src} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/vipp < ${vipp_cntrl}"
    ${cmd_preamble} ${binDir}/vipp < ${vipp_cntrl}
    checkStatus $? "vipp"
...
#
# Description:
#	Snow Fall Rate Retrieval
#
#
# Input Variables:
# 	- fmsdrDir: FMSDR data directory
#       - extResol: Extension resolution
#       - fmsdrList4sfr: FMSDR data file ist for applying sfr
#       - sfrDir: Snow fall rate retrieval directory
# 	- Logfile: Log file
# 	- sfrSrc: Sfr src dir
# 	- makeOrNot: Switches between making (1) the executables on the fly or not (0)
# 	- binDir: Bin directory where all executables reside
...
# 	- iSensor_ID: Sensor (1:N18,  2:METOPA)
# 	- nOrbits2Process: Number of orbits to process
# 	- sfr_cntrl: Sfr control file
#       - nwpSfrGridPath: Grid NWP path for SFR
#       - GridNWPSfrList: Grid NWP list for SFR
#       - inputDataPath: Inputs file directory
# 	- nprofs2process: Number of profiles to process
#       - sfr2dep_src: Sfr2dep src dir
#       - sfr2dep_cntrl: Sfr2dep control file
#	- sfrList: SFR files list
...
function sfr() {
    ...
    if [[ $# -ne 25 ]] 
	 then
	   op_msg "Error: function sfr : check number of arguments! Need 25, input $#"
	   exit 1
    fi
    local satId=$1 
    local sensorId=$2
    ...
    local retrOnOrbitOrSubOrbit=${14}
    local fmsdrDir_Chopp=${15}
    local nOrbits2Process=${16}
    local sfr_cntrl=${17}
    local nwpSfrGridPath=${18}
    local GridNWPSfrList=${19}
    local nprofs2process=${20}
    local sfr2dep_src=${21}
    local sfr2dep_cntrl=${22}
    local sfrList=${23}
    local depDir=${24}
    local depList=${25}
    ...
    echo "=============== RUNNING SFR/RETRIEVAL STEP ======================="
    #-----Switch between full-orbit and chopped orbit depending on retrOnOrbitOrSubOrbit
#    if [ ${retrOnOrbitOrSubOrbit} == 0 ]
#    then
	fmsdrDir0=${fmsdrDir}
...
#    fi
   ...
   #
   #-----Purge sfr files to avoid confusion in post-processing stage, and set up nwp path
   #
   sfrfile=" ${sfrDir}/SFR*"

   [ -f "${sfrfile}" ] && rm -f ${sfrfile}
   #
   #---set up nwp path for daily mode
   #
   #if [[ ${processingMode} == 1 ]] ; then
   #   nwpSfrGridPath=${nwpSfrGridPath}/${yyyy}${mm}${dd}
   #fi

   #
   #-----FMSDR list
   #
   ConstructList ${processingMode} ${fmsdrDir0} ${orbitInfo} ${fmsdrList4sfr} "${pref}" "*${extResol}*"
   DataExistEst "FM-SDR" "for sfr" ${fmsdrList4sfr}
   #
   #-----Control
   #
cat << EOF > ${sfr_cntrl}
...
% input: sat
%%%%%%%%%%%%%%%%%%%%%%%%%
% choice: satms, tatms, fmsdr, srs
s_iftype        fmsdr
s_flist0        ${fmsdrList4sfr}
s_flist1        xxxxxx.list 
...
%%%%%%%%%%%%%%%%%%%%%%%%%
a_name          avn
a_dn_pat        ${nwpSfrGridPath}
a_fn_pat        avn_comb{HH}.bin.{yyyy}{mm}{dd}

%%%%%%%%%%%%%%
% others
%%%%%%%%%%%%%%
ffn_lut         ${sfrDataPath}/snowShape_sphere_optics_{sen}.dat
ffn_mask        ${sfrDataPath}/mask.bin
ffn_lese        ${sfrDataPath}/lese_tab.dat

ffn_pin         ${sfrDataPath}/input_prod_{sat}_{sen}.dat
ffn_tis         ${sfrDataPath}/tis_{sat}_{sen}.dat
ffn_tisnn       ${sfrDataPath}/tis_{sat}_{sen}_NN.dat
ffn_tisxgb      ${sfrDataPath}/tis_{sat}_{sen}_XGB.dat
EOF

    DoMake ${sfrSrc} ${makeOrNot}
    cd ${binDir} || return
    cd ../scripts/  || return

    echo "${cmd_preamble} ${binDir}/sfr  ${sfr_cntrl}"
    ${cmd_preamble} ${binDir}/sfr  ${sfr_cntrl}

echo "=============== RUNNING SFR/SFR2DEP STEP ======================="
    ...
    for (( i=0; i<${nfile}; i++ )) ; do
      file=${files[$i]} 
      file1=${file/DEP_/SFR_}
	   echo "$file-->$file1"
	   cp $file $file1
    done

   ConstructList ${processingMode} ${depDir} ${orbitInfo} ${depList} "SFR*" "*"${extResol}".ORB"
   ConstructList ${processingMode} ${sfrDir} ${orbitInfo} ${sfrList} "SFR*" "*"${extResol}

    echo ${sfrList}
    set "depFilesList='${depList}'"             \
	     "sfrFilesList='${sfrList}'"	            \
        "depDir='${depDir}/'"		               \
	     "LogFile='${LogFile}'"			         \
        "norbits2process=${nOrbits2Process}"   	\
	     "nprofs2process=${nprofs2process}" 

    CreatNamList ${sfr2dep_cntrl} '&ContrlSfr2dep' "$@"
    DoMake ${sfr2dep_src} ${makeOrNot}

    echo "${cmd_preamble} ${binDir}/sfr2dep < ${sfr2dep_cntrl}"
    ${cmd_preamble} ${binDir}/sfr2dep < ${sfr2dep_cntrl}

#-----Purge intermediate SFR files
   if [ ${processingMode} == 0 ]
	then
	   rm -f ${depDir}/SFR*${OrbitInfo}* 
   else
	   rm -f ${depDir}/SFR*
   fi

    checkStatus $? "sfr"
    echo "End of step sfr"
}
...
function gridGen() {
    ...
    ConstructList ${processingMode} ${edrDir} ${OrbitInfo} ${edrList} "EDR*" "*"${extResol}"*"
    ConstructList ${processingMode} ${depDir} ${OrbitInfo} ${depList} "DEP*" "*"${extResol}"*"
    ...
    echo "${cmd_preamble} ${binDir}/gridEdr < ${grid_cntrl}_gridEdr"
    ${cmd_preamble} ${binDir}/gridEdr < ${grid_cntrl}_gridEdr
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/p2pEdr < ${grid_cntrl}_p2pEdr"
    ${cmd_preamble} ${binDir}/p2pEdr < ${grid_cntrl}_p2pEdr
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/gridDep < ${grid_cntrl}_gridDep"
    ${cmd_preamble} ${binDir}/gridDep < ${grid_cntrl}_gridDep
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/p2pDep < ${grid_cntrl}_p2pDep"
    ${cmd_preamble} ${binDir}/p2pDep < ${grid_cntrl}_p2pDep
...
function gridClimate() {
    ...
    echo "${cmd_preamble} ${binDir}/gridEdrClimate < ${grid_cntrl}_edr_climate_1.0"
    ${cmd_preamble} ${binDir}/gridEdrClimate < ${grid_cntrl}_edr_climate_1.0
    ...
    echo "${cmd_preamble} ${binDir}/gridEdrClimate < ${grid_cntrl}_edr_climate_2.5"
    ${cmd_preamble} ${binDir}/gridEdrClimate < ${grid_cntrl}_edr_climate_2.5
    ...
    echo "${cmd_preamble} binDir}/gridDepClimate < ${grid_cntrl}_dep_climate_1.0"
    ${cmd_preamble} ${binDir}/gridDepClimate < ${grid_cntrl}_dep_climate_1.0
    ...
    echo "${cmd_preamble} ${binDir}/gridDepClimate < ${grid_cntrl}_dep_climate_2.5"
    ${cmd_preamble} ${binDir}/gridDepClimate < ${grid_cntrl}_dep_climate_2.5
...
function dynamicBackground() {
    ...
    echo "${cmd_preamble} ${binDir}/dynamicBackground < ${grid_cntrl}_dynamicBackground"
    ${cmd_preamble} ${binDir}/dynamicBackground < ${grid_cntrl}_dynamicBackground
...
function gridRain() {
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/gridRain < ${grid_cntrl}_rain"
    ${cmd_preamble} ${binDir}/gridRain < ${grid_cntrl}_rain
...
mv ${tempDEPList} ${dep4BiasList}
    ...
    elif [[ ${isatId} -eq 3 || ${isatId} -eq 5 || ${isatId} -eq 18 ]] ; then
	> ${tempNWPList}
	ls ${edr4BiasDir}/EDR* > ${nwp4BiasList}_${nwpstr}
	sed ' 
...
s/edr/dep/g' ${dep4BiasList} > ${tempDEPList}
    ...
    elif [[ ${isatId} -eq 6 || ${isatId} -eq 20 || ${isatId} -eq 23 ]] ; then
	> ${tempNWPList}
	ls ${edr4BiasDir}/EDR* > ${nwp4BiasList}_${nwpstr}
	sed ' 
...
s/edr/dep/g' ${dep4BiasList} > ${tempDEPList}
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/gridEdr < ${gridNwp_cntrl}_${nwpstr}_gridEdr"
    ${cmd_preamble} ${binDir}/gridEdr < ${gridNwp_cntrl}_${nwpstr}_gridEdr
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/p2pEdr < ${gridNwp_cntrl}_${nwpstr}_p2pEdr"
    ${cmd_preamble} ${binDir}/p2pEdr < ${gridNwp_cntrl}_${nwpstr}_p2pEdr
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/gridRad < ${gridFwd_cntrl}_${nwpstr}_gridRad"
    ${cmd_preamble} ${binDir}/gridRad < ${gridFwd_cntrl}_${nwpstr}_gridRad
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/p2pRad < ${gridFwd_cntrl}_${nwpstr}_p2pRad"
    ${cmd_preamble} ${binDir}/p2pRad < ${gridFwd_cntrl}_${nwpstr}_p2pRad
    ...
    DoMake ${gridSrc} ${makeOrNot}
    echo "${cmd_preamble} ${binDir}/gridBias < ${gridBias_cntrl}_${nwpstr}"
    ${cmd_preamble} ${binDir}/gridBias < ${gridBias_cntrl}_${nwpstr}
...
function dataQualityMonitor() {
    ...
    if [[ ${satId} == "n20" ]] ; then
      find ${nedtPath} -maxdepth 1 -name "n20_atms_nedt_d2*befFM.dat" -print | sort > ${nedtList}
    fi
    if [[ ${satId} == "n21" ]] ; then
      find ${nedtPath} -maxdepth 1 -name "n21_atms_nedt_d2*befFM.dat" -print | sort > ${nedtList}
    fi
    if [[ ${satId} == "metopSGA1" ]] ; then
      find ${nedtPath} -maxdepth 1 -name "metopSGA1_mws_nedt_*befFM.dat" -print | sort > ${nedtList}
    fi
    if [[ ${satId} == "f16" || ${satId} == "f17" || ${satId} == "f18" || ${satId} == "f19" ]] ; then
    ...
    cd ${nedtMonitorSrc} || return
    if [[ ${satId} == "n18" || ${satId} == "n19" || ${satId} == "metopA" || ${satId} == "npp" || ${satId} == "n20" || ${satId} == "n21" || ${satId} == "metopB" || ${satId} == "metopC" || ${satId} == "metopSGA1" ]] ; then
    ...
    # this is for NPP, N20 ATMS daily granule NEDT monitoring
    if [[ ${satId} == "npp" || ${satId} == "n20" || ${satId} == "n21" || ${satId} == "metopSGA1" ]] ; then
    ...
    # this is for NPP, N20 ATMS daily granule QC monitoring
    if [[ ${satId} == "npp" || ${satId} == "n20" || ${satId} == "n21" || ${satId} == "metopSGA1" ]] ; then
...
#===============================================================
# Name:		    ExtractRdrFileNamesfromOrbitN20
#
#
# Type:		    Bash Shell Script
#
#
# Description:
#	Extract Orbit information from a single file input of N20 ATMS. The
#   create time is not included as it may differ from TDR/SDR and GATMO files.
#
#
# Input Variables:
# 	- rdrFile: RDR file (TATMS_n20_d20030125_t084657_e084833_b00015_c20071214193913_den_OPS_SEG.h5)
# 	- rdrPath: RDR path 
#
#
# Outputs:
...
#
#===============================================================
function ExtractRdrFileNamesfromOrbitN20() {
    preamble
    local rdrFile=$1
    local rdrPath=$2
    local orbitInfo=`echo ${rdrFile} | awk -F_ '{print$3"_"$4"_"$5"_"$6}'`
...
#===============================================================
# Name:		    ExtractRdrFileNamesfromOrbitN21
#
#
# Type:		    Bash Shell Script
#
...
#
# Input Variables:
# 	- rdrFile: RDR file (TATMS_n21_d20030125_t084657_e084833_b00015_c20071214193913_den_OPS_SEG.h5)
# 	- rdrPath: RDR path 
#
#
# Outputs:
#      - Orbit information ( EDR_TMS_n21_d20200224_t1933093_e1933409_b11754_c20200224194808315530_nobc_ops.h5.HR.ORB )
#
#===============================================================
function ExtractRdrFileNamesfromOrbitN21() {
    preamble
    local rdrFile=$1
    local rdrPath=$2
    local orbitInfo=`echo ${rdrFile} | awk -F_ '{print$3"_"$4"_"$5"_"$6}'`
...
#===============================================================
# Name:		    ExtractRdrFileNamesfromOrbitmetopSGA1
#
#
# Type:		    Bash Shell Script
#
#
# Description:
#	Extract Orbit information from a single file input of metopSGA1 MWS. The
#   create time is not included as it may differ from MWS files.
#
#
# Input Variables:
# 	- rdrFile: RDR file (EPS_SG_MWS_1B_RAD.nc)
# 	- rdrPath: RDR path 
#
#
# Outputs:
...
#
#===============================================================
function ExtractRdrFileNamesfromOrbitmetopSGA1() {
    preamble
    local rdrFile=$1
    local rdrPath=$2
    local orbitInfo=`echo ${rdrFile} | awk -F_ '{print$3"_"$4"_"$5"_"$6}'`
...
#===============================================================
# Name:		    ExtractRdrFileNamesfromOrbitGPM
#
#
# Type:		    Bash Shell Script
#
#
# Description:
#	Extract Orbit information from a single file input of GPM GMI
#
#
# Input Variables:
# 	- rdrFile: RDR file (1C-R.GPM.GMI.XCAL2015-C.20160810-S112347-E125621.013922.V04A.HDF5)
# 	- rdrPath: RDR path 
#
#
# Outputs:
#      - Orbit information (20160810-S112347-E125621)
#
#===============================================================
function ExtractRdrFileNamesfromOrbitGPM() {
    preamble
    local rdrFile=$1
    local rdrPath=$2
    local orbitInfo=`echo ${rdrFile} | awk -F. '{print$5}'`
    echo ${orbitInfo}
}
...
function SanityChecksOrbitProcess() {
   ...
   echo '${Sat_ID}'
   if [[ "${Sat_ID}" != "NN" && "${Sat_ID}" != "M2" && "${Sat_ID}" != "NP" && "${Sat_ID}" != "M1" && "${Sat_ID}" != "M3" ]]
   then
...
function determineSpatialResol() {
    ...
    if [[ ${satId} == "n18" || ${satId} == "metopA" || ${satId} == "n19" || ${satId} == "npp" || ${satId} == "n20" || ${satId} == "n21" || ${satId} == "metopB" || ${satId} == "metopC" || ${satId} == "metopSGA1" ]] ; then
...
function getDateFromFile() {
    ...
    if [[ ${satId} == "n18" || ${satId} == "n19" || ${satId} == "metopA" || ${satId} == "metopB" || ${satId} == "metopC" ]] ; then
	...
	if [[ ${filenum_amax} -eq ${filenum_mhsx} && ${filenum_amax} -gt 0 ]] ; then
        ...
        filenum=`ls -1 ${inputPath}/*.nc 2> /dev/null | wc -l` 
        if [[ ${filenum} -gt 0 ]] ; then
	    file=`ls ${inputPath}/*.nc | head -1`
	    file=`basename ${file}`
	fi
    elif [[ ${satId} == "trmm" ]] ; then
        ...
        filenum=`ls -1 ${inputPath}/1C-R* 2> /dev/null | wc -l` 
        if [[ ${filenum} -gt 0 ]] ; then
	    file=`ls ${inputPath}/1C-R* | head -1`
	    file=`basename ${file}`
	fi
    fi
    ...
    if [[ ${file} != "" ]] ; then
      ...
      if [[ ${satId} == "n18" || ${satId} == "n19" || ${satId} == "metopA" || ${satId} == "f16" || ${satId} == "f17" || ${satId} == "f18" || ${satId} == "f19" || ${satId} == "metopB" || ${satId} == "metopC" ]] ; then
	...
	local yyyymmdd=`echo $file | cut -c16-23`
	local yyyy=`echo ${yyyymmdd} | cut -c1-4`
	local mm=`echo ${yyyymmdd} | cut -c5-6`
	local dd=`echo ${yyyymmdd} | cut -c7-8`
	ymd=$yyyy-$mm-$dd
	...
	local yyyy=`echo ${file} | cut -c25-28`
	local mm=`echo ${file}   | cut -c29-30`
	local dd=`echo ${file}   | cut -c31-32`
	ymd=$yyyy-$mm-$dd
...
function WriteListOfOutputs() {	
    ...
    if [ -n "${edrFile}" ]
    then
	echo `grep EDR ${edrList} >> ${pathOut}/${ListFileName}`
    else
...
function getNextDay() {
	if [[ $# -ne 1 ]] ; then
		op_msg "Error: function getNextDay: check number of arguments: $#"
		exit 1
	fi
	yyyymmdd=$1
	yyyymmdd1=`date --date="$yyyymmdd + 1 day" +%Y%m%d`
	}

#===============================================================
# Name:		    getMiRSDAPversion
#
# Type:		    Bash Shell Script
#
# Description:
#	Gets the version information for the DAP
#
# Output Variables:
#	- MiRSDAPmajor:	The DAP major version number
#	- MiRSDAPminor:	The DAP minor version number
#	- MiRSDAPminorS: The DAP minor version number, with no leading zeros.
#	- MiRSDAPfull:	The version number from version.txt
#
#
#===============================================================

function getMiRSDAPversion() {
	local scriptdir=`dirname "$BASH_SOURCE"`
	local IN_DAP_MAJOR=`head -n 1 $scriptdir/../setup/dap_version.txt`
	MiRSDAPminorS=`head -n 2 $scriptdir/../setup/dap_version.txt | tail -n 1`
	MiRSDAPmajor=`printf "%02d\n" $IN_DAP_MAJOR`
	MiRSDAPminor=`printf "%02d\n" $MiRSDAPminorS`
	MiRSDAPfull=`cat ${scriptdir}/../version.txt`
   historyPackage=`cat ${scriptdir}/../setup/history_package.txt`
	}
...
#
# Description:
#	Tells the caller if it needs to check RDR file names to find matches to an EDR file
#
# Input Variables:
#
#       - 1. satId:	The satellite id
#
# Output Variables:
#	- isRDRneeded:	0 for false, 1 for true
#
#
#===============================================================

function needsRDRcheck() {
	if [[ $# -ne 1 ]] ; then
		op_msg "Error: function needsRDRcheck: check number of arguments: $#"
		exit 1
	fi
	local satId=$1
	case "$satId" in
		npp | n20 | n21 | metopSGA1)
			isRDRneeded=1
			;;
		*)
			isRDRneeded=0
	...
	# Verify that the output variables are set
	if [ -z ${isRDRneeded+x} ] ; then
		op_msg "Error: needsRDRcheck did not set variable isRDRneeded"
		exit 1
		fi
	}

#===============================================================
# Name:		    getTimesFromRDR
#
# Type:		    Bash Shell Script
#
# Description:
#	Gets the time ranges from RDR files
#
# Input Variables:
#
#       - 1. rdr:	The RDR file name
#	- 2/ satId:	The satellite id
#
# Output Variables:
#	- isRDRneeded:	0 for false, 1 for true
#	- yyyymmdd_rdr:	The date of the rdr file. Format varies per satellite. Only returned if isRDRneeded == 1.
#	- HMS_s_rdr:	The start time of the rdr file. Format varies per satellite. Only returned if isRDRneeded == 1.
#	- HMS_e_rdr:	The end time of the rdr file. Format varies per satellite. Only returned if isRDRneeded == 1.
#
#
#===============================================================

function getTimesFromRDR() {
	if [[ $# -ne 2 ]] ; then
		op_msg "Error: function getTimesFromRDR: check number of arguments: $#"
		exit 1
	fi
	local rdr=$1
	local satId=$2
	needsRDRcheck $satId
	if [[ ${isRDRneeded} -eq 1 ]] ; then
		case "$satId" in
			npp | n20 | n21 | metopSGA1)
				# Example name:
				# TATMS_j01_d20180425_t2351360_e2352076_b02251_c20180426001357410411_nobc_ops.h5
				yyyymmdd_rdr=`echo ${rdr} | cut -f3 -d_ | cut -c2-9`
				#echo "yyyymmdd_rdr= "${yyyymmdd_rdr}
				HMS_s_rdr=`echo ${rdr} | cut -f4 -d_ | cut -c2-8`
				#echo "HMS_s_rdr= "${HMS_s_rdr}
				HMS_e_rdr=`echo ${rdr} | cut -f5 -d_ | cut -c2-8`
				#echo "HMS_e_rdr= "${HMS_e_rdr}
				;;
			*) # All of the other satellite cases.
				op_msg "Error: invalid sat id $satId at getTimesFromRDR. needsRDRcheck returned false, so this branch should never be reached."
				exit 1
				;;
			esac
		else
	...
	# Verify that the output variables are set
	if [ -z ${yyyymmdd_rdr+x} ]; then
		op_msg "Error: getTimesFromRDR did not set variable yyyymmdd_rdr"
		exit 1
		fi
	if [ -z ${HMS_s_rdr+x} ]; then
		op_msg "Error: getTimesFromRDR did not set variable HMS_s_rdr"
		exit 1
		fi
	if [ -z ${HMS_e_rdr+x} ]; then
		op_msg "Error: getTimesFromRDR did not set variable HMS_e_rdr"
		exit 1
		fi
	}

#===============================================================
# Name:		    getNamesFromEDR
#
# Type:		    Bash Shell Script
#
# Description:
#	Construct the output IMG and SND file names from several relevant variables.
#
# Input Variables:
#
#       - 1. yyyymmdd:	Start date
#	- 2. HMS_s:	Start time (HHMMSSS)
#	- 3. yyyymmdd1:	End date
#	- 4. HMS_e:	End time (HHMMSSS)
#	- 5. satId:	The satellite id
#	- 6. edr:	The input EDR file's name
#
# Output Variables:
#	- imgFile:	The name of the IMG file
#	- sndFile:	The name of the SND file
#
#
#===============================================================

function getNamesFromEDR() {
	if [[ $# -ne 6 ]] ; then
		op_msg "Error: function getNamesFromEDR: check number of arguments: $#"
		#echo -e "Error: function getNamesFromEDR: check number of arguments: $#"
		exit 1
	fi
	local yyyymmdd=$1	# Date
	local HMS_s=$2		# Start time
	local yyyymmdd1=$3	# Date + 1 day
	local HMS_e=$4		# End time
	local satId=$5		# The satellite id
	local edr=$6		# The edr file name

	# Mildly hidden variable - use an existing timev if SCRIPT_TESTING_MODE is defined.
	# If not defined, set it within the function.
	# Needed for unit testing.
	if [ -z ${SCRIPT_TESTING_MODE+x} ]; then
		local timev=`date +%Y%m%d%H%M%S`
		else
		if [ -z ${timev+x} ]; then
			op_msg "Error: SCRIPT_TESTING_MODE, but not setting the timev variable."
			exit 1
			fi
		fi

	getMiRSDAPversion
	local dapver=v${MiRSDAPmajor}r${MiRSDAPminorS}

	case "$satId" in
		npp)
			local part=s${yyyymmdd}${HMS_s}_e${yyyymmdd1}${HMS_e}_c${timev}0.nc
			imgFile=NPR-MIRS-IMG_${dapver}_${satId}_${part}
			sndFile=NPR-MIRS-SND_${dapver}_${satId}_${part}
			;;
		n20)
			local part=s${yyyymmdd}${HMS_s}_e${yyyymmdd1}${HMS_e}_c${timev}0.nc
			imgFile=NPR-MIRS-IMG_${dapver}_${satId}_${part}
		...
		metopB | ma1)
			local part=s${yyyymmdd}${HMS_s}_e${yyyymmdd1}${HMS_e}_c${timev}0.nc
			imgFile=NPR-MIRS-IMG_${dapver}_ma1_${part}
			sndFile=NPR-MIRS-SND_${dapver}_ma1_${part}
			;;
		metopA | ma2)
			local part=s${yyyymmdd}${HMS_s}_e${yyyymmdd1}${HMS_e}_c${timev}0.nc
			imgFile=NPR-MIRS-IMG_${dapver}_ma2_${part}
			sndFile=NPR-MIRS-SND_${dapver}_ma2_${part}
			;;
		metopC | ma3)
			local part=s${yyyymmdd}${HMS_s}_e${yyyymmdd1}${HMS_e}_c${timev}0.nc
			imgFile=NPR-MIRS-IMG_${dapver}_ma3_${part}
			sndFile=NPR-MIRS-SND_${dapver}_ma3_${part}
			;;
		gpm) # From ospo
			local part=s${yyyymmdd}${HMS_s}_e${yyyymmdd1}${HMS_e}_c${timev}0.nc
			imgFile=NPR-MIRS-IMG_${dapver}_${satId}_${part}
		...
		*)
			...
			#imgFile=${edr/EDR_/IMG_}.nc
			#sndFile=${edr/EDR_/SND_}.nc
			#op_msg "Error: invalid sat id $satId"
			#exit 1
			;;
		esac
	# Verify that imgFile and sndFile are set
	if [ -z ${imgFile+x} ]; then
		op_msg "Error: getNamesFromEDR did not set variable imgFile"
		exit 1
		fi
	if [ -z ${sndFile+x} ]; then
		op_msg "Error: getNamesFromEDR did not set variable sndFile"
		exit 1
		fi
	}

#===============================================================
# Name:		    getTimesFromEDR
#
# Type:		    Bash Shell Script
#
# Description:
#	Takes an EDR file name and extracts start/end dates and times.
#
# Input Variables:
#
#	- 1. edr:	The input EDR file name
#	- 2. satId:	The satellite id
#
# Output Variables:
#	- imgFile:	The name of the IMG file
#	- sndFile:	The name of the SND file
#       - yyyymmdd:	Start date
#	- HMS_s:	Start time (HHMMSSS)
#	- yyyymmdd1:	End date
#	- HMS_e:	End time (HHMMSSS)
#
#
#===============================================================

function getTimesFromEDR() {
	if [[ $# -ne 2 ]] ; then
		op_msg "Error: function getTimesFromEDR: check number of arguments: $#"
		exit 1
	fi
	local edr=$1
	local satId=$2
	# Is the end date set? Then, unset it.
	if [ -n ${yyyymmdd1} ]; then
		unset yyyymmdd1
	fi
	...
	case "$satId" in
		npp | n20 | n21 | metopSGA1)
			# npp: EDR_TMS_npp_d20180401_t2350000_e2350316_b33307_c20180402003821814837_nobc_ops.h5.HR.ORB
			# n20: EDR_TMS_j01_d20180321_t2355146_e2355463_b01754_c20180322002454540589_nobc_ops.h5.HR.ORB
			# n21: EDR_TMS_j02_d20180321_t2355146_e2355463_b01754_c20180322002454540589_nobc_ops.h5.HR.ORB
			yyyymmdd=`echo ${edr} | cut -f4 -d_ | cut -c2-9` 
			HMS_s=`echo ${edr} | cut -f5 -d_ | cut -c2-8` 
			HMS_e=`echo ${edr} | cut -f6 -d_ | cut -c2-8` 
			local s_val=$(echo $HMS_s | sed 's/0*//')
			local e_val=$(echo $HMS_e | sed 's/0*//')
			;;
		gpm)
			# EDR_1C-R.GPM.GMI.XCAL2016-C.20180219-S210140-E223413.022608.V05A.HDF5.HR_001
			# EDR_1C-R.GPM.GMI.XCAL2015-C.20160810-S112347-E125621.013922.V04A.HDF5.HR_001
			yyyymmdd=`echo ${edr} | cut -f5 -d'.' | cut -c1-8`
			HMS_s=`echo ${edr} | cut -f5 -d'.' | cut -c11-16`0
			HMS_e=`echo ${edr} | cut -f5 -d'.' | cut -c19-24`0
			local s_val=$(echo $HMS_s | sed 's/0*//')
			local e_val=$(echo $HMS_e | sed 's/0*//')
			;;
		mtsa)
			# EDR_MT1SAPSL1A2_1.08_000_1_19_I_2018_04_08_11_14_25_2018_04_08_13_02_12_33769_33770_349_14_15_HBX_00.h5.HR_002
			yyyymdd=`echo ${edr} | cut -d'_' -f 8-10 | sed 's/_//g'`
			yyyymdd1=`echo ${edr} | cut -d'_' -f 14-16 | sed 's/_//g'`
			HMS_s=`echo ${edr} | cut -d'_' -f 11-13 | sed 's/_//g'`0
			HMS_e=`echo ${edr} | cut -d'_' -f 17-19 | sed 's/_//g'`0
			#local s_val=
			#local e_val=
			;;
		f17 | f18 | n18 | n19 | ma1 | ma2 | metopA | metopB | metopC)
			# EDR_SN.SB.D14255.S0127.E0312.B4052122.NS.ENV.ORB
			#EDR_SN.SC.D16355.S0855.E1107.B3699293.NS.ENV_001
			#EDR_SN.SB.D16354.S2055.E2237.B5224748.NS.ENV_001
			#EDR_SX.M1.D16355.S0208.E0306.B2208384.SV.HR_001
			#EDR_SX.M2.D16355.S0028.E0211.B5276970.SV.HR_001
			#EDR_SX.NN.D16354.S2004.E2127.B5969495.WI.HR_001
			#EDR_SX.NP.D16354.S1423.E1609.B4053132.GC.HR_001
			local BEGINNINGyy="20"
			local ENDINGyy=`echo ${edr} | cut -f3 -d'.' | cut -c2-3`
			local julianday=`echo ${edr} | cut -f3 -d'.' | cut -c4-6`
			#local convertJULIAN_mmdd=$(date -d "`date +%Y` -01-01 +$(( ${julianday} - 1))days" +%m%d)
			# Bugfix: Ryan: 2019-01-23 - some julian dates were treated as octal numbers.
			#Bad: local convertJULIAN_mmdd=$(date -d "${BEGINNINGyy}${ENDINGyy}-01-01 +$(( ${julianday} - 1))days" +%m%d)
			local convertJULIAN_mmdd=$(date -d "${BEGINNINGyy}${ENDINGyy}-01-01 +$(( (10#${julianday}) - 1))days" +%m%d)


			yyyymmdd=${BEGINNINGyy}${ENDINGyy}${convertJULIAN_mmdd}

			local HM_s=`echo ${edr} | cut -f4 -d'.' | cut -c2-5`
			local HM_e=`echo ${edr} | cut -f5 -d'.' | cut -c2-5`
			#NOTE: THE EDR FILENAMES (NOT FOR GPM OR S-NPP) HAVE NO SECONDS. Padding here.
			HMS_s=${HM_s}000
			HMS_e=${HM_e}000

			local s_val=$(echo $HM_s | sed 's/0*//')
			local e_val=$(echo $HM_e | sed 's/0*//')
			;;
		*)
			op_msg "Error: invalid sat id $satId"
			exit 1
	...
	# Is the end date unset? Then, figure it out.
	if [ -z ${yyyymmdd1+x} ]; then
		# Does the file span two dates? If so, adjust the end date.
		if ((  s_val <= e_val )) ; then
			echo "Normal case: HMS_s <= HMS_e"
			yyyymmdd1=${yyyymmdd}
		else
			#echo "Date change case: HMS_s > HMS_e"
			getNextDay ${yyyymmdd}
			#yyyymmdd1=`date --date="$yyyymmdd + 1 day" +%Y%m%d`
		fi
	fi

	# Verify that the mandatory return variables are set. Those are: yyyymmdd, yyyymmdd1, HMS_s, HMS_e
	if [ -z ${yyyymmdd+x} ]; then
		op_msg "Error: getTimesFromEDR did not set variable yyyymmdd"
		exit 1
		fi
	if [ -z ${yyyymmdd1+x} ]; then
		op_msg "Error: getTimesFromEDR did not set variable yyyymmdd1"
		exit 1
		fi
	if [ -z ${HMS_s+x} ]; then
		op_msg "Error: getTimesFromEDR did not set variable HMS_s"
		exit 1
		fi
	if [ -z ${HMS_e+x} ]; then
		op_msg "Error: getTimesFromEDR did not set variable HMS_e"
		exit 1
		fi

	# This sets imgFile and sndFile. The function assures that these vars are set (no check needed).
	getNamesFromEDR $yyyymmdd $HMS_s $yyyymmdd1 $HMS_e $satId $edr
	}
...
#       - rdrListFile: ???
#       - rdrType    : ???
#       - hdfBin     : location of the HDF5 binary directory.
#                      Needed to extract orbit numbers from RDRs.
#       - prodSite   : Production site
...
function mirs2nc() {
	...
	# This sets the DAP major and minor version numbers MiRSDAPmajor MiRSDAPminorS
	getMiRSDAPversion

	local edrList=`ls -1 ${edrPath}/EDR*ORB`
	local depList=`ls -1 ${depPath}/DEP*ORB`
	local rdrList=`cat ${rdrListFile}`
	#echo "rdrList= " $rdrList

	local edrFiles=($edrList)
	local depFiles=($depList)
	local rdrFiles=($rdrList)

	local nedr=${#edrFiles[@]}
	local ndep=${#depFiles[@]}
	local nrdr=${#rdrFiles[@]}

	echo "rdrListFile= " $rdrListFile
	echo "rdrType= " $rdrType
	echo "nedr,ndep,nrdr= " $nedr, $ndep, $nrdr
	echo "prodSite=" $prodSite
	echo "prodEnv=" $prodEnv

	# 1) if npp or j1, within loop over EDR files loop over all RDR files and compare with EDR files
	#    finding RDR files that match EDR files using date and time information
	# 2) once matching RDR is found extract the  starting and ending orbit numbers of each RDR file
	# 3) the orbit numbers can then be passed to mirs2nc as additional arguments
	#    which are then written to the global attributes of the netCDF file

	local jstart=0
	local hdfdumpStr="/Data_Products/ATMS-TDR/ATMS-TDR_Aggr"
	if [[ $rdrType -eq 3 ]] ; then
		hdfdumpStr="/Data_Products/ATMS-SDR/ATMS-SDR_Aggr"
	fi
	echo "hdfdumpStr= "$hdfdumpStr
	...
	for (( i=0; i<${nedr}; i++ )) ; do
		...
		# Defines imgFile, sndFile, yyyymmdd, yyyymmdd1, HMS_s, HMS_e
		getTimesFromEDR $edr $satId

		# Check if we need to get the orbit numbers from the RDR files for this satellite.
		needsRDRcheck $satId #---npp,n20,n21
		# @param[out] isRDRneeded 	int		- 0 for false, 1 for true.
		echo "For this satellite, isRDRneeded is $isRDRneeded"
		if [[ ${isRDRneeded} == "1" ]] ; then
			# search RDR file list and find matching file to EDR file based on date/time strings
			# then extract the start and end orbit numbers
			for (( j=jstart; j<${nrdr}; j++ )) ; do
				#echo "j=" ${j}
				local rdrFile=${rdrFiles[$j]}
				#echo "rdrFile= "$rdrFile

				local rdr=`basename ${rdrFile}`
				# Defines yyyymmdd_rdr, HMS_s_rdr, HMS_e_rdr
				getTimesFromRDR $rdr $satId

				if [[ ${yyyymmdd_rdr} == ${yyyymmdd} && ${HMS_s_rdr} == ${HMS_s} && ${HMS_e_rdr} == ${HMS_e} ]] ; then
					echo "Orbit info matched RDR file found for EDR file:" ${edr}
					echo "RDR file is: " $rdr
					local orbitBegin=`${hdfBin}/h5dump -a ${hdfdumpStr}/AggregateBeginningOrbitNumber $rdrFile | grep "0,0" | awk '{print $2}'`
					local orbitEnd=`${hdfBin}/h5dump -a ${hdfdumpStr}/AggregateEndingOrbitNumber $rdrFile | grep "0,0" | awk '{print $2}'`
					echo "orbitBegin, orbitEnd= " ${orbitBegin} ${orbitEnd}
					let jstart=j+1
					#echo "jstart=" $jstart
					let rdrMatch=1
				break
			...
			let start_orbit=orbitBegin
			let end_orbit=orbitEnd
		fi

      if [[ "${satId}" == "metopA" || "${satId}" == "metopB" || "${satId}" == "metopC" ]] ; then
         local orbitRoot=`echo ${edr} | cut -f6 -d'.' | cut -c2-4`
         local orbitBegin=`echo ${edr} | cut -f6 -d'.' | cut -c2-6`
         local orbitEnd0=`echo ${edr} | cut -f6 -d'.' | cut -c7-8`
         local orbitEnd=${orbitRoot}${orbitEnd0}

         start_orbit=${orbitBegin}
         end_orbit=${orbitEnd}

      elif [[ "${satId}" == "gpm" ]] ; then #EDR_1C-R.GPM.GMI.XCAL2016-C.20200701-S192640-E193138.V05A.RT-H5.HR.ORB
         start_orbit=`echo ${edr} | cut -f4 -d'-' | cut -c2-7`
         end_orbit=`echo ${edr} | cut -f5 -d'-' | cut -c2-7`
      fi

		# echo "start_orbit, end_orbit=" $start_orbit $end_orbit
		echo "${cmd_preamble} ${exePath}/mirs2nc ${edrFile} ${depFile} ${ncPath}/ ${imgFile} ${sndFile} ${start_orbit} ${end_orbit} ${prodSite} ${prodEnv} ${MiRSDAPmajor} ${MiRSDAPminorS} ${historyPackage}"
		${cmd_preamble} ${exePath}/mirs2nc ${edrFile} ${depFile} ${ncPath}/ ${imgFile} ${sndFile} ${start_orbit} ${end_orbit} ${prodSite} ${prodEnv} ${MiRSDAPmajor} ${MiRSDAPminorS} ${historyPackage}
	done
...
#       - exePath    : path of converter executable
#       - satId      : satellite string id
#       --- See others from mirs2nc (rdrListFile, rdrType,
#           hdfBin, prodSite, prodEnv).
#
#
#===============================================================
...
#
# 	- satId: Satellite short name (n18,metopA,n19,f16,f18,trmm,npp,n20,n21 etc)
...
# Input Variables:
#
#	- satId: satellite ID ( n18/metopA/n19/f16/f18/trmm/npp/n20,n21 etc)
...
function qcRadioGeo() {
    ...
    if [[ ! -e ${fileQc} ]] ; then
	  ...
	  "prodStr='${edrGridStr}'" 
      CreatNamList ${grid_cntrl}_gridEdr '&gridEdrNameList' "$@"
	echo "${cmd_preamble} }/gridEdr < ${grid_cntrl}_gridEdr"
      ${cmd_preamble} ${binDir}/gridEdr < ${grid_cntrl}_gridEdr
	...
	echo "${cmd_preamble} }/gridEdr < ${grid_cntrl}_${nwptxt}_gridEdr"
      ${cmd_preamble} ${binDir}/gridEdr < ${grid_cntrl}_${nwptxt}_gridEdr
	...
	echo "${cmd_preamble} ${binDir}/gridRad < ${grid_cntrl}_${nwptxt}_gridRad"
      ${cmd_preamble} ${binDir}/gridRad < ${grid_cntrl}_${nwptxt}_gridRad
	...
	echo "${cmd_preamble} binDir}/gridBias < ${grid_cntrl}_${nwptxt}_gridBias"
      ${cmd_preamble} ${binDir}/gridBias < ${grid_cntrl}_${nwptxt}_gridBias
	...
	echo "${cmd_preamble} }/p2pEdr < ${grid_cntrl}_${nwptxt}_p2pEdr"
      ${cmd_preamble} ${binDir}/p2pEdr < ${grid_cntrl}_${nwptxt}_p2pEdr
	...
	echo "${cmd_preamble} ${binDir}/p2pRad < ${grid_cntrl}_${nwptxt}_p2pRad"
      ${cmd_preamble} ${binDir}/p2pRad < ${grid_cntrl}_${nwptxt}_p2pRad
...
function emspectrum() {
  ...
  echo "${cmd_preamble} ${dirBin}/emspectrum < ${namelist1}"
  ${cmd_preamble} ${dirBin}/emspectrum < ${namelist1}
...
function gridGen_parallel() {
    ...
    for prod in ${prods[@]}; do
      ...
      DoMake ${gridSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/gridEdr < ${grid_cntrl}_gridEdr_${prod} &"
      ${cmd_preamble} ${binDir}/gridEdr < ${grid_cntrl}_gridEdr_${prod} &
    ...
    for prod in ${prods[@]}; do
      ...
      DoMake ${gridSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/p2pEdr < ${grid_cntrl}_p2pEdr_${prod} &"
      ${cmd_preamble} ${binDir}/p2pEdr < ${grid_cntrl}_p2pEdr_${prod} &
    ...
    for prod in ${prods[@]}; do
      ...
      DoMake ${gridSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/gridDep < ${grid_cntrl}_gridDep_${prod} &"
      ${cmd_preamble} ${binDir}/gridDep < ${grid_cntrl}_gridDep_${prod} &
    ...
    for prod in ${prods[@]}; do
      ...
      DoMake ${gridSrc} ${makeOrNot}
	echo "${cmd_preamble} ${binDir}/p2pDep < ${grid_cntrl}_p2pDep_${prod} &"
      ${cmd_preamble} ${binDir}/p2pDep < ${grid_cntrl}_p2pDep_${prod} &
...
# 	- extResol: Extension resolution (CR/LR/HR/UAS/LAS/EVN/IMG)
# 	- satId: string satellite id (n18/n19/metopA/f16/f18/trmm/npp/n20/n21)
# 	- fileExt:  File extension ( yyyy-mm-dd, etc)
# 	- gridSrc: Grid generation source code directory
...
function biasFigsGen_parallel() {
    ...
    for prod in ${prods[@]}; do
      ...
      DoMake ${gridSrc} ${makeOrNot}
      echo "${cmd_preamble} ${binDir}/gridEdr < ${gridNwp_cntrl}_${nwpstr}_gridEdr_${prod} &"
      ${cmd_preamble} ${binDir}/gridEdr < ${gridNwp_cntrl}_${nwpstr}_gridEdr_${prod} &
    ...
    for prod in ${prods[@]}; do
      ...
      DoMake ${gridSrc} ${makeOrNot}
      echo "${cmd_preamble} ${binDir}/p2pEdr < ${gridNwp_cntrl}_${nwpstr}_p2pEdr_${prod} &"
      ${cmd_preamble} ${binDir}/p2pEdr < ${gridNwp_cntrl}_${nwpstr}_p2pEdr_${prod} &
...
# This is only for NPP and N20 ATMS granule case only
...
function determineNedtExt() {
  ...
  local file1
  local filen
  if [[ ${rdrType} -eq 3 && ${satId} == 'npp' ]] ; then
    file1=`ls -1 ${dirRdr}/SATMS_npp_d* | head -1`
    filen=`ls -1 ${dirRdr}/SATMS_npp_d* | tail -1`
  elif [[ ${rdrType} -eq 1 && ${satId} == 'npp' ]] ; then
    file1=`ls -1 ${dirRdr}/TATMS_npp_d* | head -1`
    filen=`ls -1 ${dirRdr}/TATMS_npp_d* | tail -1`
  fi

  if [[ ${rdrType} -eq 3 && ${satId} == 'n20' ]] ; then
    file1=`ls -1 ${dirRdr}/SATMS_j01_d* | head -1`
    filen=`ls -1 ${dirRdr}/SATMS_j01_d* | tail -1`
  elif [[ ${rdrType} -eq 1 && ${satId} == 'n20' ]] ; then
    file1=`ls -1 ${dirRdr}/TATMS_j01_d* | head -1`
    filen=`ls -1 ${dirRdr}/TATMS_j01_d* | tail -1`
  fi

  if [[ ${rdrType} -eq 3 && ${satId} == 'n21' ]] ; then
    file1=`ls -1 ${dirRdr}/SATMS_j02_d* | head -1`
    filen=`ls -1 ${dirRdr}/SATMS_j02_d* | tail -1`
  elif [[ ${rdrType} -eq 1 && ${satId} == 'n21' ]] ; then
    file1=`ls -1 ${dirRdr}/TATMS_j02_d* | head -1`
    filen=`ls -1 ${dirRdr}/TATMS_j02_d* | tail -1`
  fi
 
  if [[ ${satId} == 'metopSGA1' ]] ; then
    file1=`ls -1 ${dirRdr}/MWS_metopSGA1_d* | head -1`
    filen=`ls -1 ${dirRdr}/MWS_metopSGA1_d* | tail -1`
  fi
...
function qcAvg() {
    ...
    local julday=`date --date=${yyyymmdd} +%j`
    local yy=`echo $ymd | cut -c3-4`
    ...
    if [[ ${satId} == 'npp' || ${satId} == 'n20' || ${satId} == 'n21' ]] ; then
      ...
      mv -f ${pathMon}/QC_MON.M1.D${yy}${julday}.S*HR.ORB ${pathMon}/${ymd}/ 2> /dev/null
      file_qc_tmp=${controlDataPath}/${satId}_qcMon_${ymd}_tmp
      cat ${pathMon}/${ymd}/QC_MON.M1.D${yy}${julday}.S*HR.ORB > ${file_qc_tmp} 2> /dev/null
    elif [[ ${satId} == 'metopC' ]] ; then
      mv -f ${pathMon}/QC_MON.M3.D${yy}${julday}.S*HR.ORB ${pathMon}/${ymd}/ 2> /dev/null
      file_qc_tmp=${controlDataPath}/${satId}_qcMon_${ymd}_tmp
      cat ${pathMon}/${ymd}/QC_MON.M3.D${yy}${julday}.S*HR.ORB > ${file_qc_tmp} 2> /dev/null
    elif [[ ${satId} == 'gpm' ]] ; then
      mv -f ${pathMon}/QC_MON-R.${satId^^}.GMI.XCAL*-C.${yyyymmdd}-S*HR.ORB ${pathMon}/${ymd}/ 2> /dev/null
      file_qc_tmp=${controlDataPath}/${satId}_qcMon_${ymd}_tmp
      cat ${pathMon}/${ymd}/QC_MON-R.${satId^^}.GMI.XCAL*-C.${yyyymmdd}-S*HR.ORB > ${file_qc_tmp} 2> /dev/null
    fi
...
# Description:
#	Removes MiRS SDR, TDR, FMSDR and NEDT files associated with 1st
#       and 3rd granules when processing at NDE/ESPC for N20. 
#       Should be called after the fm step so that only the 2nd 
#       granule is processed by the 1dvar (if AAPP code was used 
#       for fm).
#
...
#
# History:
#      	- 11/22/2016     Junye Chen based on  selMidGranule4NPP
#
#===============================================================

function selMidGranule4N20() {
    if [[ $# -ne 1 ]] ; then
      op_msg "Error: function selMidGranule4N20: check number of arguments: $#"
      exit 1
    fi
    ...
    echo "End of step selMidGranule4N20"
...
function selMidGranule4N21() {
    if [[ $# -ne 1 ]] ; then
      op_msg "Error: function selMidGranule4N21: check number of arguments: $#"
      exit 1
    fi
    ...
    echo "End of step selMidGranule4N21"
