...
. ${oldargs}/metopC_pcf_nde.bash
. ${rootPath}/scripts/script_functions.bash

#Redirect standard out to the algorithm log file (lots of output causes MiRS to hang with stout buffer)
exec &>${working_directory}/metopC_logFile.dat
...
HDF5BIN=`grep HDF5BIN ${rootPath}/setup/paths | cut -f2 -d '=' | sed -e 's/^[ 	]*//'`
...
tdrSensor1Dir=${tdrSensor1Path}/${rdirExt}/${sensor1}
tdrSensor2Dir=${tdrSensor2Path}/${rdirExt}/${sensor2}
sdrSensor1Dir=${sdrSensor1Path}/${rdirExt}/${sensor1}
sdrSensor2Dir=${sdrSensor2Path}/${rdirExt}/${sensor2}
...
if [[ ${satId} == "n20" ]] ; then
fwd2hdf5Dir=${externalDataPath}/rdr/n20_atms/${rdirAnalysExt}
fi
...
if [[ "${step_clean}" -eq 1 ]] ; then
  if [[ "${satId}" == "f16" || "${satId}" == "f17" || "${satId}" == "f18" || "${satId}" == "f19" || "${satId}" == "gpm" || "${satId}" == "npp" || "${satId}" == "n20" ]] ; then
...
#--------------------------------------------------------------------------------
ProdFilesGen='metopC_scs_nde.PSF'
