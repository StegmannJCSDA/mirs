			...
			implements ActionListener, TreeSelectionListener, ItemListener, FocusListener 
{
  ...
  private String sweClimoUseKey 		= new String("sweClimoUse");              
  private String ffUseKey 		        = new String("ffUse");              
  private String imsSfcUseKey   		= new String("imsSfcUse");              
  private String ndayLagImsKey   		= new String("ndayLagIms");              
  ...
  private String sweClimoUse 		        = null ; 
  private String ffUse 		                = null ; 
  private String imsSfcUse 		        = null ; 
  private String ndayLagIms 		        = null ; 
  ...
  //sweClimoUse
  private JCheckBox sweClimoUseCheck = new JCheckBox("Use SWE Climatology in VIPP Yes/No", false);

  //ffUse
  private JCheckBox ffUseCheck = new JCheckBox("Use Forest Fraction VIPP SWE Yes/No", false);

  //imsSfcUse
  private JCheckBox imsSfcUseCheck = new JCheckBox("Use IMS Sfc type in NWP and/or 1DVAR Yes/No", false);

  //ndayLagIms
  private JLabel ndayLagImsLabel = new JLabel("Number of days lag between processing day and IMS analysis");
  private JTextField ndayLagImsField = new JTextField(32);
 ...
 //makeOrNot
  ...
  private DefaultMutableTreeNode sweClimoNode;
  private DefaultMutableTreeNode ffNode;
  private DefaultMutableTreeNode imsSfcNode;
  ...
  { 
    ...
    */
    ...
    sweClimoNode  = new DefaultMutableTreeNode("SWE Climatology Use");
    ffNode        = new DefaultMutableTreeNode("Forest Fraction Use");
    imsSfcNode    = new DefaultMutableTreeNode("IMS Surface Analysis Use");
    ...
    sweClimoUseCheck.addItemListener(this);
    ffUseCheck.addItemListener(this);
    imsSfcUseCheck.addItemListener(this);
    ...
    ndayLagImsField.addFocusListener(this);
  ...
  public void itemStateChanged(ItemEvent event) {
    ...
    */
    ...
    else if ( event.getSource() == sweClimoUseCheck ) {
    
	String oldVal =  sweClimoUse;

	if ( sweClimoUseCheck.isSelected() )
	    sweClimoUse="1";
	else
	    sweClimoUse="0";
	    
	if ( ! sweClimoUse.equals(oldVal) ) {
	    if ( preferences.containsKey(sweClimoUseKey) ) 
	    	preferences.remove(sweClimoUseKey);
	    preferences.put(sweClimoUseKey, sweClimoUse);
	}	     
    }

    else if ( event.getSource() == ffUseCheck ) {
    
	String oldVal =  ffUse;

	if ( ffUseCheck.isSelected() )
	    ffUse="1";
	else
	    ffUse="0";
	    
	if ( ! ffUse.equals(oldVal) ) {
	    if ( preferences.containsKey(ffUseKey) ) 
	    	preferences.remove(ffUseKey);
	    preferences.put(ffUseKey, ffUse);
	}	     
    }

    else if ( event.getSource() == imsSfcUseCheck ) {
    
	String oldVal =  imsSfcUse;

	if ( imsSfcUseCheck.isSelected() )
	    imsSfcUse="1";
	else
	    imsSfcUse="0";
	    
	if ( ! imsSfcUse.equals(oldVal) ) {
	    if ( preferences.containsKey(imsSfcUseKey) ) 
	    	preferences.remove(imsSfcUseKey);
	    preferences.put(imsSfcUseKey, imsSfcUse);
	}	     
    }
  ...
  public void focusLost(FocusEvent event)
  {
    ...
    else if ( event.getSource() == ndayLagImsField ) {
	preferences.remove(ndayLagImsKey);
	preferences.put(ndayLagImsKey, ndayLagImsField.getText());
    }
  ...
  { 
    ...
    else if ( selectedNode == sweClimoNode ) {
	...
	JPanel titlePanel   = new JPanel();
	JPanel useSweClimoPanel = new JPanel(new GridLayout(3,1));

	titlePanel.add(new JLabel("SWE Climatology Use"));
	titlePanel.setBackground(new Color(176,196,222));
	...
	String useSweClimoTitle = new String("SWE Climatology Use");
	TitledBorder useSweClimoTitleBorder=BorderFactory.createTitledBorder(blackline, useSweClimoTitle);
	useSweClimoPanel.setBorder(useSweClimoTitleBorder);	
	useSweClimoPanel.add(sweClimoUseCheck);

	rightPanel.add(titlePanel);
	rightPanel.add(useSweClimoPanel);
	...
	SpringLayout.Constraints useSweClimoCons = rightLayout.getConstraints(useSweClimoPanel);
        useSweClimoCons.setX(Spring.constant(50));
        useSweClimoCons.setY(Spring.constant(45));
        useSweClimoCons.setWidth(Spring.constant(525));
        useSweClimoCons.setHeight(Spring.constant(75));
	
	rightPanel.paintAll(g);
	rightPanel.setVisible(true);

    }
    else if ( selectedNode == ffNode ) {
	...
	JPanel titlePanel   = new JPanel();
	JPanel useFFPanel = new JPanel(new GridLayout(3,1));

	titlePanel.add(new JLabel("Forest Fraction Use"));
	titlePanel.setBackground(new Color(176,196,222));
	...
	String useFFTitle = new String("Forest Fraction Use");
	TitledBorder useFFTitleBorder=BorderFactory.createTitledBorder(blackline, useFFTitle);
	useFFPanel.setBorder(useFFTitleBorder);	
	useFFPanel.add(ffUseCheck);

	rightPanel.add(titlePanel);
	rightPanel.add(useFFPanel);
	...
	SpringLayout.Constraints useFFCons = rightLayout.getConstraints(useFFPanel);
        useFFCons.setX(Spring.constant(50));
        useFFCons.setY(Spring.constant(45));
        useFFCons.setWidth(Spring.constant(525));
        useFFCons.setHeight(Spring.constant(75));
	
	rightPanel.paintAll(g);
	rightPanel.setVisible(true);

    }
    else if ( selectedNode == imsSfcNode ) {
	...
	JPanel titlePanel   = new JPanel();
	JPanel useImsSfcPanel = new JPanel(new GridLayout(3,1));
	JPanel ndayLagImsPanel = new JPanel();

	titlePanel.add(new JLabel("IMS Analysis"));
	titlePanel.setBackground(new Color(176,196,222));
	...
	String useImsSfcTitle = new String("IMS Analysis Use");
	TitledBorder useImsSfcTitleBorder=BorderFactory.createTitledBorder(blackline, useImsSfcTitle);
	useImsSfcPanel.setBorder(useImsSfcTitleBorder);	
	useImsSfcPanel.add(imsSfcUseCheck);

	String ndayLagImsTitle = new String("Number of day lag between processing date and IMS analysis");
	TitledBorder titleBorder=BorderFactory.createTitledBorder(blackline, ndayLagImsTitle);
	ndayLagImsPanel.setBorder(titleBorder);

	ndayLagImsPanel.add(ndayLagImsField);

	rightPanel.add(titlePanel);
	rightPanel.add(useImsSfcPanel);
	rightPanel.add(ndayLagImsPanel);
	...
	SpringLayout.Constraints useImsSfcCons = rightLayout.getConstraints(useImsSfcPanel);
        useImsSfcCons.setX(Spring.constant(50));
        useImsSfcCons.setY(Spring.constant(45));
        useImsSfcCons.setWidth(Spring.constant(525));
        useImsSfcCons.setHeight(Spring.constant(75));
	
	SpringLayout.Constraints ndayLagImsCons = rightLayout.getConstraints(ndayLagImsPanel);
        ndayLagImsCons.setX(Spring.constant(50));
        ndayLagImsCons.setY(Spring.constant(170));
        ndayLagImsCons.setWidth(Spring.constant(525));
        ndayLagImsCons.setHeight(Spring.constant(60));
	...
	String rdrTypeTitle = new String("Select RDR for NPP/ATMS or N20/ATMS");
	...
	else if ( satId.equals("f16") || satId.equals("f17") || satId.equals("f18") || satId.equals("f19")) {
  ...
  public void getDefaultValues(Map<String,String> inputPreferences) {
    ...
    ndayLagIms = inputPreferences.get(ndayLagImsKey);    
    if ( ndayLagIms != null )
    	ndayLagImsField.setText(ndayLagIms);
    ...
    sweClimoUse = inputPreferences.get(sweClimoUseKey);
    if ( sweClimoUse!= null && sweClimoUse.equals("1") )
	sweClimoUseCheck.setSelected(true);
    else
    	sweClimoUseCheck.setSelected(false);

    ffUse = inputPreferences.get(ffUseKey);
    if ( ffUse!= null && ffUse.equals("1") )
	ffUseCheck.setSelected(true);
    else
    	ffUseCheck.setSelected(false);

    imsSfcUse = inputPreferences.get(imsSfcUseKey);
    if ( imsSfcUse!= null && imsSfcUse.equals("1") )
	imsSfcUseCheck.setSelected(true);
    else
    	imsSfcUseCheck.setSelected(false);
