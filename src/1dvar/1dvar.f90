...
PROGRAM retr1dvar
  ...
  !---Data to hold collocated surface type data from IMS analysis
  LOGICAL           :: useImsSfc
  INTEGER           :: nprfImsColloc,iuImsColloc
  TYPE(DEP_type)    :: DepImsColloc
  ...
  INTEGER, PARAMETER                                :: ntmax=20
  REAL,     DIMENSION(ntmax)                        :: Tim=0.
  REAL                                              :: t0,t1,t00
  ...
  !!! TBbias_NN realted
  REAL,               DIMENSION(:),  ALLOCATABLE   :: TBbias_NN_out
  REAL                                             :: clw_out,tpw_out,tskin_out
  !!!

  !---Variables related to handling mixed sea/ice scenes
  LOGICAL :: sfcMixed_ice_ocean
  INTEGER, DIMENSION(:,:), ALLOCATABLE :: ChanSelOrig
  INTEGER, DIMENSION(:,:), ALLOCATABLE :: EDR_cntrlRetr_sfcOrig
  REAL, DIMENSION(:), ALLOCATABLE :: Emiss_ext
  REAL :: Tskin_ext
  !---Various scalar variables
  REAL    :: xalt,xcover,trx,tpw1,TskPreclass
  INTEGER :: iuMeasur=0,iuExtern,iuOutput,iuMonitor,Error_status,allocate_status
  INTEGER :: nprofiles=0,nMeasurData=0,nExternData,nAtm,nSfc,nG,nR,nChan,nch
  ...
  LOGICAL :: OkRetrvl,CvgceReached,useBkgExt,clwTooHigh
  !---Control flag for resetting EDRs when QC(1)=2
  ! 1: reset EDR 
  ! 0: do not reset EDR
  INTEGER, PARAMETER :: resetEDR=1
  !-------------------------------------------------------------------
  ...
  !---- Read header of IMS collocation file, which is a DEP type file (if flag set)----
  useImsSfc = CntrlConfig%useImsSfc .EQ. 1
  if(useImsSfc)then
     CALL ReadHdrDEP(iuImsColloc,CntrlConfig%imsCollocFile,DepImsColloc,nprfImsColloc)
     print *,'finished reading imsColloc DEP header,nprfsImsColloc=',nprfImsColloc
  endif
!  stop
...
! Tskin stored in the Atm, not Sfc, structure
  iEDR_sfcT  = FindIndxStr(GeophStatsT_Atm(1)%EDR_Desc,'TSKIN')
  ...
  !---Allocate and initialize original channel selection array
  !---Allocate and initialize original tuning parameters array for sfc EDRs
  ALLOCATE(ChanSelOrig(nchan,CntrlConfig%nattempts))
  ALLOCATE(EDR_cntrlRetr_sfcOrig(nEDRs_sfc,CntrlConfig%nattempts))
  DO iattempt=1,CntrlConfig%nattempts
     ChanSelOrig(1:nchan,iattempt)=TunParams(iattempt)%ChanSel(1:nchan)
     EDR_cntrlRetr_sfcOrig(1:nEDRs_sfc,iattempt)=TunParams(iattempt)%EDR_cntrlRetr_sfc(1:nEDRs_sfc)
  ENDDO
  ALLOCATE(Emiss_ext(nchan))
  ...
  ALLOCATE(TBbias_NN_out(nChan))
  TBbias_NN_out(1:nChan)=DEFAULT_VALUE_REAL
  ...
  ProfLoop: DO iprof=1,nprofiles
      ...
      IF (CntrlConfig%ExternDataAvailab .ne. 0) THEN
         ...
         IF( (UseorNotExterData .eq. 1) .and. &
            ...
            call InitHdrScene(Scene_Ext%nLev,Scene_Ext%nLay,Scene_Ext%nChan,Scene_Ext%CentrFreq,Scene_Ext%polarity,  &
                              Scene_Ext%pres_lev,Scene_Ext%pres_lay,Scene_ExtIn,Scene_Ext%nParmCLW,                  &
                              Scene_Ext%nParmRain,Scene_Ext%nParmSnow,Scene_Ext%nParmIce,Scene_Ext%nParmGrpl,        &
                              Scene_Ext%nAbsorb,Scene_Ext%AbsorbID,Scene_Ext%nqc,Scene_Ext%iTyp,Scene_Ext%nPosScan,  &
                              Scene_Ext%nScanLines,Scene_Ext%AlgSN                                                   )
            call CopyScene(Scene_Ext,Scene_ExtIn,ierr)
            call FillSceneNegValues(Scene_ExtIn,Scene_Ext)
            call DestroyScene(Scene_ExtIn)
         ENDIF
      ENDIF

      !---Read content of IMS collocation file (if flag set)
      !---This will replace Preclassified surface type with type from IMS
      IF(useImsSfc)THEN
         CALL ReadDep(iuImsColloc,DepImsColloc,ierr)
         IF (ierr.ne.0) CALL ErrHandl(ErrorType,Err_ReadingFile,'. DEP ImsColloc file. (1dvar)')
      ENDIF
      !---Determine the surface type and water fraction 
!ykl      call Read_topography(CntrlConfig%Topogr,Ym%lat,Ym%lon,xalt,stypeSfc,xcover,stypeSfcCRTM)
      call Read_topography_onetwentieth(CntrlConfig%Topogr,TunParams(1)%Sensor_ID,Ym%lat,Ym%lon, &
           Ym%iscanPos,xalt,stypeSfc)
      ...
      IF (OKRetrvl) THEN
        ...
        sfcMixed_ice_ocean = .FALSE.
        !---Start the iterative process (#of retrieval attempts)
       AttemptsLoop: DO iattempt=1,CntrlConfig%nattempts
          !---Reset some tuning parameters back to original values each new profile
          !---Necessary because subroutine SeaIceTypeAdjust below may modify the original
          !---Tuning parameters in certain scenes possibly containing sea ice
           TunParams(iattempt)%ChanSel(1:nchan)=ChanSelOrig(1:nchan,iattempt)
           TunParams(iattempt)%EDR_cntrlRetr_sfc(1:nEDRs_sfc)=EDR_cntrlRetr_sfcOrig(1:nEDRs_sfc,iattempt)
           ...
           !---Replace Preclassified surface type with type from IMS (if flag set)
           IF(useImsSfc)THEN
!              print *,'PreClassif: SfcClass, IMS: sfcClass=',SfcClass,DepImsColloc%iTypSfc
           !--- Correct MiRS misclassifications due to small water bodies (inland lakes) 
           !--- sometimes classified as snow
           !--- with high resolution (~ 5 km) data. MiRS Land/Ocean mask does not contain small lakes.
              IF(SfcClass .GE. 2 .AND. SfcClass .LE. 3)then
                 !--- In cases of IMS land/snow, always use IMS type
                 IF(DepImsColloc%iTypSfc .GE. 2 .AND. DepImsColloc%iTypSfc .LE. 3)&
                      SfcClass = DepImsColloc%iTypSfc 
                 !--- In cases of IMS ice (when MiRS sfc type is land/snow), 
                 !--- this is likely a small (subpixel) frozen inland lake, so set to snow
                 !--- NB: this may sometimes cause false snow artifacts 
                 !--- in transition seasons when surrounding land is snow-free
                 IF(DepImsColloc%iTypSfc .EQ. 1)SfcClass = 3 
                 !--- In cases of IMS water (when MiRS sfc type is land/snow), 
                 !--- this is likely a small (subpixel) unfrozen inland lake, so set to land
                 IF(DepImsColloc%iTypSfc .EQ. 0)SfcClass = 2
              ENDIF
              !--- Extra cleanup to handle occasional false water detection 
              !--- by MiRS preclassifier (due to land/sea mask and geolocation)
!              IF(DepImsColloc%iTypSfc .GE. 2 .AND. DepImsColloc%iTypSfc .LE. 3)then
!                 IF(SfcClass .eq. 0)SfcClass = 2
!              ENDIF
           ENDIF
           ...
           !---For ocean/ice scenes, check for differences between FG emis and BG
           !--- and possibly reclassify sfc and/or atm type to improve convergence
           !--- For AMSU/MHS and ATMS, channel 2 (31v) may be turned off in this case since it reduces convergence
           IF(UseorNotExterData .eq. 0)THEN           
              !---In the case when no external data are used
              !---copy Bkg Tskin and Emiss into Scene_Ext so that call to SeaIceAdjust still works
              Tskin_ext=XbAtmTot(GeophStatsT_Atm(1)%EDR_IDs(iEDR_SfcT),sfcClass+1,1)
              Emiss_ext(1:nchan)=XbSfcTot(GeophStatsT_Sfc(1)%EDR_IDs(iEDR_emis):&
                   GeophStatsT_Sfc(1)%EDR_IDs(iEDR_emis)+&
                   GeophStatsT_Sfc(1)%Stats(iEDR_emis,1)%npEDR-1,sfcClass+1,1)
           ELSE
              !---Otherwise, just use external data
              if(Scene_ext%Tskin .gt. 0.)then
                 Tskin_ext=Scene_ext%Tskin
              else
                 Tskin_ext=XbAtmTot(GeophStatsT_Atm(1)%EDR_IDs(iEDR_SfcT),sfcClass+1,1)
              endif
              if(Scene_ext%Emiss(1) .gt. 0.)then
                 Emiss_ext(1:nchan)=Scene_ext%Emiss(1:nchan)
              else
                 Emiss_ext(1:nchan)=XbSfcTot(GeophStatsT_Sfc(1)%EDR_IDs(iEDR_emis):&
                      GeophStatsT_Sfc(1)%EDR_IDs(iEDR_emis)+&
                      GeophStatsT_Sfc(1)%Stats(iEDR_emis,1)%npEDR-1,sfcClass+1,1)
           ...
           ENDIF
           call SeaIceTypeAdjust(CntrlConfig%SensorID,nchan,Ym%CentrFreq,iattempt,stypeSfc,&
                Ym%lat,&
                XbAtmTot(GeophStatsT_Atm(1)%EDR_IDs(iEDR_SfcT),sfcClass+1,1),&
                Tskin_ext,&
                XbSfcTot(GeophStatsT_Sfc(1)%EDR_IDs(iEDR_emis):GeophStatsT_Sfc(1)%EDR_IDs(iEDR_emis)+&
                GeophStatsT_Sfc(1)%Stats(iEDR_emis,1)%npEDR-1,sfcClass+1,1),&
                Emiss_ext(1:nchan),sfcMixed_ice_ocean,sfcClass,atmClass,&
                TunParams(iattempt)%ChanSel(1:nchan))
...
!           print *,'iprof,iatm,isfc,XbSfc(1:20)=',iprof,iatm,isfc,XbSfc(1:20)
           ...
           ENDIF
!           print *,'after: iprof,iatm,isfc,XbSfc(1:20)=',iprof,iatm,isfc,XbSfc(1:20)
           ...
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Bias application !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           !---Determine if Neural Network bias correction is used or not
           IF ( any((/any(TunParams(iattempt)%applyBias_oc_byChan .eq. 2), &
                      any(TunParams(iattempt)%applyBias_ic_byChan .eq. 2), &
                      any(TunParams(iattempt)%applyBias_ld_byChan .eq. 2), &
                      any(TunParams(iattempt)%applyBias_sn_byChan .eq. 2)/)) ) THEN

              !---Bias application using NN
              CALL TBbias_NN_cal(Ym%tb,nChan,mean(Ym%angle(1:Ym%nchan)),Ym%lat,SfcClass,CntrlConfig%TBbias_NNModelFile, & 
                                CntrlConfig%TPW_NNModelFile,CntrlConfig%Tskin_NNModelFile,TBbias_NN_out,tpw_out,tskin_out,clw_out)
              !---Bias application using NN
              Call apply_bias_NN(Ym%tb,tb,Ym%iscanPos,Ym%nPosScan,nposBias,Bias,TBbias_NN_out,Slope,Intercept,nChan,&
          ...
          !---Disable retrieval of layers below surface level
           ...
           IterLoop: DO iter=0,TunParams(iattempt)%nIterations-1
              ...
              !---However: If nominal convergence reached, 
              !---then also check for unrealistically high CLW 
              !---over land/ocean (associated with spurious rain rates in 1st attempt)
              !---and if true, then continue iterating, even if other convergence criteria are met
              call checkCLW(Scene,clwTooHigh)
              IF (CvgceReached .AND. &
                   .NOT. clwTooHigh) &
                   EXIT AttemptsLoop
           END DO IterLoop
           !---Attempt a second retrieval based on TunParams(iattempt)%ChiSqThresh4Attempt
           !---However: Also check for unrealistically high CLW 
           !---over land/ocean (associated with spurious rain rates in 1st attempt)
           !---and make additional attempt, even if convergence criterion is met
           call checkCLW(Scene,clwTooHigh)
           IF (ChiSq(nIterTot(iprof)).le.TunParams(iattempt)%ChiSqThresh4Attempt .AND. &
                .NOT. clwTooHigh) &
                EXIT AttemptsLoop
        END DO AttemptsLoop
        ...
        ENDIF
        !---If QC(1)=2 (bad) and resetEDR=1, reset all retrieved variables to missing
        !---To retain retrieved EDR values for diagnostics, set resetEDR=0, and recompile
        IF(Scene%qc(1) .EQ. 2 .and. resetEDR .eq. 1)THEN
           Scene%Temp_lay(1:Scene%nLay)=DEFAULT_VALUE_REAL
           Scene%Absorb_lay(1:Scene%nLay,Scene%iH2O)=DEFAULT_VALUE_REAL
           Scene%Clw(1:Scene%nParmCLW)=DEFAULT_VALUE_REAL
           Scene%Rain(1:Scene%nParmRain)=DEFAULT_VALUE_REAL
           Scene%Graupel(1:Scene%nParmGrpl)=DEFAULT_VALUE_REAL
           Scene%Tskin=DEFAULT_VALUE_REAL
           Scene%Emiss(1:Scene%nchan)=DEFAULT_VALUE_REAL
        ENDIF
      ELSE
        !---Pre-classification
        TskPreclass = DEFAULT_VALUE_REAL
        ...
        Scene%nIter=-99
        Scene%ChiSq=-99.
        !---Set channel selection array to zero when no retrieval attempted 
        Scene%ChanSel(1:Scene%nchan) = 0
        !---Set EDRs to missing when no retrieval attempted
        Scene%Temp_lay(1:Scene%nLay)=DEFAULT_VALUE_REAL
        Scene%Absorb_lay(1:Scene%nLay,Scene%iH2O)=DEFAULT_VALUE_REAL
        Scene%Clw(1:Scene%nParmCLW)=DEFAULT_VALUE_REAL
        Scene%Rain(1:Scene%nParmRain)=DEFAULT_VALUE_REAL
      ...
      call setQC(CntrlConfig%SensorID,Scene,nqc,Ym%qc,Ym%nqc,OKRetrvl)
      !---Set to default value all non-officially delivered products
      call set2defaultNonOfficialproducts(Scene)
      !---Set to zero value for CLW, graupel, and rain if too small
      call setSceneHydro2zero(Scene)
      !---Write out scene
  ...
  DEALLOCATE(ChanSelOrig)
  DEALLOCATE(EDR_cntrlRetr_sfcOrig)
  DEALLOCATE(Emiss_ext)
  ...
  !---Close IMS collocation file (DEP format) if flag set
  IF(useImsSfc) CLOSE(iuImsColloc)
