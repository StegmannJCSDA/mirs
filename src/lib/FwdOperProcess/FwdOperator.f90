...
!      11-22-2016   Junye Chen, add JPSS-1, N20
!
!-----------------------------------------------------------------------------------------------
...
!==============================================================================
  SUBROUTINE InterfaceCRTM(Scene,ChannelInfo,Atmos,Sfc,Options,RTSolution,TB)
    ...
    !---Turn on scattering and set to 4 stream solution
  ...
  SUBROUTINE GetSensorInfo(INT_SensorID, CHAR_SensorID, nSensors)
    ...
    IF (INT_SensorID .eq. SENSOR_ID_METOPB) THEN
       ...
       CHAR_SensorID = (/'amsua_metop-b', &
            'mhs_metop-b  ' /)
    ENDIF
    IF (INT_SensorID .eq. SENSOR_ID_METOPC) THEN
       nSensors = 2
       ALLOCATE(CHAR_SensorID(nSensors))
       CHAR_SensorID = (/'amsua_metop-c', &
            'mhs_metop-c  ' /)
    ENDIF
    ...
    ENDIF
    IF (INT_SensorID .eq. SENSOR_ID_N20) THEN
       nSensors = 1
       ALLOCATE(CHAR_SensorID(nSensors))
       CHAR_SensorID = (/'atms_n20'/)
    ENDIF
    IF (INT_SensorID .eq. SENSOR_ID_N21) THEN
       nSensors = 1
       ALLOCATE(CHAR_SensorID(nSensors))
       CHAR_SensorID = (/'atms_n21'/)
    ENDIF
    IF (INT_SensorID .eq. SENSOR_ID_metopSGA1) THEN
       nSensors = 1
       ALLOCATE(CHAR_SensorID(nSensors))
       CHAR_SensorID = (/'mws_metopSGA1'/)
    ENDIF
