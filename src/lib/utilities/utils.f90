...
MODULE utils
  ...
  PUBLIC :: compute_height,apply_bias,apply_bias_NN,intrpLevs2Lays,Levs2Laysintrp, regr_clw_atms
...
! updated to use layer value of water vapor
    ...
    LevLoop: DO i=1,nl
       ...
       if(i.lt.nl) then
          if(P(i+1).lt.psurf) then
             dP=P(i+1)-P(i)
!org             water=water+(h2o(i)+h2o(i+1))*0.5*dP
             water=water+h2o(i)*dP
          else
!org             water=water+(h2o(i)+h2o(i+1))*0.5*(psurf-P(i))
             water=water+h2o(i)*(psurf-P(i))
             EXIT LevLoop
          endif
       else
          water=water+h2o(i)*(psurf-P(i))
...
!         applyBias_ew_byChan        I              Flag vector of channels where to apply bias over everywhere
!                                                   = 0: no correction
!                                                   = 1: static bias correction
!                                                   = 2: NN bias correction
!
!         posBias:              Static bias. It's a funnction of scan position and channels.
!         posBiasNN:            NN generated bias. It's a funnction of channels.
!
! Modules needed:
!       - None
!
! History:
!       08-31-2007      Cezar Kongoli
!       10-23-2007      Sid Boukabara. Added capacity to ignore bias application
!       altogether (no boas file provided)
!       11-15-2007      Sid Boukabara. Modified the code for applying the bias
!       (-2: not apply, -1 everywhere, >=0 surface type)
!       11-22-2007      Sid Boukabara. Added capacity to choose which channels
!       to correct for bias, by surface type
!       2020            Yan Zhou @ UMD
!===============================================================
  SUBROUTINE apply_bias_NN(fmTB,ecfmTB,iscPos,nscposRad,nscposBias,posBias,posBiasNN,posSlope, &
                      posIntercept,nchan,iToApply,iApplMethod,iSfc,errReadingBias,&
                      applyBias_oc_byChan,applyBias_ic_byChan,applyBias_ld_byChan,&
                      applyBias_sn_byChan)
    INTEGER                               :: iToApply,iApplMethod,iscPos,nchan,iSfc,ichan,nscposRad,nscposBias,errReadingBias
    ...
    IF (iToApply .eq. -1) THEN ! apply everywhere
       do ichan=1,nchan
          ...
          IF (iSfc .eq. OC_TYP) THEN
             IF (applyBias_oc_byChan(ichan).eq.1) THEN
                IF (iApplMethod .eq.0) ecfmTB(ichan)=fmTB(ichan)-posBias(indPos,ichan)
                IF (iApplMethod .eq.1) ecfmTB(ichan)=fmTB(ichan)*posSlope(indPos,ichan)+posIntercept(indPos,ichan)
             ELSE IF (applyBias_oc_byChan(ichan).eq.2) THEN
                ecfmTB(ichan)=fmTB(ichan)-posBiasNN(ichan)
             ENDIF
          ENDIF
          ! Sea ice
          IF (iSfc .eq. SEAICE_TYP) THEN
             IF (applyBias_ic_byChan(ichan).eq.1) THEN
                IF (iApplMethod .eq.0) ecfmTB(ichan)=fmTB(ichan)-posBias(indPos,ichan)
                IF (iApplMethod .eq.1) ecfmTB(ichan)=fmTB(ichan)*posSlope(indPos,ichan)+posIntercept(indPos,ichan)
             ELSE IF (applyBias_ic_byChan(ichan).eq.2) THEN
                ecfmTB(ichan)=fmTB(ichan)-posBiasNN(ichan)
             ENDIF
          ENDIF
          ! Land
          IF (iSfc .eq. LD_TYP) THEN
             IF (applyBias_ld_byChan(ichan).eq.1) THEN
                IF (iApplMethod .eq.0) ecfmTB(ichan)=fmTB(ichan)-posBias(indPos,ichan)
                IF (iApplMethod .eq.1) ecfmTB(ichan)=fmTB(ichan)*posSlope(indPos,ichan)+posIntercept(indPos,ichan)
             ELSE IF (applyBias_ld_byChan(ichan).eq.2) THEN
                ecfmTB(ichan)=fmTB(ichan)-posBiasNN(ichan)
             ENDIF
          ENDIF
          ! Snow
          IF (iSfc .eq. SNOW_TYP) THEN
             IF (applyBias_sn_byChan(ichan).eq.1) THEN
                IF (iApplMethod .eq.0) ecfmTB(ichan)=fmTB(ichan)-posBias(indPos,ichan)
                IF (iApplMethod .eq.1) ecfmTB(ichan)=fmTB(ichan)*posSlope(indPos,ichan)+posIntercept(indPos,ichan)
             ELSE IF (applyBias_sn_byChan(ichan).eq.2) THEN
                ecfmTB(ichan)=fmTB(ichan)-posBiasNN(ichan)
             ENDIF
          ENDIF
       enddo
    ENDIF
  END SUBROUTINE apply_bias_NN
...
!       11-01-2014      Craig Smith:
!                        -For F18 only, set the dead Ch 8 emissivity = E(91v)
!                         so that values > 1 do not cause all emiss to be
!                         set to -999 (missing) at end of code.
!                         This ensures that bad Ch 8 emissivities do not clobber
!                         the truth (analytic) emissivities when called from
!                         CollocNwp.f90.  However, it has no salient effect on
!                         the results of the call from ApplyRegress.f90, since
!                         Ch 8 emissivity is not used as truth value in 1DVAR,
!                         and regressions are no longer used for F18 first
!                         guess.
!===============================================================
...
!liusy+
!---For GPM/GMI
    REAL,    DIMENSION(4)              :: EmGMIBase_H
    REAL,    DIMENSION(5)              :: EmGMIBase_V
!liusy.
!----------------------------------------------------------------
    ...
    ENDDO
    !---Adjust AMSU/MHS Channels (N18, N19, MetopA)
    IF (sensor_id .eq. sensor_id_n18 .or. sensor_id .eq. sensor_id_metopA .or. &
        sensor_id .eq. sensor_id_n19) THEN
       ...
       ENDIF
    ENDIF
    !---Adjust AMSU/MHS Channels (MetopB treated differently due to bad channel 15)
    IF (sensor_id .eq. sensor_id_metopB) THEN
       IF (Em(1) .ge. 1 .or. Em(1) .le. 0 .or. Em(16) .le. 0 .or. Em(16) .ge. 1) THEN 
          EmissOut=-999.
          RETURN
       ENDIF
       IF (Em(2) .gt. 1 .or. Em(2) .le. 0) EmissOut(2)=EmissOut(1)-random_noise(2)
       ...
       ENDIF
    ENDIF
    !---Adjust AMSU/MHS Channels (MetopC currently handled same as MetopB, using chan 16 rather than 15)
    !---This handling could change to use chan 15 rather than 16 as for N18, etc. but Tuning file would
    !---need to be updated as well to change channel selection array.
    IF (sensor_id .eq. sensor_id_metopC) THEN
       IF (Em(1) .ge. 1 .or. Em(1) .le. 0 .or. Em(16) .le. 0 .or. Em(16) .ge. 1) THEN 
          EmissOut=-999.
          RETURN
       ENDIF
    ...
    IF (sensor_id .eq. sensor_id_f18) THEN
       ...
       ENDDO
 !      IF (Em(8) .lt. 0) EmissOut(8) = Em(18)-random_noise(8)
       EmissOut(8) = Em(18)-random_noise(8) !Sets dead channel=91v so that large
 !                                           Positive values do not clobber
 !                                           emiss for all channels at end
       IF (Em(9) .lt. 0) EmissOut(9) = Em(18)-random_noise(9)
       IF (Em(10) .lt. 0) EmissOut(10) = EmissOut(9)-random_noise(10)
       IF (Em(11) .lt. 0) EmissOut(11) = EmissOut(9)-random_noise(11)
       DO ichan=19,24
       ...
       ENDDO
    ENDIF
    IF (sensor_id .eq. sensor_id_f19) THEN
       DO ichan=2,7
          IF (Em(ichan) .lt. 0) EmissOut(ichan) = EmissOut(1)-random_noise(ichan)
       ENDDO
       IF (Em(8) .lt. 0) EmissOut(8) = Em(18)-random_noise(8)
       ...
       ENDDO
    ENDIF
    !---Adjust NPP and N20 ATMS Channels
    IF (sensor_id .eq. sensor_id_npp .or. sensor_id .eq. sensor_id_n20 .or. &
        sensor_id .eq. sensor_id_n21 ) THEN
       IF (Em(1) .ge. 1 .or. Em(1) .le. 0 .or. Em(16) .ge. 1 .or. Em(16) .le. 0 &
            .or. Em(3) .le. 0 .or. Em(3) .ge. 1 ) THEN 
!            .or. Em(3) .le. 0 .or. Em(3) .ge. 1 .or. Em(17) .le. 0 .or. Em(17) .ge. 1) THEN 
       ...
       IF (Em(2) .gt. 1 .or. Em(2) .le. 0) EmissOut(2)=EmissOut(1)-random_noise(2)
!JChen 2017
       IF (Em(17) .ge. 1 .or. Em(17) .le. 0) THEN
          Em(17) = 0.91-random_noise(17)-0.7*(0.90-Em(1)+0.93-Em(2)+0.89-Em(3)+0.89-Em(16))/4.

         IF (Em(1) .ge. 0.9 .and. Em(2) .ge. 0.91 .and. Em(3) .ge. 0.88 .and. Em(16) .ge. 0.89) &
          Em(17) = 0.925-random_noise(17)-0.6*(0.97-Em(1)+0.96-Em(2)+0.92-Em(3)+0.96-Em(16))/4.
         IF (Em(1) .lt. 0.9 .and. Em(2) .lt. 0.90 .and. Em(3) .lt. 0.85 .and. Em(16) .ge. 0.86 .and. &
           Em(16)-Em(3) .ge. 0.06) &
           Em(17) = 0.88-random_noise(17)-0.6*(0.86-Em(1)+0.88-Em(2)+0.80-Em(3)+0.90-Em(16))/4.
         IF (Em(1) .lt. 0.9 .and. Em(2) .lt. 0.91 .and. Em(3) .le. 0.88 .and. Em(16) .le. 0.89 .and. &
           abs(Em(3)-Em(16)) .le. 0.05) &
           Em(17) = 0.87-random_noise(17)-0.6*(0.86-Em(1)+0.87-Em(2)+0.84-Em(3)+0.86-Em(16))/4.
       ELSE
          Em(17) = Em(17)+0.04       
       ENDIF
       IF (Em(3) .gt. 0 .and. Em(3) .lt. 1 .and. Em(17) .gt. 0 .and. Em(17) .lt. 1) THEN
          cntValid = COUNT(Em(3:17) .eq. Em(3:17))
          ALLOCATE(Emiss2Intp(2),Freq2Intp(2),Emiss2Intp2(cntValid),Freq2Intp2(cntValid))
          Emiss2Intp(1)=Em(3)
          Emiss2Intp(2)=Em(17)
          Freq2Intp(1)=cFreq(3)
    ...
    ENDIF
    !---Adjust metopSGA1 MWS Channels
    IF (sensor_id .eq. sensor_id_metopSGA1 ) THEN
       IF (Em(1) .ge. 1 .or. Em(1) .le. 0 .or. Em(17) .ge. 1 .or. Em(17) .le. 0 &
            .or. Em(3) .le. 0 .or. Em(3) .ge. 1 ) THEN
!            .or. Em(3) .le. 0 .or. Em(3) .ge. 1 .or. Em(17) .le. 0 .or. Em(17)
!            .ge. 1) THEN
          EmissOut=-999.
          RETURN
       ENDIF
       IF (Em(2) .gt. 1 .or. Em(2) .le. 0) EmissOut(2)=EmissOut(1)-random_noise(2)
!JChen 2017
       IF (Em(18) .ge. 1 .or. Em(18) .le. 0) THEN
          Em(18) = 0.91-random_noise(18)-0.7*(0.90-Em(1)+0.93-Em(2)+0.89-Em(3)+0.89-Em(17))/4.

         IF (Em(1) .ge. 0.9 .and. Em(2) .ge. 0.91 .and. Em(3) .ge. 0.88 .and. Em(17) .ge. 0.89) &
          Em(18) = 0.925-random_noise(18)-0.6*(0.97-Em(1)+0.96-Em(2)+0.92-Em(3)+0.96-Em(17))/4.
         IF (Em(1) .lt. 0.9 .and. Em(2) .lt. 0.90 .and. Em(3) .lt. 0.85 .and. Em(17) .ge. 0.86 .and. &
           Em(17)-Em(3) .ge. 0.06) &
           Em(18) = 0.88-random_noise(18)-0.6*(0.86-Em(1)+0.88-Em(2)+0.80-Em(3)+0.90-Em(17))/4.
         IF (Em(1) .lt. 0.9 .and. Em(2) .lt. 0.91 .and. Em(3) .le. 0.88 .and. Em(17) .le. 0.89 .and. &
           abs(Em(3)-Em(17)) .le. 0.05) &
           Em(18) = 0.87-random_noise(18)-0.6*(0.86-Em(1)+0.87-Em(2)+0.84-Em(3)+0.86-Em(17))/4.
       ELSE
          Em(18) = Em(18)+0.04
       ENDIF
       IF (Em(3) .gt. 0 .and. Em(3) .lt. 1 .and. Em(18) .gt. 0 .and. Em(18) .lt. 1) THEN
          cntValid = COUNT(Em(3:18) .eq. Em(3:18))
          ALLOCATE(Emiss2Intp(2),Freq2Intp(2),Emiss2Intp2(cntValid),Freq2Intp2(cntValid))
          Emiss2Intp(1)=Em(3)
          Emiss2Intp(2)=Em(18)
          Freq2Intp(1)=cFreq(3)
          Freq2Intp(2)=cFreq(18)
          Freq2Intp2(1:cntValid)=cFreq(3:18)
          !---Interpolate for AMSU sounding channels
          CALL LINT(Freq2Intp,Emiss2Intp,2,2,cntValid,Freq2Intp2,Emiss2Intp2)
          EmissOut(3:16)=Emiss2Intp2(1:cntValid-2)
          EmissOut(18)=Emiss2Intp2(cntValid)
          DEALLOCATE(Freq2Intp,Emiss2Intp,Freq2Intp2,Emiss2Intp2)
       ENDIF
       EmissOut(19)=EmissOut(18)-random_noise(19)
       EmissOut(20)=EmissOut(19)-random_noise(20)
       EmissOut(21)=EmissOut(19)-random_noise(21)
       EmissOut(22)=EmissOut(19)-random_noise(22)
       EmissOut(23)=EmissOut(19)-random_noise(23)
       EmissOut(24)=EmissOut(19)-random_noise(24)
    ENDIF
...
!liusy+
    IF (sensor_id .eq. sensor_id_gpm) THEN
       !---Sorting H and V pol by ascending frequency
       !---VPOL
       EmGMIBase_V(1)   =  EmissIn(1 )
       EmGMIBase_V(2)   =  EmissIn(3 )
       EmGMIBase_V(3)   =  EmissIn(5 )
       EmGMIBase_V(4)   =  EmissIn(6 )
       EmGMIBase_V(5)   =  EmissIn(8 )

       !---HPOL
       EmGMIBase_H(1)   =  EmissIn(2 )
       EmGMIBase_H(2)   =  EmissIn(4 )
       EmGMIBase_H(3)   =  EmissIn(7 )
       EmGMIBase_H(4)   =  EmissIn(9 )

       !---Follow TRMM/TMI method for channels 1-9, interpolation
       !---Follow SSMI/S method for channels 10-13, add random noise

       !---Interpolate for H pol 18Ghz(Ch4),36Ghz(Ch7) if missing
       IF (EmissIn(4) .le. 0 .or. EmissIn(7) .le. 0  ) THEN
          cntValid = COUNT(EmGMIBase_H(1:4) .eq. EmGMIBase_H(1:4))
          IF (cntValid .ne. 4) THEN
             EmissOut   =  -999.
             RETURN
          ENDIF
          ...
          Emiss2Intp(1) =  EmGMIBase_H(1)
          Emiss2Intp(2) =  EmGMIBase_H(4)
       ...
       !---Interpolate for V pol 18Ghz (Ch3),36Ghz(Ch6),89Ghz(Ch8),166Ghz(Ch10),183 Ghz(Ch12) if missing
       IF (EmissIn(3)  .le. 0 .or. EmissIn(6)  .le. 0  ) THEN
          cntValid = COUNT(EmGMIBase_V(1:5) .eq. EmGMIBase_V(1:5))
          IF (cntValid .ne. 5) THEN
             EmissOut   =  -999.
             RETURN
          ENDIF
          ...
          Emiss2Intp(1) =  EmGMIBase_V(1)
          Emiss2Intp(2) =  EmGMIBase_V(5)
       ...
       !Set 23.8 GHz to -999 to always get interpolated value
       EmGMIBase_V(3)   =  -999.
       !---Interpolate 23.8 GHz V pol
       IF (EmGMIBase_V(3) .le. 0) THEN
          cntValid   =  COUNT(EmGMIBase_V(2:4) .eq. EmGMIBase_V(2:4))
          IF (cntValid .ne. 3) THEN
             EmissOut   =  -999.
             RETURN
          ENDIF
          ...
          Emiss2Intp(1) =  EmGMIBase_V(2)
          Emiss2Intp(2) =  EmGMIBase_V(4)
       ...
       ENDIF
       !
       !---For high freq channels of 10-13
       !---using the channel that has the same polarization to fill missing values
       !
       IF(Em(10) .lt. 0.) EmissOut(10) = Em(8) - random_noise(10) !---V pol
       IF(Em(11) .lt. 0.) EmissOut(11) = Em(9) - random_noise(11) !---H pol
       IF (ANY(Em(12:13) < 0.) ) THEN !---V pol
          EmissOut(12) = EmissOut(10) - random_noise(12)
          EmissOut(13) = EmissOut(10) - random_noise(13)
       END IF
 
    END IF !---sensor_id_gpm
!liusy.
!---if any channel includes invalid value, then all the channels set to missing (liusy notes)
    DO iChan=1,nChan
...
!7) Over-write the input image (points that are not missing)
  ...
  subroutine SeaIceTypeAdjust(SensorID,nchan,CentrFreq,iattempt,stypeSfc,lat,Tskin_bkg,Tskin_ext,&
       Emis_bkg,Emis_ext,sfcMixed_ice_ocean,sfcClass,atmClass,ChanSel)

    integer, intent(in) :: nchan,SensorID,iattempt,stypeSfc
  ...
  !---First attempt only
    if(iattempt .gt. 1)return

  !---Only process AMSUA/MHS and ATMS for now 
    if(.not.(SensorID .eq. sensor_id_n18 .or. SensorID .eq. sensor_id_n19 .or. &
         SensorID .eq. sensor_id_metopA .or. SensorID .eq. sensor_id_metopB .or. &
         SensorID .eq. sensor_id_npp .or. SensorID .eq. sensor_id_n20 .or. &
         SensorID .eq. sensor_id_metopC .or. sensorID .eq. sensor_id_n21 .or. &
         SensorID .eq. sensor_id_metopSGA1))return
  ...
  !---Must be either ocean or ice according to static land/ocean mask
    if (.not. (stypeSfc .eq. OC_TYP .or. stypeSfc .eq. SEAICE_TYP))return
...
!                 print *,'Modified: ChanSel=',ChanSel(1:nchan)
  ...
  subroutine regr_clw_atms(tb,angle,lat,clw)


   REAL, DIMENSION(:), INTENT(IN)  :: tb
   REAL,               INTENT(IN)  :: lat
   REAL,               INTENT(IN)  :: angle
   REAL, INTENT(INOUT)             :: clw

   INTEGER, PARAMETER              :: nbin=10
   INTEGER, PARAMETER              :: ncoeff=9
   INTEGER, DIMENSION(nbin+1)      :: bins
   REAL                            :: angledeg
   REAL,    DIMENSION(nbin)        :: C0
   REAL,    DIMENSION(nbin,ncoeff) :: An
   REAL                            :: tb1,tb2,tb16,tb17,tb18
   INTEGER                         :: ich,idx,ibin,binnum
   INTEGER, PARAMETER              :: nChan4Regr=5
   INTEGER, DIMENSION(nChan4Regr)  :: indCh



   !---Fill angle bin ranges
   bins=(/-66,-52,-39,-26,-13,0,13,26,39,52,66/)
   !---Fill array with constants
  ! C0=(/-14.09709, -16.65445, -18.90298, -19.87510, -20.69270, &
  !      -20.22070, -19.47007, -18.77584, -16.98032, -14.13982/)
   C0=(/26.90532, 47.5366, 74.1856, 84.1347, 103.1971, 100.86844, &
        81.0286, 72.3599, 38.9335, 22.6934/)
   An(1,:) =(/0.00405,-0.03680,0.00573,-0.00166,-0.00053,1.20909,-5.73851,0.01782, 0.00022/)
   An(2,:) =(/0.00662,-0.06973,0.01049,-0.00255,-0.00073,1.65164,-9.58266,0.08084, 0.00022/)
   An(3,:) =(/0.01113,-0.11424,0.01774,-0.00438,-0.00113,2.31829,-14.61933,0.15824,0.00010/)
   An(4,:) =(/0.01657,-0.12843,0.01718,-0.00473,-0.00100,3.02678,-17.01696,0.15281,0.00008/)
   An(5,:) =(/0.01456,-0.14566,0.01562,-0.00294,-0.00166,2.89137,-20.13955,0.19376,0.00011/)
   An(6,:) =(/0.01411,-0.14225,0.01541,-0.00278,-0.00178,2.85289,-19.70847,0.18368,0.00011/)
   An(7,:) =(/0.01456,-0.12277,0.01750,-0.00446,-0.00111,2.77267,-16.32499,0.20941,0.00012/)
   An(8,:) =(/0.01059,-0.11144,0.01809,-0.00415,-0.00127,2.21368,-14.29861,0.22593,0.00015/)
   An(9,:) =(/0.00472,-0.05596,0.00923,-0.00202,-0.00119,1.45355,-7.93502,0.03585, 0.00022/)
   An(10,:)=(/0.00302,-0.02948,0.00452,-0.00079,-0.00129,1.11056,-4.91486,-0.01365,0.00023/)

   !---Index of channel predictors
   indCh=(/1,2,16,17,18/)
   ibin=0

   !---Determine angle bin to use
   getbin: DO binnum=1,nbin
!        angledeg=angle*(180/3.1415)
        angledeg = angle
        IF (angledeg >= bins(binnum) .and. angledeg < bins(binnum+1)) THEN
                ibin=binnum
!                print *,'ibin:',ibin,angle
                EXIT getbin

        ENDIF
   ENDDO getbin
   !---Initialize CLW to constant
!   print *,'angledeg,ibin=',angledeg,ibin
   if(ibin .lt. 1)then
      clw=0.
      return
   endif
   clw=C0(ibin)
   !---Apply tb-based predictors
   DO iCh=1,nChan4regr
        idx=indCh(iCh)
        clw=clw+(tb(idx)*An(ibin,iCh))
   ENDDO
   !---Finalize cloud derivation from extra predictors
!   clw=clw+(cos(Angle)*An(ibin,4))

   if(tb(1) .ge. 300.)then
      clw=clw+( log(300.-299.)*An(ibin,6) )
   else
      clw=clw+( log(300.-tb(1))*An(ibin,6) )
   endif
!   clw=clw+( log(300.-tb(1))*An(ibin,6) )
   if(tb(2) .ge. 300.)then
      clw=clw+( log(300.-299.)*An(ibin,7) )
   else
      clw=clw+( log(300.-tb(2))*An(ibin,7) )
   endif
!   clw=clw+( log(300.-tb(2))*An(ibin,7) )
   if(tb(16) .ge. 300.)then
      clw=clw+( log(300.-299.)*An(ibin,8) )
   else
      clw=clw+( log(300.-tb(16))*An(ibin,8) )
   endif
!   clw=clw+( log(300.-tb(16))*An(ibin,8) )
   clw=clw+(lat*An(ibin,9))
   !---Regression done in log space...convert back to physical space
   !clw=exp(clw)
   !---Low threshold detectability is 0.09 mm clw
   if (clw < 0.05) clw=0.
end subroutine  regr_clw_atms
