...
MODULE IO_Scene
  ...
  PUBLIC :: set2defaultNonOfficialproducts,setSceneEDRs2default,FillSceneNegValues,setSceneHydro2zero
  PUBLIC :: CopyScene
...
! Type:         Subroutine
! Description:  set Hydros in EDR to zero if called
!===============================================================
  SUBROUTINE setSceneHydro2zero(Scene)
    TYPE(Scene_type)      :: Scene

    !---Set these parameters to default values
    where (Scene%CLW(:)     < epsilonHydro .AND. Scene%CLW(:) > 0.    ) Scene%CLW(:)     = 0.
    where (Scene%Rain(:)    < epsilonHydro .AND. Scene%Rain(:) > 0.   ) Scene%Rain(:)    = 0.
    where (Scene%Graupel(:) < epsilonHydro .AND. Scene%Graupel(:) > 0.) Scene%Graupel(:) = 0.

  END SUBROUTINE setSceneHydro2zero
...
!
!
! Description:  Copies content of one scene structure to another
!
!
! Arguments:
!
!      Name                 Type            Description
!      ---------------------------------------------------
!       - Scene              I              Input Structure
!       - Scene              I/O            Output Structure to be filled 
!                                           with content of input Structure
!       - ierr               O              Error index when reading
!
!
! Modules needed:
...
!
! History:
!       05-07-2020      Chris Grassotti NOAA/NESDIS/STAR, UMD/ESSIC/CISESS
!
!===============================================================

  SUBROUTINE CopyScene(Scene1,Scene2,ierr)
    TYPE(Scene_type)   :: Scene1,Scene2
    INTEGER            :: ierr

    ierr=0

    !---Check Metadata dimensions for compatibility
    if(Scene1%iH2O .ne. Scene2%iH2O .or. &
       Scene1%iO3 .ne. Scene2%iO3 .or. &
       Scene1%nLev .ne. Scene2%nLev .or. &
       Scene1%nLay .ne. Scene2%nLay .or. &
       Scene1%nChan .ne. Scene2%nChan .or. &
       Scene1%nPosScan .ne. Scene2%nPosScan .or. &
       Scene1%nScanLines .ne. Scene2%nScanLines .or. &
       Scene1%nAbsorb .ne. Scene2%nAbsorb .or. &
       Scene1%nParmRain .ne. Scene2%nParmRain .or. &
       Scene1%nParmCLW .ne. Scene2%nParmCLW .or. &
       Scene1%nParmRain .ne. Scene2%nParmRain .or. &
       Scene1%nParmSnow .ne. Scene2%nParmSnow .or. &
       Scene1%nParmIce .ne. Scene2%nParmIce .or. &
       Scene1%nParmGrpl .ne. Scene2%nParmGrpl .or. &
       Scene1%nqc .ne. Scene2%nqc)then
       ierr=1
       print *,'Error: incompatible metadata in Scene1 and Scene2'
       return
    endif

    !---Metadata
    Scene2%AbsorbID(1:Scene1%nAbsorb) = Scene1%AbsorbID(1:Scene1%nAbsorb)
    Scene2%pres_lay(1:Scene1%nLay)          = Scene1%pres_lay(1:Scene1%nLay)
    Scene2%pres_lev(1:Scene1%nLev)          = Scene1%pres_lev(1:Scene1%nLev)
    Scene2%CentrFreq(1:Scene1%nchan)        = Scene1%CentrFreq(1:Scene1%nchan)
    Scene2%polarity(1:Scene1%nchan)         = Scene1%polarity(1:Scene1%nchan)

    !---Atmospheric constituents
    Scene2%pres_lay(1:Scene1%nLay)=Scene1%pres_lay(1:Scene1%nLay)
    Scene2%pres_lev(1:Scene1%nLev)=Scene1%pres_lev(1:Scene1%nLev)
    Scene2%Temp_lay(1:Scene1%nLay)=Scene1%Temp_lay(1:Scene1%nLay)
    Scene2%Absorb_lay(1:Scene1%nLay,1:Scene1%nAbsorb)=&
         Scene1%Absorb_lay(1:Scene1%nLay,1:Scene1%nAbsorb)
    !---hydrometeors
    Scene2%Clw(1:Scene1%nParmCLW)=Scene1%Clw(1:Scene1%nParmCLW)
    Scene2%Rain(1:Scene1%nParmRain)=Scene1%Rain(1:Scene1%nParmRain)
    Scene2%Graupel(1:Scene1%nParmGrpl)=Scene2%Graupel(1:Scene1%nParmGrpl)
    Scene2%Snow(1:Scene1%nParmSnow)=Scene1%Snow(1:Scene1%nParmSnow)
    Scene2%Ice(1:Scene1%nParmIce)=Scene1%Ice(1:Scene1%nParmIce)
    !---Emissivity/reflectivity vectors
    Scene2%Emiss(1:Scene1%nchan)=Scene1%Emiss(1:Scene1%nchan)
    !---Surface-level paremeters
    Scene2%angle=Scene1%angle
    Scene2%WindSp=Scene1%WindSp
    Scene2%Tskin=Scene1%Tskin
    Scene2%SfcPress=Scene1%SfcPress
    Scene2%iTypSfc=Scene1%iTypSfc
    Scene2%WindU=Scene1%WindU
    Scene2%WindV=Scene1%WindV
    Scene2%RelAziAngle=Scene1%RelAziAngle
    Scene2%SolZenAngle=Scene1%SolZenAngle
    Scene2%SnowDepth=Scene1%SnowDepth
    !---QC variables 
    Scene2%qc(1:Scene1%nqc)=Scene1%qc(1:Scene1%nqc)
    !---Positioning variables
    Scene2%lat=Scene1%lat
    Scene2%lon=Scene1%lon
    Scene2%node=Scene1%node
    Scene2%scanUTC=Scene1%scanUTC
    Scene2%scanYear=Scene1%scanYear
    Scene2%scanDay=Scene1%scanDay
    Scene2%iscanPos=Scene1%iscanPos
    Scene2%iscanLine=Scene1%iscanLine
    Scene2%iscanLine=Scene1%iTyp
    !---In case the scene is a retrieved one
    IF (Scene1%iTyp .eq. 1) THEN
       Scene2%nAttempt=Scene1%nAttempt
       Scene2%nIter=Scene1%nIter
       Scene2%ChiSq=Scene1%ChiSq
       Scene2%YFwd(1:Scene1%nchan)=Scene1%YFwd(1:Scene1%nchan)
       Scene2%ChanSel(1:Scene1%nchan)=Scene1%ChanSel(1:Scene1%nchan)
       Scene2%Ym(1:Scene1%nchan)=Scene1%Ym(1:Scene1%nchan)
       Scene2%YmCorr(1:Scene1%nchan)=Scene1%YmCorr(1:Scene1%nchan)
    ENDIF
    RETURN
  END SUBROUTINE CopyScene
