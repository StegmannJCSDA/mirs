...
MODULE GeophCovBkg
  ...
  PUBLIC :: LoadBkgAtm_Ext,LoadBkgAtm_Ext_Ascii,GetBkgAtm_Ext,ReplaceBkgAtm_Ext,Destroy_BkgAtm_Ext
...
!===============================================================
  ...
  SUBROUTINE TuneBkgAtm(iEDR_clw,iEDR_rain,iEDR_snow,iEDR_ice, &
    ...
    REAL                           :: FactRed_grpl=1.5,FactRed_clw=2.,FactRed_rain=1.
...
!===============================================================
  ...
  SUBROUTINE LoadBkgAtm_Ext_Ascii()
...
!         bkgAtm_Ext%Stats(:,1,bkgAtm_Ext%nLon+1,bkgAtm_Ext%nHr+1,bkgAtm_Ext%nDay+1,1)
    ...
    RETURN
  END SUBROUTINE  LoadBkgAtm_Ext_Ascii
...
!
! Description:  Reads/loads atmospheric backgroud database with spatial and temporal visibilities. 
!               The subroutine is adapted from LoadBkgAtm_Ext_Ascii to read binary files.
!
!
! Arguments:  None
!
...
!
! History:
!       10-25-2014      Pan Liang, AER Inc @ NOAA/NESDIS/STAR
!
!===============================================================
  ...
  SUBROUTINE LoadBkgAtm_Ext()
    INTEGER                  :: nZ,nLat,nLon,nHr,nDay,nType
    INTEGER                  :: iu,iLat,iLon,iHr,iDay,iTyp
    INTEGER                  :: iEDR,jEDR,recd

    iu=get_lun()
    OPEN(iu,file=CntrlConfig%BkgAtmFile_Ext,access='direct',status='old',recl=1204)
    recd=1

    !---Read the hearder
    read(iu,rec=recd)bkgAtm_Ext%nEDRs,bkgAtm_Ext%nZ,bkgAtm_Ext%nLat,bkgAtm_Ext%nLon,bkgAtm_Ext%nHr,&
         bkgAtm_Ext%nDay,bkgAtm_Ext%nTypes,bkgAtm_Ext%nLev,bkgAtm_Ext%nLay
    recd=recd+1
    ALLOCATE(bkgAtm_Ext%EDR_IDs(bkgAtm_Ext%nEDRs))
    ALLOCATE(bkgAtm_Ext%EDR_Desc(bkgAtm_Ext%nEDRs))
    ALLOCATE(bkgAtm_Ext%iSpaceMode(bkgAtm_Ext%nEDRs))
    ...
    ALLOCATE(bkgAtm_Ext%Type_IDs(bkgAtm_Ext%nTypes))
    ALLOCATE(bkgAtm_Ext%Type_Desc(bkgAtm_Ext%nTypes))
    read(iu,rec=recd)bkgAtm_Ext%EDR_IDs(1:bkgAtm_Ext%nEDRs),bkgAtm_Ext%EDR_Desc(1:bkgAtm_Ext%nEDRs),&
         bkgAtm_Ext%iSpaceMode(1:bkgAtm_Ext%nEDRs),bkgAtm_Ext%Lat1,bkgAtm_Ext%LatN,bkgAtm_Ext%dLat,bkgAtm_Ext%Lon1,&
         bkgAtm_Ext%LonN,bkgAtm_Ext%dLon,bkgAtm_Ext%Hr1,bkgAtm_Ext%HrN,bkgAtm_Ext%dHr,bkgAtm_Ext%julDay(1:bkgAtm_Ext%nDay),&
         bkgAtm_Ext%Type_IDs(1:bkgAtm_Ext%nTypes),bkgAtm_Ext%Type_Desc(1:bkgAtm_Ext%nTypes)

    !---Read pressure info
    ALLOCATE(bkgAtm_Ext%pres_lev(bkgAtm_Ext%nLev))
    ALLOCATE(bkgAtm_Ext%pres_lay(bkgAtm_Ext%nLay))
    recd=recd+1
    read(iu,rec=recd)bkgAtm_Ext%pres_lev
    recd=recd+1
    read(iu,rec=recd)bkgAtm_Ext%pres_lay
       ...
       DO iDay=1,bkgAtm_Ext%nDay
          DO iHr=1,bkgAtm_Ext%nHr
             DO iLon=1,bkgAtm_Ext%nLon
                DO iLat=1,bkgAtm_Ext%nLat
                   recd=recd+1
                   read(iu,rec=recd)bkgAtm_Ext%Stats(:,iLat,iLon,iHr,iDay,iTyp)
                ENDDO
                if(iLon .eq. bkgAtm_Ext%nLon)bkgAtm_Ext%Stats(:,:,iLon+1,iHr,iDay,iTyp)=bkgAtm_Ext%Stats(:,:,1,iHr,iDay,iTyp)
             ENDDO
             if(iHr .eq. bkgAtm_Ext%nHr)bkgAtm_Ext%Stats(:,:,:,iHr+1,iDay,iTyp)=bkgAtm_Ext%Stats(:,:,:,1,iDay,iTyp)
