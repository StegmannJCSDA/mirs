...
MODULE QCchecking
  ...
  PUBLIC :: setQC,setQCDep,setQCSfrDep,checkCLW
...
!===================================================================
  ...
  SUBROUTINE setQC(SensorID,Scene,nqc,Ymqc,Ymnqc,OKRetrvl)
    IMPLICIT NONE
    !---I/O variables   
    TYPE(Scene_type)                          :: Scene
    INTEGER                                   :: SensorID
    ...
    REAL                                      :: ChiSqMAX=10.0
    REAL,            PARAMETER                :: ChiSqMIN=5.0
    REAL,            PARAMETER                :: RWPMIN=0.05,RWPMED=0.2,RWPMAX=2.0
    REAL,            PARAMETER                :: GWPMIN=0.1,GWPMED=0.5,GWPMAX=3.0
    REAL,            PARAMETER                :: TSKINMIN=150.0,TSKINMAX=350.0,dTMAX=3.0
    REAL,            PARAMETER                :: TEMPMIN=140.0,TEMPMAX=350.0,QMIN=0.0,QMAX=90.0
    REAL,            PARAMETER                :: EMISSMIN=0.0,EMISSMAX=1.0,TPWMIN=0.0,TPWMAX=500.0
    REAL,            PARAMETER                :: ICLWMIN=0.0,ICLWMAX=100.0,RWPBMIN=0.0,RWPBMAX=100.0,CLRSKY=0.02 
    REAL,            PARAMETER                :: GWPBMIN=0.0,GWPBMAX=100.0,MAXPRESQ=200
    ...
    !-Precipitation flag
    IF (RWP >= RWPMIN .OR. GWP >= GWPMIN)           QC(2)=IBSET(QC(2),2)
    !-Type of precipitation flag
    IF (RWP >= RWPMIN .AND. RWP <  RWPMED ) QC(2)=IBSET(QC(2),3)
    IF (GWP >= GWPMIN .AND. GWP <  GWPMED ) QC(2)=IBSET(QC(2),3)
    IF (RWP >= RWPMED .AND. RWP <  RWPMAX ) QC(2)=IBSET(QC(2),4)
    IF (GWP >= GWPMED .AND. GWP <  GWPMAX ) QC(2)=IBSET(QC(2),4)
    IF (RWP >= RWPMAX )                     QC(2)=IBSET(QC(2),5)
    IF (GWP >= GWPMAX )                     QC(2)=IBSET(QC(2),5)
    ...
    IF (GWP < GWPBMIN .OR. GWP > GWPBMAX)    QC(2)=IBSET(QC(2),13)
    !-Measurement QC flag
    IF (ANY(Ymqc .GT. 0) .OR. Scene%lat <-90. .OR.  Scene%lon < -180.)      QC(2)=IBSET(QC(2),14) !---in very rare cases, YMQC are good, but lat and lon are missing (-999)
    DO iqc=0,Ymnqc-1
    ...
    !-Validity flags
    IF (RWP >= RWPMIN .OR. GWP >= GWPMIN) THEN
...
!      QC(3)= IBSET(QC(3),8) ! Water Vapor Profile
    ...
    !-For F17, only: For scan positions 89,90, set QC(4) bit 15, and additionally
    ! if QC(0)=0 and using high-resolution data, set QC(1)=1. 
    !-Needed due to end of scan roll-off in calibrated TBs creating end-of-scan bias in EDRs.

    IF(SensorID .eq. sensor_id_f17) THEN
       If(Scene%nPosScan .eq. 90 .and. Scene%iscanPos .ge. 89) THEN
          QC(4)= IBSET(QC(4),15)! General calibration problem
          IF(QC(1) .le. 1)  QC(1)=1 ! 1=SOME PROBLEM
       ENDIF
    ENDIF

    !-For F18, only: For scan position 90, set QC(4) bit 15, and additionally
    ! if QC(0)=0 and using high-resolution data, set QC(1)=1. 
    !-Needed due to end of scan roll-off in calibrated TBs creating end-of-scan bias in EDRs.

    IF(SensorID .eq. sensor_id_f18) THEN
       If(Scene%nPosScan .eq. 90 .and. Scene%iscanPos .eq. 90) THEN
          QC(4)= IBSET(QC(4),15)! General calibration problem
          IF(QC(1) .le. 1)  QC(1)=1 ! 1=SOME PROBLEM
       ENDIF
    ENDIF
...
!===================================================================
  ...
  SUBROUTINE setQCDep (DEP)
    !---I/O variables   
    TYPE(DEP_type)                   :: DEP
    !--- Variable declaration 
    REAL,            PARAMETER       :: RRLGHT=1.0,RRHGT=5.0
    !---Setting the Precipitation Flags
    IF (DEP%RR .GT. 0.0) THEN
       DEP%qc(1)=1
       !---Setting to 1 the Bit 2 of qc(2) (Precipitation Flag)
       DEP%qc(2)=IBSET(DEP%qc(2),2)
       IF(DEP%RR .LT. RRLGHT) THEN
          !-Light Precipitation
          DEP%qc(2)=IBSET(DEP%qc(2),3)
       ELSE IF(DEP%RR .GE. RRLGHT .AND. DEP%RR .LT. RRHGT) THEN
          !-Medium Precipitation
          DEP%qc(2)=IBSET(DEP%qc(2),4)
       ELSE IF(DEP%RR .GE. RRHGT) THEN
          !-High Precipitation
          DEP%qc(2)=IBSET(DEP%qc(2),5)
       ENDIF
    ENDIF
  END SUBROUTINE setQCDep
...
!
! Description: Sets the quality control flags of the MIRS retrieved 
!              products for the SFR data/
!
! History:
!       03-06-2019     Ryan Honeyager, IMSG Inc @ NOAA/NESDIS/STAR
!
!===================================================================

  SUBROUTINE setQCSfrDep (DEP, sfrqc1, sfrqc2)
    !---I/O variables   
    TYPE(DEP_type)                   :: DEP
    INTEGER                          :: sfrqc1
    INTEGER                          :: sfrqc2
    !---SFR convergence
    IF (sfrqc1 .GT. 0) THEN
       !---Setting to 1 the Bit 16 of qc(2)
       DEP%qc(2)=IBSET(DEP%qc(2),16)
    ENDIF
    !---SFR indeterminate rain/snow rate
    IF (sfrqc2 .GT. 0) THEN
       !---Setting to 1 the Bit 17 of qc(2)
       DEP%qc(2)=IBSET(DEP%qc(2),17)
    ENDIF
  END SUBROUTINE setQCSfrDep
...
!
!
! Description: Checks CLW value and returns logical value
!              depending on whether threshold is exceeded
!
! History:
!       05-07-2020     Chris Grassotti, NOAA/NESDIS/STAR, UMD/ESSIC/CISESS
!
!===================================================================
  subroutine checkCLW(Scene, clwTooHigh)
    real,    parameter    :: maxCLW=0.5
    type(Scene_type)      :: Scene    
    logical               :: clwTooHigh

    clwTooHigh = .FALSE.
    if ((Scene%iTypSfc .eq. 2 .OR. Scene%iTypSfc .eq. 0) .and. &
         ColumIntegr(Scene%nLay,Scene%Pres_lay(1:Scene%nLay), &
         Scene%SfcPress,Scene%CLW(1:Scene%nLay)) .gt. maxCLW)then
       clwTooHigh = .TRUE.
    endif
    return
  end subroutine checkCLW
