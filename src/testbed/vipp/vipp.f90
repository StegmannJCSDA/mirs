...
!       12-08-2016      Junye Chen, ESSIC/UMD @ STAR/NESDIS/NOAA
!                          Added JPSS1 N20 ATMS
!       02-03-2020      Yong-Keun Lee, ESSIC/UMD @ STAR/NESDIS/NOAA
!                          Added JPSS2 N21 ATMS
!
!===============================================================
program vipp
  USE misc
  ...
  INTEGER            :: iu_listSweClimo,nFilesSweClimo,nprfSweClimo,iuSweClimo
  INTEGER            :: iu_listFF,nFilesFF,nprfFF,iuFF
  INTEGER            :: landindex=0,landindx=0
  INTEGER            :: surface_type
  REAL               :: tpw
  REAL               :: CLWmax=5.0 ! Maximum possible value of CLW
  REAL               :: hydro_max=10.0 ! Maximum possible value of GWP, RWP, LWP
  REAL               :: rr_max=100.0 ! Maximum possible value of RR
  REAL               :: GTHK,FLVH
  REAL               :: SweClimo, SweStdev
  !---Variables for forest fraction correction
  REAL               :: FFtot
  !---Pointers
  CHARACTER(LEN=LEN_FILE), DIMENSION(:), POINTER  :: edrFiles,depFiles,sweClimoFiles,dumFiles
  CHARACTER(LEN=LEN_FILE), DIMENSION(:), POINTER  :: ffCollocfiles
  !---Structures
  TYPE(Scene_type)                           :: Scene
  TYPE(DEP_type)                             :: DEP, DepSweClimo, FFcolloc
  ! Sea Ice variables
  ...
  !---Control flag for resetting DEPs when QC(1)=2
  ! 1: reset DEP
  ! 0: do not reset DEP
  INTEGER, PARAMETER :: resetDEP=1

  !---Control flag for using GPM preclassified land type in snow retrieval
  ! 1: use sfctyp=land, so retrieve snow when preclassified sfctyp=2 or 3 (land or snow)
  ! 0: do not use sfctyp=land, so retrieve snow ONLY when preclassified sfctyp=3 (snow)
  ! NB: set to 0 when using IMS sfctype, assuming high confidence in snow detection
  ! NB: set to 1 when not using IMS sfctype, assuming less confidence in snow detection
  INTEGER, PARAMETER :: useSfcTypLandGPM=0

  !---Namelist data 
  CHARACTER(LEN=LEN_FILE) :: edrFilesList=DEFAULT_VALUE_STR4
  CHARACTER(LEN=LEN_FILE) :: sweClimoFilesList=DEFAULT_VALUE_STR4
  CHARACTER(LEN=LEN_FILE) :: ffCollocFilesList=DEFAULT_VALUE_STR4
  ...
  INTEGER                 :: useSweClimo=DEFAULT_VALUE_INT
  INTEGER                 :: useFF=DEFAULT_VALUE_INT

  NAMELIST /VIPPNMLST/edrFilesList,SeaiceFile,SnowCoverFile,pathDEPOut,&
       LogFile,norbits2process,nprofs2process,sensor_id,sweClimoFilesList,&
       useSweClimo,ffCollocFilesList,useFF
  !-----------------------------------------------------
  ...
  !---If flag set, read the file names of SWE climatology data
  if(useSweClimo .eq. 1)then
     call ReadList(iu_listSweClimo,trim(sweClimoFilesList),sweClimoFiles,nFilesSweClimo,dumFiles,'','')
     IF (nfilesSweClimo .lt. 1) CALL ErrHandl(ErrorType,Err_NoFilesFound,'') 
     nfilesSweClimo=minval((/norbits2process,nfilesSweClimo/))
     if(nfilesSweClimo .ne. nfiles)&
          CALL ErrHandl(ErrorType,Err_Incompat,'. Nqcnfiles (scene and sweClimo). (VIPP)')
     print *,'Finished reading sweClimo file list, nfilesSweClimo=',nfilesSweClimo
     DEALLOCATE(dumFiles)
  endif
!  stop

  !---If flag set, read the file names of forest fraction data
  if(useFF .eq. 1)then
     call ReadList(iu_listFF,trim(ffCollocFilesList),ffCollocFiles,nFilesFF,dumFiles,'','')
     IF (nfilesFF .lt. 1) CALL ErrHandl(ErrorType,Err_NoFilesFound,'') 
     nfilesFF=minval((/norbits2process,nfilesFF/))
     if(nfilesFF .ne. nfiles)&
          CALL ErrHandl(ErrorType,Err_Incompat,'. Nqcnfiles (scene and ffColloc). (VIPP)')
     print *,'Finished reading ffColloc file list, nfilesFF=',nfilesFF
     DEALLOCATE(dumFiles)
  endif
!  stop
  ...
  FilesLoop: DO ifile=1,nfiles
     ...
     if(useSweClimo .eq. 1)then
     !---- Read header of SweClimo file, which is a DEP type file (if flag set)----
        CALL ReadHdrDEP(iuSweClimo,sweClimoFiles(ifile),DepSweClimo,nprfSweClimo)
        print *,'finished reading sweClimo DEP header,nprfsSweClimo=',nprfSweClimo
     endif
     if(useFF .eq. 1)then
     !---- Read header of forest fraction file, which is a DEP type file (if flag set)----
        CALL ReadHdrDEP(iuFF,ffCollocFiles(ifile),FFcolloc,nprfFF)
        print *,'finished reading FF DEP header,nprfFF=',nprfFF
     endif
     ...
     ProfilesLoop: DO iprof=1,nPrf
        ...
        SweClimo=DEFAULT_VALUE_REAL
        SweStdev=DEFAULT_VALUE_REAL
        FFtot=DEFAULT_VALUE_REAL
        !---Read content of SWE Climatology file (if flag set)
        IF(useSweClimo .eq. 1)THEN
           CALL ReadDep(iuSweClimo,DepSweClimo,ierr)
           IF (ierr.ne.0) CALL ErrHandl(ErrorType,Err_ReadingFile,'. DEP SweClimo file. (VIPP)')
           SweClimo=DepSweClimo%SWE
           SweStdev=DepSweClimo%SnowCover
        ENDIF
        !---Read content of forest fraction file (if flag set)
        IF(useFF .eq. 1)THEN
           CALL ReadDep(iuFF,FFcolloc,ierr)
           IF (ierr.ne.0) CALL ErrHandl(ErrorType,Err_ReadingFile,'. DEP FF file. (VIPP)')
           FFtot=FFcolloc%SWE ! FF stored in SWE variable
        ENDIF
...
! Set itypAtm to missing, as it is currently not used
        DEP%iTypAtm    = DEFAULT_VALUE_INT
        ...
        !liusy+ reset hydrometeor to maximum value
        IF (DEP%CLW > hydro_max) THEN
           DEP%CLW  = hydro_max
        ENDIF
        IF (DEP%RWP > hydro_max) THEN
           DEP%RWP  = hydro_max
        ENDIF
        IF (DEP%GWP > hydro_max) THEN
           DEP%GWP  = hydro_max
        ENDIF
        IF (DEP%LWP > hydro_max) THEN
           DEP%LWP  = hydro_max
        ENDIF
        !liusy.
        !---Implement a climatological sea-ice mask for GWP and RWP prducts
        IF(sensor_id .EQ. SENSOR_ID_F16 .OR. & 
             sensor_id .EQ. SENSOR_ID_N18 .OR. &
             sensor_id .EQ. SENSOR_ID_N19 .OR. &
             sensor_id .EQ. SENSOR_ID_METOPA .OR. &
             sensor_id .EQ. SENSOR_ID_METOPB .OR. &
             sensor_id .EQ. SENSOR_ID_METOPC .OR. &
             sensor_id .EQ. SENSOR_ID_METOPSGA1 .OR. &
             sensor_id .EQ. SENSOR_ID_NPP .OR. &
             sensor_id .EQ. SENSOR_ID_N20 .OR. &
             sensor_id .EQ. SENSOR_ID_N21 .OR. &
             sensor_id .EQ. SENSOR_ID_F17 .OR. &
             sensor_id .EQ. SENSOR_ID_F18 .OR. &
             sensor_id .EQ. SENSOR_ID_F19) THEN
           IF(DEP%iTypSfc .EQ. SEAICE_TYP .OR. DEP%iTypSfc .EQ. OC_TYP ) THEN
              ...
              IF(surface_type .NE. OC_TYP) THEN
                 DEP%RWP = DEFAULT_VALUE_REAL
                 DEP%GWP = DEFAULT_VALUE_REAL
                 DEP%LWP = DEP%CLW
              ENDIF
           ENDIF
        ENDIF
...
!        IF (Scene%iTypSfc .ne. OC_TYP)                                      DEP%LWP = DEFAULT_VALUE_REAL
        ...
        IF (Scene%iTypSfc .eq. SEAICE_TYP .or. Scene%iTypSfc .eq. SNOW_TYP) DEP%LWP = DEFAULT_VALUE_REAL
        ...
        DEP%Prob_SF    = DEFAULT_VALUE_REAL
        DEP%Prob_RF    = DEFAULT_VALUE_REAL
        ...
        if(Sensor_id .ne. sensor_id_mtma .and. Sensor_id .ne. sensor_id_mtsa)then
           ...
           CALL ComputeSWEandGS(sensor_id,Scene%Emiss(1:Scene%nchan),Scene%Ym(1:Scene%nchan),&
                Scene%nchan,Scene%Tskin,DEP%lat,DEP%lon, &
                Scene%iTypSfc,Scene%qc,SnowData,nprofsnow,nparamsnow,DEP%SnowGS,DEP%SWE,&
                SweClimo,SweStdev,useSweClimo,FFtot,useFF)

           IF (ComputeSfcTyp(DEP%SIC, DEP%SWE) .GE. 0) DEP%iTypSfc = ComputeSfcTyp(DEP%SIC, DEP%SWE)
           CALL DetectSnowCover(DEP%SWE,DEP%SnowCover)
           if( sensor_id .eq. sensor_id_fy3ri .or. &
                sensor_id .eq. sensor_id_trmm ) DEP%iTypSfc = Scene%iTypSfc
        ...
        !--Check for excessive CLW (and hence RR) values and reset to missing.
        !--NB: This is primarily a short-term fix for the high CLW values produced over land
        !--A longer-term fix should be made in 1dvar.f90, governing the criteria
        !--triggering a 2nd attempt.
        IF(DEP%iTypSfc .EQ. LD_TYP .AND. DEP%CLW .GT. CLWmax)THEN ! liusy: reset the CLW to CLWmax (5.0) and move before RR function?
           DEP%CLW=DEFAULT_VALUE_REAL
           DEP%RR=DEFAULT_VALUE_REAL
        ENDIF
        !liusy+ reset 
        IF (DEP%RR > rr_max) THEN
           DEP%RR  = rr_max
        ENDIF
        !liusy.
        ...
        !---Apply QC to DEP structure
        CALL setQCDep(DEP)
        !---If QC(1)=2 (bad) and resetDEP, or if niter=-99 (no retrieval attempted)
        !---reset all derived products to missing
        !---To retain derived product values for diagnostics, set resetDEP=0, and recompile
        IF(Scene%nIter .eq. -99 .or. (DEP%qc(1) .EQ. 2 .and. resetDEP .eq. 1))THEN
           DEP%TPW        = DEFAULT_VALUE_REAL
           DEP%GWP        = DEFAULT_VALUE_REAL
           DEP%RWP        = DEFAULT_VALUE_REAL
           DEP%LWP        = DEFAULT_VALUE_REAL
           DEP%SIC        = DEFAULT_VALUE_REAL
           DEP%SIC_FY     = DEFAULT_VALUE_REAL
           DEP%SIC_MY     = DEFAULT_VALUE_REAL
           DEP%SWE        = DEFAULT_VALUE_REAL
     ...
     CLOSE (iuEDR)
     CLOSE (iuDEP)
     !---Close the SweClimo file (if flag was set)
     IF(useSweClimo .eq. 1)THEN
        CLOSE (iuSweClimo)
     ENDIF
     !---Close the forest fraction file (if flag was set)
     IF(useFF .eq. 1)THEN
        CLOSE (iuFF)
     ENDIF
     !---Release memory
  ...
  DEALLOCATE(depFiles)
  if(useSweClimo .eq. 1)then
     DEALLOCATE(sweClimoFiles)
  endif
  if(useFF .eq. 1)then
     DEALLOCATE(ffCollocFiles)
  endif
CONTAINS
  ...
  SUBROUTINE RegressRR(SatID,Year,JDay,Lon,Lat,SfcTyp,LWPI,GWPI,RWPI,CLWI,TPWI,Q_lay, &
    ...
    IF(SfcTyp .EQ. OC_TYP .OR. SfcTyp .EQ. LD_TYP) THEN
  ...
  SUBROUTINE ComputeSWEandGS(sensor_id,Emissivity,Ym,nchan,Tskin,flat,flon,SfcTyp,qc,SnowData,nprofSnow,&
       nparmSnow,EffGrainSize,ComputeSWE,SweClimo,SweStdev,useSweClimo,FFtot,useFF)
    IMPLICIT NONE
    REAL                  :: Tskin,mindistance,distance,distance0,ComputeSWE,slope1,slope2,slope3,slope4, &
         slope5,slope6,slope7,slope8,slope9,coeff19,coeff22,coeff23,coeff31,coeff37,coeff50,coeff89, &
         coeff92,coeff157,coeff190,flat,flon,slope3a
    INTEGER                  :: sensor_id,i,nchan,SfcTyp,nprofSnow,nparmSnow
    REAL,    DIMENSION(:) :: Emissivity, Ym
    ...
    REAL :: SWE0 ! Background mean SWE (cm)
    REAL :: GS0 ! Background mean GS (mm)
    ...
    REAL :: sweClimo, SweStdev, FFtot
    INTEGER :: useSweClimo, useFF
    REAL :: BiasSweClimo = 3.5 ! assumed bias in SWE climatology (cm)
  !---Variables for forest fraction correction 
    REAL               :: dFF,corEm2331,corEm3150,corEm2350,corEm5088,corEm3188
    REAL, PARAMETER    :: FF0=0.0
    REAL, PARAMETER    :: dEm2331=0.0348 ! empirical FF correction factor to emissivity gradient
    REAL, PARAMETER    :: dEm3150=0.0619 ! empirical FF correction factor to emissivity gradient
    REAL, PARAMETER    :: dEm2350=0.0967 ! empirical FF correction factor to emissivity gradient
    REAL, PARAMETER    :: dEm5088=0.0457 ! empirical FF correction factor to emissivity gradient
    REAL, PARAMETER    :: dEm3188=0.1074 ! empirical FF correction factor to emissivity gradient
    !--- emissivity gradient upper limit for FF correction (no correction above this value)
    REAL, PARAMETER    :: MaxEm3188CorrectTaper=0.15 
    !--- emissivity gradient lower limit for FF correction (taper correction linearly above this value)
    REAL, PARAMETER    :: MinEm3188CorrectTaper=0.10 
    REAL               :: CorTaper

!    if(flat .ge. 60. .and. flat .le. 65. .and. &
!         flon .ge.90. .and. flon .le. 95.)then
!       print *,'computeSWEandGS: flat,flon,SweClimo,SweStdev,useSweClimo',&
!            flat,flon,SweClimo,SweStdev,useSweClimo
!       write(98,*)'computeSWEandGS: flat,flon,SweClimo,SweStdev,useSweClimo',&
!            flat,flon,SweClimo,SweStdev,useSweClimo
!    endif
    ...
    ! Additional update to include option of using SWE climatology information
    ! C. Grassotti, AER, Inc.
    ! March 2015


    ! Set background SWE and GS mean and standard deviation (uncertainty)
    ! Values depend on whether climatology is being used.
    ! Bkg uncertainty is further multiplied by sensor-dependent scale factors 
    ! scaleBGerrSWE and scaleBGerrGS.

    ! Use SWE climatology
    SWE0         = DEFAULT_VALUE_REAL !liusy+
    if(useSweClimo .gt. 0)then
       
       if(SweClimo .gt. 0.)SWE0=SweClimo-BiasSweClimo ! adjust SWE climo for bias
       SWE0=max(0.,SWE0) ! SWE (cm)
       GS0=0.30 ! GS (mm)
       errSWE=8.07 ! BG SWE uncertainty (cm),  equivalent to 32 cm snow depth uncertainty
       errGS=0.35  ! BG GS uncertainty (mm)
    ! Do not use SWE climatology
    else
       SWE0=1.5 ! Bkg mean SWE (cm)
       GS0=0.40 ! Bkg mean GS (mm)
       errSWE=2.52 ! Bkg SWE uncertainty (cm)
       errGS=0.2 ! Bkg GS uncertainty (cm)
    endif

!    print *,'useSweClimo,SWE0=',useSweClimo,SWE0

    ! Initialize curent SWE and GS to missing
   !liusy+
    multSWE      = DEFAULT_VALUE_REAL
    wtSWE        = DEFAULT_VALUE_REAL
    deltaSWE     = DEFAULT_VALUE_REAL
   !liusy.
    ...
    IF ( (Sensor_id .eq. sensor_id_n18 .or. Sensor_id .eq. sensor_id_n19) &
       ...
       ! These are scaling factors that multiply the assumed uncertainty in SWE and GS
       ! Ideally, they should be the same for all sensors, but testing shows that thus far AMSUA/MHS factors need to 
       ! be slightly different than those for SSMIS, possibly due to the emissivity catalogs and/or the number of terms used 
       ...
       !---Initialize forest fraction correction variables
       dFF=0.
       corEm2331=0.
       corEm2331=0.
       corEm2350=0.
...
!      corTaper=0.
       ...
       !--- If flag set, compute corrections to retrieved emissivity gradients
       !    based on forest fraction
       if(useFF .gt. 0)then
          if(FFtot .ge. 0.)then
             if((Emissivity(2) - Emissivity(16)) .gt. &
                  MinEm3188CorrectTaper .and. &
                  (Emissivity(2) - Emissivity(16)) .le. &
                  MaxEm3188CorrectTaper)then
                corTaper=(MaxEm3188CorrectTaper-&
                     (Emissivity(2) - Emissivity(16)))/&
                     (MaxEm3188CorrectTaper-MinEm3188CorrectTaper)
             elseif((Emissivity(2) - Emissivity(16)) .gt. &
                  MaxEm3188CorrectTaper)then
                corTaper=0.
             endif
             if(corTaper .gt. 1 .or. corTaper .lt. 0.)then
                print *,'ERROR: corTaper invalid: ',corTaper
             endif
             dFF=FFtot-FF0
             corEm2331=dEm2331*dFF
             corEm3150=dEm3150*dFF
             corEm2350=dEm2350*dFF
             corEm5088=dEm5088*dFF*corTaper
             corEm3188=dEm3188*dFF*corTaper
!             corEm5088=dEm5088*dFF
!             corEm3188=dEm3188*dFF
          endif
       endif
       ...
       DO i=1,nprofSnow
          ...
          slope1 = ( (Emissivity(1) - Emissivity(2) + corEm2331) -  (EM23(i) - EM31(i)) )**2
          slope2 = ( (Emissivity(1) - Emissivity(3) + corEm2350) -  (EM23(i) - EM50(i)) )**2
          slope6 = ( (Emissivity(2) - Emissivity(3) + corEm3150) -  (EM31(i) - EM50(i)) )**2
! needed with CRTM/MiRS emissivity model catalog 
! test "bias adjusting" the table values: reduce gradients by factor
          slope7 = ( (Emissivity(2) - Emissivity(16) + corEm3188) -  0.5*(EM31(i) - EM89(i)) )**2
          slope9 = ( (Emissivity(3) - Emissivity(16) + corEm5088) -  (EM50(i) - EM89(i)) )**2
          slope8 = ( (Emissivity(2) - Emissivity(17)) -  (EM31(i) - EM157(i)) )**2
          ...
          slope7 = slope7/(coeff89**2) 
          slope9 = slope9/(coeff89**2) 

! Screen for invalid values in catalog (model not valid for higher SWE and/or GS for some frequencies)
          if(EM50(i) .eq. DEFAULT_VALUE_REAL) then
             slope2 = DEFAULT_VALUE_REAL
             slope6 = DEFAULT_VALUE_REAL
             slope9 = DEFAULT_VALUE_REAL
          endif
          if(EM89(i) .eq. DEFAULT_VALUE_REAL) then
             slope3 = DEFAULT_VALUE_REAL
             slope4 = DEFAULT_VALUE_REAL
             if(slope2 .ne. DEFAULT_VALUE_REAL .and. slope6 .ne. DEFAULT_VALUE_REAL)then
                slope7 = (slope1+slope2+slope6)/3.
             else
                slope7 = DEFAULT_VALUE_REAL
             endif
             slope9 = DEFAULT_VALUE_REAL
          endif

          ! Distance is the sum over several channel combinations (in this case: 23-31, 23-50, 31-50 GHz)
          distance = slope1+slope2+slope6+slope7
          if(slope1 .eq. DEFAULT_VALUE_REAL .or. &
             slope2 .eq. DEFAULT_VALUE_REAL .or. &
             slope7 .eq. DEFAULT_VALUE_REAL .or. &
             slope6 .eq. DEFAULT_VALUE_REAL)then
             distance=DEFAULT_VALUE_REAL
          endif
          ...
          if(distance0 .eq. DEFAULT_VALUE_REAL)distance=DEFAULT_VALUE_REAL
          if(distance .eq. DEFAULT_VALUE_REAL)cycle
       ...
       ELSE
          IF( (tskin .gt. 273.0) .or. &
               (Emissivity(2) - Emissivity(17)) .lt. 0.08 .and. &
               Ym(17) .gt. 245. .and. &
               (Emissivity(1) - Emissivity(2)) .lt. 0.0) then
             ComputeSWE = 0.0
          ENDIF
          !--- Mask out Sahara (reduce false alarms over desert in winter)
          IF(flat .ge. 15. .and. flat .le. 30. &
               .and. flon .ge. -20. .and. flon .le. 40.) THEN
             ComputeSWE = 0.0
          ENDIF
       ENDIF
...
! Screen for invalid values in catalog (model not valid for higher SWE and/or GS for some frequencies)
    ...
    IF ( Sensor_id .eq. sensor_id_metopC .and. (SfcTyp .eq. SNOW_TYP .or. SfcTyp .eq. LD_TYP) ) THEN
...
! Screen for invalid values in catalog (model not valid for higher SWE and/or GS for some frequencies)
    ...
    IF ( (Sensor_id .eq. sensor_id_f16 .or. &
       ...
       if(useFF .gt. 0)then
          print *,'WARNING: VIPP SWE forest fraction correction not implemented for F16,F17,F18,F19'
          print *,'Processing normally without correction.'
       endif
       ...
       DO i=1,nprofSnow
          ...
          slope3a = ( (Emissivity(14) - Emissivity(16)) -  (EM22V(i) - EM37V(i)) )**2 ! new term: better results
          ...
          slope3a=slope3a/(coeff22**2) ! best result
!          slope3a=slope3a/(0.070**2)
!          slope4=slope4/(coeff92**2)


          ! Distance is the sum over several channel combinations (in this case: 19H-37H, 19V-37V, 19V-22V GHz)
!          distance = slope1+slope2+slope3 ! this was operational
          distance = slope1+slope2+slope3+slope3a ! this gave best results so far
!          distance = slope1+slope2+slope3+slope4 
          distance0=distance
       ...
       IF ( ABS(ComputeSWE) .lt. epsilon) EffGrainSize = 0.0
       !Test: make sure sweClim is being used everywhere, set retrieved SWE=climo
!       IF(SweClimo .ge. 0.)ComputeSWE=SweClimo
    ...
    IF ( (Sensor_id .eq. sensor_id_amsre .or. Sensor_id .eq. sensor_id_gcomw1) &
       ...
       if(useFF .gt. 0)then
          print *,'WARNING: VIPP SWE forest fraction correction not implemented for AMSRE and AMSR2'
          print *,'Processing normally without correction.'
       endif
...
! For GPM, if useSfcTypLandGPM=0, only allow SWE and GS retrieval if preclassified type is snow
! NB: assumes high confidence in sfctyp classification, such as when IMS type is used
    IF ( Sensor_id .eq. sensor_id_gpm ) THEN 
    IF ( SfcTyp .eq. SNOW_TYP .or. SfcTyp .eq. LD_TYP .or. &
         SfcTyp .eq. DESERT_TYP )THEN
       ComputeSWE   = 0.0
       EffGrainSize = 0.0
    ENDIF
    IF ( SfcTyp .eq. SNOW_TYP .or. &
       ( useSfcTypLandGPM .eq. 1 .and. SfcTyp .eq. LD_TYP) .or. &
         SfcTyp .eq. DESERT_TYP ) THEN
       ...
       ! test: assume BG GS=0.4 and higher constraint
       GS0=0.40 ! GS (mm)
       if(useSweClimo .gt. 0)then
          scaleBGerrSWE=0.5 !oper
!          scaleBGerrSWE=1.
!          scaleBGerrSWE=10.
!oper          scaleBGerrGS=0.5
          scaleBGerrGS=.01
       else
          scaleBGerrSWE=2.24 ! sqrt(5.0)
          scaleBGerrGS=0.32 ! sqrt(0.10)
       ...
       if(useFF .gt. 0)then
          print *,'WARNING: VIPP SWE forest fraction correction not implemented for GPM'
          print *,'Processing normally without correction.'
       endif
       ...
       DO i=1,nprofSnow
          ...
          ! Compute distance of retrieved emissivity (gradients) to catalog values
          slope1 = ( (Emissivity(4) - Emissivity(7)) -  (EM19H(i) - EM37H(i)) )**2
          slope2 = ( (Emissivity(3) - Emissivity(6)) -  (EM19V(i) - EM37V(i)) )**2
          slope3 = ( (Emissivity(3) - Emissivity(5)) -  (EM19V(i) - EM22V(i)) )**2
!          slope4 = ( (Emissivity(4) - Emissivity(9)) -  (EM19H(i) - EM92H(i)) )**2
!          slope5 = ( (Emissivity(3) - Emissivity(8)) -  (EM19V(i) - EM92V(i)) )**2
!          slope4 = ( (Emissivity(4) - Emissivity(9)) -  0.05*(EM19H(i) - EM92H(i)) )**2
!          slope5 = ( (Emissivity(3) - Emissivity(8)) -  0.05*(EM19V(i) - EM92V(i)) )**2
          slope4 = ( (Emissivity(4) - Emissivity(9)) -  0.20*(EM19H(i) - EM92H(i)) )**2
          slope5 = ( (Emissivity(3) - Emissivity(8)) -  0.20*(EM19V(i) - EM92V(i)) )**2
          ...
          ! Distance is the sum over several channel combinations (in this case: 19H-37H, 19V-37V, 19V-22V GHz)
!          distance = slope1+slope2+slope3 ! oper: 19h-37h, 19v-37v, 19v-22v
!          distance = slope1+slope3 ! oper: 19h-37h, 19v-37v, 19v-22v
!          distance = slope1+slope2+slope3+slope4+slope5 ! oper: 19h-37h, 19v-37v, 19v-22v, 19h-89h, 19v-89v
!          distance = slope1+slope2+slope3+slope4 ! oper: 19h-37h, 19v-37v, 19v-22v, 19h-89h, 19v-89v
          distance = slope1+slope3+slope4 ! oper: 19h-37h, 19v-22v, 19h-89h
!          distance = slope1+slope4 ! oper: 19h-37h, 19v-37v, 19v-22v, 19h-89h, 19v-89v
!          distance = slope4+slope5 ! oper: 19h-37h, 19v-37v, 19v-22v, 19h-89h, 19v-89v
!          distance = slope4 ! oper: 19h-37h, 19v-37v, 19v-22v, 19h-89h, 19v-89v


! Screen for invalid values in catalog (model not valid for higher SWE and/or GS for some frequencies)
          if(EM19H(i) .eq. DEFAULT_VALUE_REAL) then
             slope1 = DEFAULT_VALUE_REAL
             slope4 = DEFAULT_VALUE_REAL
          endif
          if(EM19V(i) .eq. DEFAULT_VALUE_REAL) then
             slope2 = DEFAULT_VALUE_REAL
             slope3 = DEFAULT_VALUE_REAL
             slope5 = DEFAULT_VALUE_REAL
          endif
          if(EM22V(i) .eq. DEFAULT_VALUE_REAL) then
             slope3 = DEFAULT_VALUE_REAL
          endif
          if(EM37H(i) .eq. DEFAULT_VALUE_REAL) then
             slope1 = DEFAULT_VALUE_REAL
          endif
          if(EM37V(i) .eq. DEFAULT_VALUE_REAL) then
             slope2 = DEFAULT_VALUE_REAL
          endif
          if(EM92H(i) .eq. DEFAULT_VALUE_REAL) then
             slope4 = DEFAULT_VALUE_REAL
          endif
          if(EM92V(i) .eq. DEFAULT_VALUE_REAL) then
             slope5 = DEFAULT_VALUE_REAL
          endif
          if(&
!             slope1 .eq. DEFAULT_VALUE_REAL .or. &
!             slope2 .eq. DEFAULT_VALUE_REAL .or. &
             slope4 .eq. DEFAULT_VALUE_REAL .or. &
             slope5 .eq. DEFAULT_VALUE_REAL &
!             .or. &
!             slope3 .eq. DEFAULT_VALUE_REAL &
             )then
             distance=DEFAULT_VALUE_REAL
          endif
       ...
       IF( (flat .le. -62.) .or. (flat .ge. 59.6 .and. flat .le. 83.4 .and. flon .ge. -71.8 .and. &
            flon .le. -14.1) ) THEN
          tb19H = tskin * emissivity(4)
          PRtb19  = tskin * (emissivity(3) - emissivity(4))
          if(tb19H .le. 215.0 .and. PRtb19 .ge. 23.0) then
             EffGrainSize = 0.75
             ComputeSWE = 15.0
          ENDIF
       ELSE IF( flat .ge. 0. .and. flat .le. 30. .and. &
               flon .ge. -20. .and. flon .le. 60.) THEN
          ComputeSWE = 0.0
       ELSE
          IF(  (tskin .gt. 273.0) .or. (Emissivity(3) - Emissivity(6) .lt. 0.0) &
               .or. (Emissivity(4) - Emissivity(7) .lt. 0.0) &
               .or. (Emissivity(4) - Emissivity(9) .lt. 0.02)) THEN
!          IF(  (Emissivity(3) - Emissivity(6) .lt. 0.0) &
!               .or. (Emissivity(4) - Emissivity(7) .lt. 0.0)) THEN
!          IF(  (tskin .gt. 273.0))THEN
             ComputeSWE = 0.0
          ENDIF
          !--- Mask out Sahara (reduce false alarms over desert in winter)
          IF(flat .ge. 15. .and. flat .le. 30. &
    ...
    IF ((Sensor_id .eq. sensor_id_npp .or. Sensor_id .eq. sensor_id_n20  &
         .or. Sensor_id .eq. sensor_id_n21 .or. Sensor_id .eq. sensor_id_metopSGA1) .and. &
         (SfcTyp .eq. SNOW_TYP .or. SfcTyp .eq. LD_TYP) ) THEN
...
!      corTaper=0.
       ...
       if(useFF .gt. 0)then
          if(FFtot .ge. 0.)then
!          if(FFtot .ge. 0. .and. tskin .le. 273. .and. &
!            (Emissivity(2) - Emissivity(16)) .ge. 0.05 .or. &
!            (Emissivity(2) - Emissivity(17)) .ge. 0.05 .or. &
!            (Emissivity(1) - Emissivity(2)) .ge. 0.0) then
             if((Emissivity(2) - Emissivity(16)) .gt. &
                  MinEm3188CorrectTaper .and. &
                  (Emissivity(2) - Emissivity(16)) .le. &
                  MaxEm3188CorrectTaper)then
...
! Screen for invalid values in catalog (model not valid for higher SWE and/or GS for some frequencies)
          ...
          if(EM89(i) .eq. DEFAULT_VALUE_REAL) then
             slope3 = DEFAULT_VALUE_REAL
             slope4 = DEFAULT_VALUE_REAL
!             if(slope7 .ne. 0.)slope7 = DEFAULT_VALUE_REAL
             if(slope2 .ne. DEFAULT_VALUE_REAL .and. slope2 .ne. DEFAULT_VALUE_REAL)then
                slope7 = (slope1+slope2+slope6)/3.
             else
                slope7 = DEFAULT_VALUE_REAL
             endif
          ...
          ! Distance is the sum over several channel combinations (in this case: 23-31, 23-50, 31-50 GHz)
          distance = slope1+slope2+slope6+slope7 ! good results if 31-89 table scaled by 0.5
          if(slope1 .eq. DEFAULT_VALUE_REAL .or. &
             slope2 .eq. DEFAULT_VALUE_REAL .or. &
            slope7 .eq. DEFAULT_VALUE_REAL .or. &
!             slope9 .eq. DEFAULT_VALUE_REAL .or. &
             slope6 .eq. DEFAULT_VALUE_REAL)then
             distance=DEFAULT_VALUE_REAL
          endif
       ...
       ELSE
          IF( (tskin .gt. 273.0) .or. &
! for 2016-01-24?
!               (Emissivity(2) - Emissivity(16)) .lt. 0.04 .or. &
! for 2016-01-24?
               (Emissivity(2) - Emissivity(17)) .lt. 0.08 .and. &
               Ym(17) .gt. 245. .and. &
               (Emissivity(1) - Emissivity(2)) .lt. 0.0) then
  ...
  SUBROUTINE ComputeSIC(sensor_id,Emissivity,nchan,Tskin,angle,flat,SfcTyp,qc,SeaiceData, &
    ...
    ! NB: limited testing shows that 0.055 rather than 0.045 improves FY/MY classification (increases FY/decreases MY)
    REAL,    PARAMETER            :: AMSUA_SICFYMY=0.055 
    REAL,    PARAMETER            :: ATMS_SICFYMY=0.055 
    ...
    IF( (Sensor_id .eq. sensor_id_n18 .or. Sensor_id .eq. sensor_id_n19) &
       ...
       DO i=indexLow,indexHigh
          ...
          if(gradient50 .ge. AMSUA_SICFYMY) then
          ...
          ENDIF
          IF(distance1 .le. mindistance1 .and. gradient50 .lt. AMSUA_SICFYMY) THEN
       ...
       if(SIC .gt. SIC_HIGH) then
          if(c_total1 .gt. c_total .and. gradient50 .lt. AMSUA_SICFYMY) then
             SIC = c_total1
             SIC_FY = c_fy1
             SIC_MY = c_my1
          endif
       ...
       if (SIC .lt. SIC_LOW .and. SIC .gt. 0.0) then
          ...
          if(SIC .gt. 0.0) then
             if(c_total1 .lt. c_total .and. gradient50 .lt. AMSUA_SICFYMY) then
                SIC = c_total1
                SIC_FY = c_fy1
                SIC_MY = c_my1
             endif
    ...
    IF (Sensor_id .eq. sensor_id_metopC .and. (SfcTyp .eq. SEAICE_TYP .or.  SfcTyp .eq. OC_TYP)) THEN
       if(nAngleStratification .eq. 1 .and. nLatStratification .eq. 1) then
          indexLow = 1
          indexHigh = nprofSeaIce
       endif
    ...
    ! Modified SIC algorithm for F16/F17/F18/F19 SSMIS
    ! C. Grassotti, Sept. 2010
    ...
    IF ((Sensor_id .eq. sensor_id_npp .or. Sensor_id .eq. sensor_id_n20  &
       ...
       DO i=indexLow,indexHigh
          ...
          if(gradient50 .ge. ATMS_SICFYMY) then
             distance = sqrt( (diff31 + diff50) / 2.0)
          else
             distance    = sqrt((diff(1) + diff(2) + diff(3) + diff(16) + diff(17) ) / 5.0)
             distance1   = sqrt((diff31 + diff50 + diff89 + diff157) / 4.0)
          ...
          ENDIF
          IF(distance1 .le. mindistance1 .and. gradient50 .lt. ATMS_SICFYMY) THEN
             mindistance1 = distance1
             !               c_total1 = 100.0*SeaIceData(i,nchan+1)
             !               c_fy1 = 100.0*SeaIceData(i,nchan+2)
             !               c_my1 = 100.0*SeaIceData(i,nchan+3)
       ...
       SIC_MY = c_my
       if(SIC .gt. SIC_HIGH) then 
          if(c_total1 .gt. c_total .and. gradient50 .lt. ATMS_SICFYMY) then
             SIC = c_total1
             SIC_FY = c_fy1
             SIC_MY = c_my1
          endif
       ...
       if (SIC .lt. SIC_LOW .and. SIC .gt. 0.0) then
          ...
          if(SIC .gt. 0.0) then
             if(c_total1 .lt. c_total .and. gradient50 .lt. ATMS_SICFYMY) then
                SIC = c_total1
                SIC_FY = c_fy1
                SIC_MY = c_my1
             endif
  ...
  FUNCTION ComputeSfcTyp(SIC,SWE)
    ...
    REAL       ::SWE,SIC
    ComputeSfcTyp = DEFAULT_VALUE_INT
  ...
  ! History:
  !       05-18-2009      Flavio Iturbide-Sanchez, IMSG Inc @ NOAA/NESDIS/STAR
  !       07-11-2014      Tanvir Islam, CIRA @ NOAA/NESDIS/STAR
  !
  !===============================================================
  SUBROUTINE MM5RR(SfcTyp,QC,CLW,GWP,RWP,RR)
    ...
    INTEGER(2), DIMENSION(:) :: QC
    REAL                     :: CLW,GWP,RWP
    REAL                     :: RR,RR_CLW,RR_GWP,RR_RWP

    RR=0.0   !---- This is must, otherwise, if not met, RR gets a random value ----
    ...
    IF( (SfcTyp .EQ. OC_TYP) .AND. (QC(1) .GE. 0) .AND. (QC(1) .NE. 2)) THEN
       IF( (CLW .GT. 0.275) .OR. (GWP .GT. 0.001) .OR. (RWP .GT. 0.001)) THEN
          ...
          IF(GWP .LT. 0.0) GWP = 0.0
          IF(RWP .LT. 0.0) RWP = 0.0
          IF(Sensor_id .ne. sensor_id_mtsa) THEN
             !--RR = f(CLW) & RR=f(GWP+RWP)
             RR_CLW = 2.339*CLW**1.156 !-- This is regressed between MiRS CLW and 2A12 RR
             RR = RR_CLW + 3.879*(GWP+RWP)**1.103
             !--Blended RR
             !RR_CLW = 2.339*CLW**1.156
             !RR_GWP = 5.798*GWP**1.054
             !RR_RWP = 10.03*RWP**1.13
             !RR = RR_CLW +(RR_GWP+RR_RWP)/2
          ELSEIF(Sensor_id .eq. sensor_id_mtsa) THEN
!             RR = 1.37*GWP**1.67  !---V10
             RR = 5.798*GWP**1.054
          ENDIF
       ENDIF
    ENDIF
    !-Over Land Surface
    IF((SfcTyp .EQ. LD_TYP) .AND. (QC(1) .GE. 0) .AND. (QC(1) .NE. 2)) THEN
       IF( (CLW .GT. 0.275) .OR. (RWP .GT. 0.001) .OR. (GWP .GT. 0.001)) THEN
          IF(CLW .LT. 0.0) CLW = 0.0
          IF(GWP .LT. 0.0) GWP = 0.0
          IF(RWP .LT. 0.0) RWP = 0.0
! For GPM, use RR relationship that includes RWP 
          IF(Sensor_id .EQ. sensor_id_gpm) THEN
             !--RR = f(GWP+RWP)
             RR_CLW = 2.339*CLW**1.156 !-- This is regressed between MiRS CLW and 2A12 RR
             RR = RR_CLW + 3.879*(GWP+RWP)**1.103
          ELSEIF(Sensor_id .ne. sensor_id_mtsa) THEN
             !--RR = f(GWP)
!             RR_GWP = 5.798*GWP**1.054
!             RR = RR_GWP
             !--RR = f(GWP+RWP)
             !RR = 3.879*(GWP+RWP)**1.103
             RR_CLW = 2.339*CLW**1.156 !-- This is regressed between MiRS CLW and 2A12 RR
             RR = RR_CLW + 3.879*(GWP+RWP)**1.103
             !--Blended RR
             !RR_GWP = 5.798*GWP**1.054
...
!             RR = 1.37*GWP**1.67 !---V10
  ...
  SUBROUTINE regressWspd(sensor_id,SfcTyp,rrMirs,Tbu,Emiss,Wspd)
    ...
    INTEGER, PARAMETER     :: nCoef_f19=6,nCoef_f18=6,nCoef_f17=6,nCoef_f16=6,&
         nCoef_trmm=6,nCoef_gpm=6,nCoef_mtma=6
...
!!!! F19
    ...
    ! From training (2011-08-02): TB 19h,19v,22v,37h,37v and ECMWF Wspd
    ! Unc TBs AND Filtering out all AnalysEm <=0. 
    REAL, PARAMETER, dimension(nCoef_f19) :: coefRegress_f19=(/180.43318,-0.12356,0.2545,0.06159,0.76884,-1.59048/)
    INTEGER, PARAMETER, dimension(nCoef_f19-1) :: indPred_f19=(/12,13,14,15,16/)
...
!!!! MADRAS
    ...
    elseif(sensor_id .eq. sensor_id_f19)then
       ! Filter out points likely to be precip-contaminated using polarization ratios PR(19), PR(37)
       if((Emiss(13)-Emiss(12))/(Emiss(13)+Emiss(12)) .le. emPRmin .or. &
            (Emiss(16)-Emiss(15))/(Emiss(16)+Emiss(15)) .le. emPRmin )then
          return
       endif
       wspd0=coefRegress_f19(1)
       do icoef=2,nCoef_f19
          wspd0=wspd0+coefRegress_f19(icoef)*Tbu(indPred_f19(icoef-1))
       enddo
