...
float freq_n20[nchan_atms] = { 23.800, 31.400, 50.300, 51.760, 52.800, 53.596, 54.400, 54.940, 55.500, 57.290,
      57.290, 57.290, 57.290, 57.290, 57.290, 88.200, 165.500, 183.310, 183.310, 183.310,183.310, 183.310 } ;

int polar_n20[nchan_atms] = { 2, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 3, 3, 3, 3, 3, 3 };
...
int rdr2tdr_n20_atms_TDR(char *file_RDR,char* file_GEO,char *file_OUT) {
   ...
   beamTime[iscan][ifov] =  out_BeamTime[index] ;
    ...
    if( NCHAN == nchan_atms ) { 
   for(int ichan=0; ichan<NCHAN; ichan++) {
       header.freq[ichan]  = freq_n20[ichan];
       header.polar[ichan] = polar_n20[ichan];
   }
    }
...
int rdr2tdr_n20_atms_SDR_proxy(char *file_RDR, char *file_OUT) {
...
int rdr2tdr_n20_atms_SDR(char *file_RDR,char* file_GEO,char *file_OUT,char *file_NEDT,char *instrFile) {
...
int rdr2tdr_n20_atms_SDR_remap(char *file_RDR,char* file_GEO,char *file_OUT) {
  ...
  */
  ...
  if(tdr_type == '1' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      rdr2tdr_n20_atms_TDR(file_RDR,file_GEO,file_OUT);
    }
  }
  else if(tdr_type == '2' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      rdr2tdr_n20_atms_SDR_proxy(file_RDR,file_OUT);
    }
  }
  else if(tdr_type == '3' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      if( file1.good() && file2.good() ) {
        file1.close();
   file2.close();
        rdr2tdr_n20_atms_SDR(file_RDR,file_GEO,file_OUT,file_NEDT,instrConfigFile);
      }
    }
  }
  else if(tdr_type == '4' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      rdr2tdr_n20_atms_SDR_remap(file_RDR,file_GEO,file_OUT);
    }
  }
