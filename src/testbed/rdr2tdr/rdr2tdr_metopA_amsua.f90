...
Program rdr2tdr_amsua
  ...
  INTEGER,      PARAMETER             :: mxScan = 10000, nWarmView =2,nchar = 2560,n_a1 = 510, n_a2=120,nflag=16
                                         !---nchar=2560 = 640*4
  ...
  INTEGER(2),        DIMENSION(nflag) :: scanLineCalibFlg
  ...
  CHARACTER(1), DIMENSION(2,n_a1)     :: count_a1b
  CHARACTER(1), DIMENSION(2,n_a2)     :: count_a2b
  ...
  AMSUA: DO ifile=1,nFiles
     !---Open AMSU RDR file
     iu_ama=get_lun()
     OPEN(iu_ama, FILE=trim(rdrFilesAmsuA(ifile)), FORM='UNFORMATTED', ACCESS='DIRECT', RECL=nchar,&
  ...
  WHERE(ABS(NEDT) .gt. 5. ) NEDT = 9999.999
