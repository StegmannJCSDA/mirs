/***************************************************************************************************
 * 
 * This is the C++ decoder for N20 ATMS data. 4 subrotines to handle different input data types.
 *
 * Author: Junye Chen <Junye.Chen@noaa.gov>
 *
 * Date: 11/22/2016 
 *
 ***************************************************************************************************/
...
const int nchan_atms = 22;
const int nchan_n18  = 20;
const int min_sec    = 10;

float freq_n21[nchan_atms] = { 23.800, 31.400, 50.300, 51.760, 52.800, 53.596, 54.400, 54.940, 55.500, 57.290,
      57.290, 57.290, 57.290, 57.290, 57.290, 88.200, 165.500, 183.310, 183.310, 183.310,183.310, 183.310 } ;

int polar_n21[nchan_atms] = { 2, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 3, 3, 3, 3, 3, 3 };
       
float freq_n18[nchan_n18] = { 23.800, 31.400, 50.300, 52.799, 53.595, 54.400, 54.941, 55.499, 57.290, 57.290,
      57.290, 57.290, 57.290, 57.290, 89.000, 89.000, 157.000, 183.311, 183.311, 190.311 } ;

int polar_n18[nchan_n18] = { 2, 2, 2, 2, 3, 3, 2, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 3, 3, 2 } ;
...
int rdr2tdr_n21_atms_TDR(char *file_RDR,char* file_GEO,char *file_OUT) {
   ...
   beamTime[iscan][ifov] =  out_BeamTime[index] ;
    ...
    if( NCHAN == nchan_atms ) { 
   for(int ichan=0; ichan<NCHAN; ichan++) {
       header.freq[ichan]  = freq_n21[ichan];
       header.polar[ichan] = polar_n21[ichan];
   }
    }
    else if ( NCHAN == nchan_n18 ) {
   for(int ichan=0; ichan<NCHAN; ichan++) {
   ...
   if( secsAfter1958 < 10 ) { // missing value
       seconds = 9;
   }
   else if( secsAfter1958 < 1719792035 ) { // 1719792035 ---> 2012/07/01 00:00:00 
   ...
   // qc
   //comments out QF19 in TATMS temporarilly liusy
   //QF19: space view changed
// if( QF19[iscan] > 0 ) {
//         ms.qc[0] = 1;
//         // cout << "QF19 L1070" << iscan << file_RDR << "\n";         
//        }        
//liusy.
//comments out QF1  in GEO temporarilly liusy
// if( QF1_ATMSSDRGEO[iscan] != 0 ) {
//         ms.qc[0] = 1;
//      //  cout << QF1_ATMSSDRGEO[iscan] << "QF1_ATMSSDRGEO L1074" << iscan << file_RDR << "\n";         
//        } 
//liusy.
...
int rdr2tdr_n21_atms_SDR_proxy(char *file_RDR, char *file_OUT) {
...
int rdr2tdr_n21_atms_SDR(char *file_RDR,char* file_GEO,char *file_OUT,char *file_NEDT,char *instrFile) {
...
int rdr2tdr_n21_atms_SDR_remap(char *file_RDR,char* file_GEO,char *file_OUT) {
...
int main( int argc, char *argv[] )
{
  ...
  /*
  char tdr_type = '3' ;
  strcpy( rdrFileList,"/disk1/pub/wchen/mirs_gfortran_linux_x64/data/InputsData/n20_atms_rdrFiles_2010-09-06.list");
  strcpy( pathTDR,"/disk1/pub/wchen/mirs_gfortran_linux_x64/data/TestbedData/DynamicData/tdr/n20_atms/2010-09-06/"); 
  strcpy( pathNEDT,"/disk1/pub/wchen/mirs_gfortran_linux_x64/data/TestbedData/nedt/n20_atms/");    
  strcpy( instrConfigFile,"/disk1/pub/wchen/mirs_gfortran_linux_x64/data/StaticData/InstrConfigInfo/InstrConfig_n20_atms.dat");
  strcpy( geoFileList,"/disk1/pub/wchen/mirs_gfortran_linux_x64/data/InputsData/n20_atms_geoFiles_2010-09-06.list");
  */
  ...
  */
  ...
  if(tdr_type == '1' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      rdr2tdr_n21_atms_TDR(file_RDR,file_GEO,file_OUT);
    }
  }
  else if(tdr_type == '2' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      rdr2tdr_n21_atms_SDR_proxy(file_RDR,file_OUT);
    }
  }
  else if(tdr_type == '3' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      if( file1.good() && file2.good() ) {
        file1.close();
   file2.close();
        rdr2tdr_n21_atms_SDR(file_RDR,file_GEO,file_OUT,file_NEDT,instrConfigFile);
      }
    }
  }
  else if(tdr_type == '4' ) 
  {
    for(int ifile=0; ifile<nfile; ifile++ ) {
      ...
      rdr2tdr_n21_atms_SDR_remap(file_RDR,file_GEO,file_OUT);
    }
  }
