...
! 11/23/2016  Junye Chen        added JPSS-1 (N20) ATMS
!-------------------------------------------------------------------------------
Program Calib_generic_rad
  ...
  integer,parameter  :: maxclwedge=0.05
  integer,parameter  :: nbin = 40
  integer            :: maxChan=20,nchan,nAve
  integer            :: maxPrf=3000
  
  integer            :: iuNWP,iuFWD,iuFM, iuOut
...
!  CHARACTER(LEN=256), DIMENSION(:), POINTER :: outFiles
  
  integer                       :: iscan,ifov,nfov
  integer                       :: minfov, maxfov
  ...
  real,dimension(:,:),allocatable :: meanBiasAve
  ...
  integer,dimension(:),allocatable :: indAve
...
!  outPath='/data/data293/pub/kgarrett/mirs_repo/data/TestbedData/DynamicData/nwp_analys/npp_atms/clw/'
!  call ReadList(iuNWP,TRIM(fileListNWP),filesNWP,nFileNWP,outFiles,outPath,'NEW_')
  ...
  if( sensorId .eq. SENSOR_ID_N18 .or. sensorId .eq. SENSOR_ID_METOPA .or. &
      ...
      sensorId .eq. SENSOR_ID_METOPC ) then
    maxChan = 20
    if( fmType .eq. 0 ) then 
       nfov = 30
    else
  ...
  else if( sensorId .eq. SENSOR_ID_F16 .or. sensorId .eq. SENSOR_ID_F17 .or. &
       sensorId .eq. SENSOR_ID_F18 .or. sensorId .eq. SENSOR_ID_F19 ) then
    maxChan = 24
  ...
  else if( sensorId .eq. SENSOR_ID_N20 .or. sensorId .eq. SENSOR_ID_N21 ) then
    minres = 70.
    maxChan = 22
    
    if( fmType .eq. 0 ) then 
!chen dont use this option, not possible to generate low resolution in fm step yet 
       nfov = 32
    else
       nfov = 96
    endif
  ...
  else if( sensorId .eq. SENSOR_ID_metopSGA1 ) then
    minres = 70.
    maxChan = 24

    if( fmType .eq. 0 ) then
!chen dont use this option, not possible to generate low resolution in fm step
!yet
       nfov = 32
    else
       nfov = 95
  ...
  minfov = 0
  maxfov = nfov+1
  if ( sensorId .eq. SENSOR_ID_NPP .or. sensorId .eq. SENSOR_ID_N20 .or. sensorId .eq. SENSOR_ID_N21 ) then
     minfov = 6
     maxfov = 91
  endif
  
  if ( sensorId .eq. SENSOR_ID_metopSGA1 ) then
     minfov = 6
     maxfov = 91
  endif
  ...
  do ifile = 1, nfile
    ...
    do iFM = 1, nFM
      ...
      if ( sensorId .eq. SENSOR_ID_NPP .or. sensorId .eq. SENSOR_ID_N20 .or. sensorId .eq. SENSOR_ID_N21 ) then
         ...
         CALL regr_clw_atms(FM%tb(:),FM%angle(1),FM%lat,clwScans(iscanTotal,ifov))
         !print *,'clw: ',clwScans(iscanTotal,ifov)
      endif
...
!    CALL InitHdrScene(NWP%nLev,NWP%nLay,NWP%nChan,NWP%CentrFreq,                 &
!         NWP%polarity,NWP%pres_Lev,NWP%pres_Lay,NWP,NWP%nLay,NWP%nLay,NWP%nLay,NWP%nLay,NWP%nLay,&
!         NWP%nAbsorb,NWP%AbsorbID,NWP%nqc,0,NWP%nPosScan,NWP%nScanLines,0)
    !---Write header of adjusted scene
!    CALL WriteHdrScene(iuOut,outFiles(ifile),NWP,nNWP)
...
!      NWP%CLW=0
!      NWP%CLW(10) = clwScans(iscanTotal,ifov)
!      if (NWP%iTypSfc .ne. 0) NWP%CLW=0
!      call WriteScene(iuOut,NWP)
...
!    CLOSE(iuOut)
  ...
  allocate( meanBiasAve(nfov,nchan) )
    ...
    LoopTpw: do itpw = 1, ntpw
      ...
      LoopWind: do iwind = 1, nwind
        ...
        LoopFov: do ifov = 1, nfov
          ...
          if (ifov .le. minfov .or. ifov .ge. maxfov) then
          if( sfcScans(iscan,ifov)   .eq. 0       .and. &
              ...
              clwScans(iscan,ifov)   .le. maxclwedge  .and. &
          ...
          if (ifov .gt. minfov .or. ifov .lt. maxfov) then
          if( sfcScans(iscan,ifov)   .eq. 0       .and. &
              latScans(iscan,ifov)   .ge. minlat  .and. &
              latScans(iscan,ifov)   .le. maxlat  .and. &
              tpwScans(iscan,ifov)   .ge. mintpw  .and. &
     ...
     cont2 = 0
         ...
         !    if (iscan .eq. 96) print *,ABS(tbdiffScan(iscan,ifov)-meanTbdiffScan), &
                                 !      tbsimuScan(iscan,ifov),tbmeasScan(iscan,ifov),clwScans(iscan,ifov)
...
!if (ichan .eq. 1) print *, mean4,ifov,cont1,cont2,cont4
  ...
  do ichan = 1, nchan
    ...
    !---do 5 FOV moving average to smooth out bias
       do ifov = 1, nfov
          if (ifov .eq. 1) then 
             nAve=2 
             allocate(indAve(nAve))
             indAve=(/1,2/)
          endif
          if (ifov .eq. 2) then
             nAve=3
          ...
          if (ifov .ge. 3 .and. ifov .le. 94) then 
             nAve=5
             allocate(indAve(nAve))
             indAve=(/ifov-2,ifov-1,ifov,ifov+1,ifov+2/)
          endif
          if (ifov .eq. 96) then
             nAve=2
             allocate(indAve(nAve))
          ...
          meanBiasAve(ifov,ichan) = SUM(meanBiasByHist2Output(indAve,ichan))/nAve
    ...
    !---do 5 FOV moving average to smooth out bias
       do ifov = 1, nfov
          ...
          if (ifov .ge. 3 .and. ifov .le. 93) then
             nAve=5
             allocate(indAve(nAve))
             indAve=(/ifov-2,ifov-1,ifov,ifov+1,ifov+2/)
          endif
  ...
  if( iBiasComputMethod .eq. 0 ) then
    CALL WriteBias2(fileBias,nchan,nfov,freqs,meanBiasReg2Output,Slope2Output,&
     ...
     if ( sensorId .eq. SENSOR_ID_NPP .or. sensorId .eq. SENSOR_ID_N20 .or.  &
          sensorId .eq. SENSOR_ID_N21 .or. sensorId .eq. SENSOR_ID_metopSGA1 ) then
        CALL WriteBias2(fileBias,nchan,nfov,freqs,meanBiasAve,Slope2Output,&
                    Intercept2Output,meanTbsimu2Output,meanTbmeas2Output)
     else
