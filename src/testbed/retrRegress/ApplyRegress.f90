...
!       12-08-2016      Junye Chen - Added JPSS-1 (N20) ATMS capability
!
!===============================================================
...
Program ApplyRegress
  ...
  !
  !--initialization liusy+
  !
  TbIndxArr = DEFAULT_VALUE_INT
  !
  !---Read algorithms
  DO ialgo=1,nAlgors
    DO iTyp=1,nSfcTypes
  ...
  FileLoop: DO ifile=1,nFiles 
    ...
    ProfLoop: DO iprof=1,nprofiles
       ...
       IF (sensor_id .eq. sensor_id_n18 .or. sensor_id .eq. sensor_id_metopA .or. &
           ...
           sensor_id .eq. sensor_id_metopC .or. sensor_id .eq. sensor_id_n21 .or. & 
           sensor_id .eq. sensor_id_npp .or. sensor_id .eq. sensor_id_n20    .or. &
...
!ykl       call Read_topography(Topogr,Ym%lat,Ym%lon,xalt,stype,xcover,stypeCRTM)
       call Read_topography_onetwentieth(Topogr,sensor_id,Ym%lat,Ym%lon,Ym%iscanPos,xalt,stype)
...
!       call Read_topography_onetwentieth(Topogr,sensor_id,Ym%lat,Ym%lon,Ym%iscanPos,Ym%Angle(1),xalt,stype,xcover,stypeCRTM)
       !----Avoid applying regression on all bad TBs or latitudes out of range
       IF ((ALL(Ym%tb(1:Ym%nchan) .lt. 0.)) .or. (abs(Ym%lat) .gt. 90.)) THEN
          ...
          IF (EDR(ivar) .gt. -999.) THEN
             ...
             IF (  ABS(clw + 999.) .lt. epsilon .and. sensor_id .ne. sensor_id_trmm) THEN
                IF (sensor_id .eq. sensor_id_n18 .or. sensor_id .eq. sensor_id_metopA .or. &
                    ...
                    sensor_id .eq. sensor_id_metopC .or. sensor_id .eq. sensor_id_n21 .or. &
                    sensor_id .eq. sensor_id_npp .or. sensor_id .eq. sensor_id_n20    .or. &
                    sensor_id .eq. sensor_id_metopSGA1 ) &
