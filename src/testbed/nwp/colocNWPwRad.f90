...
program colocNWPwRad
  ...
  CHARACTER(LEN=len), DIMENSION(:),       POINTER     :: imsCollocFiles
  ...
  INTEGER            :: iu_listImsColloc,nFilesImsColloc,nprfImsColloc,iuImsColloc
  REAL               :: xalt,xcover
  ...
  CHARACTER(LEN=len) :: imsCollocFilesList=DEFAULT_VALUE_STR4
  ...
  INTEGER            :: imsSfcUse=DEFAULT_VALUE_INT
  ...
  NAMELIST /ControlNWP/RadFileList,atmNWPFilesList,sfcNWPFilesList,pathNWPout,&
            ...
            nwp_source,imsSfcUse,imsCollocFilesList,Coeff_Path,BiasFile,TuningFile
...
!  IF (nfilesNWPatm .lt. 1) CALL ErrHandl(ErrorType,Err_NoFilesFound,'(atm-NWP files in coloc)') 
  ...
  !---If flag set, read the file names of collocated IMS data
  if(imsSfcUse .eq. 1)then
     call ReadList(iu_listImsColloc,trim(imsCollocFilesList),imsCollocFiles,nFilesImsColloc,dumFiles,'','')
     IF (nfilesImsColloc .lt. 1) CALL ErrHandl(ErrorType,Err_NoFilesFound,'') 
     nfilesImsColloc=minval((/norbits2process,nfilesImsColloc/))
     if(nfilesImsColloc .ne. nfile)&
          CALL ErrHandl(ErrorType,Err_Incompat,'. nfiles (Rad and imsColloc). (colocNWPwRad)')
     print *,'Finished reading imsColloc file list, nfilesImsColloc=',nfilesImsColloc
  endif
!  stop
...
!  FilesLoop: DO ifile=1,1
     ...
     !---- Read header of IMS collocation file, which is a DEP type file (if flag set)----
      if(imsSfcUse .eq. 1)then
         CALL ReadHdrDEP(iuImsColloc,imsCollocFiles(ifile),DepImsColloc,nprfImsColloc)
...
!ykl        call Read_topography(Topogr,Ym%lat,Ym%lon,xalt,iSfcTypOut,xcover,iSfcTypCRTM)
        call Read_topography_onetwentieth(Topogr,sensor_id,Ym%lat,Ym%lon,Ym%iscanPos,xalt,iSfcTypOut)
	   ...
	   call  determineNWPindex(Ym%lat,Ym%lon,minLatNWP,dLatNWP,minLonNWP,dLonNWP,&
		 ...
		 ilat,ilon,itime,nlat,nlon,ntime)
           ...
           !---Replace Preclassified surface type with type from IMS
           IF(imsSfcUse .eq. 1)THEN
              CALL ReadDep(iuImsColloc,DepImsColloc,ierr)
              IF (ierr.ne.0) CALL ErrHandl(ErrorType,Err_ReadingFile,'. DEP ImsColloc file. (colocNwpwRad)')
!              print *,'PreClassif: SfcClass, IMS: sfcClass=',SfcClass,DepImsColloc%iTypSfc
!               IF(DepImsColloc%iTypSfc .GE. 0 .AND. DepImsColloc%iTypSfc .LE. 3)SfcClass = DepImsColloc%iTypSfc
              IF(DepImsColloc%iTypSfc .GE. 0 .AND. DepImsColloc%iTypSfc .LE. 3)THEN
                 IF(SfcClass .GE. 0 .AND. SfcClass .LE. 1 .AND. &
                      DepImsColloc%iTypSfc .LE. 1)THEN
                    SfcClass = DepImsColloc%iTypSfc
                 ELSE IF(SfcClass .GE. 2 .AND. SfcClass .LE. 3 .AND. &
                      DepImsColloc%iTypSfc .GE. 2)THEN
                    SfcClass = DepImsColloc%iTypSfc
                 ENDIF
              ENDIF
           ENDIF
...
!      DEALLOCATE(emissivity,freq,Emiss2Adjust,Emiss_Adjusted,tb)
      ...
      CLOSE(iuOut)
      CLOSE(iuMeasur)
      IF(imsSfcUse .eq. 1) CLOSE(iuImsColloc)
