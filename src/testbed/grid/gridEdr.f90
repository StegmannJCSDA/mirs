...
Program gridEdr
  ...
  CHARACTER(LEN=9)    :: satId=DEFAULT_VALUE_STR4  ! string type satellite id: n18, m2, f16, f18
  ...
  if      ( strcmp(satId, SATID_F16)    .eq. 0 ) then
     ...
     if( fmType .eq. 0 ) then
        ...
        STOP 'Error: Incorrect fmType for N20 ATMS'
     endif
     ...
     if( fmType .eq. 0 ) then
        ...
        STOP 'Error: Incorrect fmType for N21 ATMS'
     endif
  ...
  prodLoop: DO iprod = 1, NPROD
     cendLoop: DO icend = 1, NCEND
        ...
        if( INT_SATID .eq. SENSOR_ID_N18 .or. INT_SATID .eq. SENSOR_ID_METOPA .or. &
             ...
             INT_SATID .eq. SENSOR_ID_METOPC .or. INT_SATID .eq. SENSOR_ID_N21 .or.  &
             INT_SATID .eq. SENSOR_ID_metopSGA1 ) then
