...
!       Junye   Chen    12/20/2016        Added JPSS1 (N20)
!
!***************************************************************************************************
...
Program gridDep
  ...
  CHARACTER(LEN=16)   :: satId=DEFAULT_VALUE_STR4  ! n18, m2, f16, n19, npp, n20
  ...
  if      ( strcmp(satId, SATID_F16)    .eq. 0 ) then
  ...
  else if ( strcmp(satId, SATID_F19)    .eq. 0 ) then
     INT_SATID = SENSOR_ID_F19
     if( fmType .eq. 0 ) then
        NFOV=30
     else if( fmType .eq. 1 ) then
        NFOV=60
        ...
        STOP 'Error: Incorrect fmType for F19'
     endif
  ...
  else if ( strcmp(satId, SATID_METOPC) .eq. 0 ) then
     INT_SATID = SENSOR_ID_METOPC
     if( fmType .eq. 0 ) then
        NFOV=30
        SCAN_ANG=3.3
     else if( fmType .eq. 1 ) then
        ...
        STOP 'Error: Incorrect fmType for metopC'
     endif
  ...
  else if ( strcmp(satId, SATID_N20) .eq. 0 ) then
     INT_SATID = SENSOR_ID_N20
     RSAT=824.0
     if( fmType .eq. 0 ) then
        NFOV=32
   SCAN_ANG=3.3
   ...
   SCAN_ANG=1.1
        ...
        STOP 'Error: Incorrect fmType for JPSS1 (N20) ATMS'
     endif
     
  else if ( strcmp(satId, SATID_N21) .eq. 0 ) then
     INT_SATID = SENSOR_ID_N21
     RSAT=824.0
     if( fmType .eq. 0 ) then
        NFOV=32
   SCAN_ANG=3.3
   ...
   SCAN_ANG=1.1
        ...
        STOP 'Error: Incorrect fmType for JPSS2 (N21) ATMS'
     endif
     
  else if ( strcmp(satId, SATID_metopSGA1) .eq. 0 ) then
     INT_SATID = SENSOR_ID_metopSGA1
     RSAT=835.0
     if( fmType .eq. 0 ) then
        NFOV=32
        SCAN_ANG=3.3
        ...
        STOP 'Error: Incorrect fmType for metopSGA1 MWS'
     endif
...
!      endif
  ...
  cendLoop: DO icend = 1, NCEND
     ...
     if( INT_SATID .eq. SENSOR_ID_N18 .or. INT_SATID .eq. SENSOR_ID_METOPA .or. &
          ...
          INT_SATID .eq. SENSOR_ID_METOPC .or. &
          INT_SATID .eq. SENSOR_ID_NPP .or. INT_SATID .eq. SENSOR_ID_N20 .or.  &
          INT_SATID .eq. SENSOR_ID_N21 .or. INT_SATID .eq. SENSOR_ID_metopSGA1  ) THEN
        ...
        call get_fov_size(fov_size,NFOV,SCAN_ANG,RSAT)
              ...
              INT_SATID .eq. SENSOR_ID_F17 .or. INT_SATID .eq. SENSOR_ID_F19 ) then
        fov_size(:) = 0.6745
     else
        fov_size(:) = 1.0
     endif
    ...
    !-----------------------------------------------------
     ...
     !---- extract variables from Dep structure ----
           if( strcmp(prodId,'sfc2') .eq. 0 ) then
              ...
              var1D = Dep%WindSp
           else if( strcmp(prodId,'sfr') .eq. 0 ) then
              var1D = Dep%SFR
           else if( strcmp(prodId,'sfrprob') .eq. 0 ) then
              var1D = Dep%Prob_SF
           endif
        ...
        !---- also set missing values of hydrometeors to -888.0
                  !---- this is mainly to handle plotting of missing hydrometeors over 
                  !---- areas covered by sea ice climatology mask
                    if ( ( strcmp(prodId,'iwp') .eq. 0 .or. strcmp(prodId,'rwp') .eq. 0   .or.  &
                         strcmp(prodId,'lwp') .eq. 0 .or. strcmp(prodId,'rr' ) .eq. 0 ) .and. &
                         var1D .lt. -900. ) then
                       var1D_grid(lonbox,latbox)  = -888.0                             
