...
Program gridBias
  ...
  !-- namelist data
  character(LEN=9)               :: satId=DEFAULT_VALUE_STR4
  ...
  integer                        :: isatId=DEFAULT_VALUE_INT  ! 1-N18,2-MetopA,3-F16,4-N19,6-NPP,9-TRMM,10-GPM,14-MetopB,20-N20(JPSS1),21-MetopC
  ...
  if ( isatId .eq. sensor_id_n18 .or. isatId .eq. sensor_id_metopA .or. &
       ...
       isatId .eq. sensor_id_metopC .or. &
       isatId .eq. sensor_id_npp .or. isatId .eq. sensor_id_mtsa   .or. &
       isatId .eq. sensor_id_n20 .or. isatId .eq. sensor_id_n21 .or. isatId .eq. sensor_id_metopSGA1) then
    dependString='angle'
  else
  ...
  !*************************************************************************************************
    subroutine computeBias2(fileDepen,fileChisq,fileSfc,fileClw,fileRr,fileMirs,fileNwp,fileBias,fileAsym,&
      ...
      NBIN=30
      if (isatId .eq. sensor_id_npp .or. isatId .eq. sensor_id_n20 .or. isatId .eq. sensor_id_n21) NBIN=35
      if (isatId .eq. sensor_id_metopSGA1 ) NBIN=35
      if (isatId .eq. sensor_id_trmm  ) NBIN=26
      if (isatId .eq. sensor_id_gpm   ) NBIN=20
      if (isatId .eq. sensor_id_amsre ) NBIN=191
      if (isatId .eq. sensor_id_gcomw1) NBIN=243
      if (isatId .eq. sensor_id_mtma  ) NBIN=26
      ...
      if ( isatId .eq. sensor_id_n18 .or. isatId .eq. sensor_id_metopA .or. &
           isatId .eq. sensor_id_n19 .or. isatId .eq. sensor_id_metopB .or. &
           isatId .eq. sensor_id_metopC) then
         do ibin = 1, NBIN+1
            bin_box(ibin) = 4 * (ibin-1) - 60
         enddo
      else if( isatId .eq. sensor_id_npp .or. isatId .eq. sensor_id_n20 .or. isatId .eq. sensor_id_n21) then
         do ibin = 1, NBIN+1
            bin_box(ibin) = 4 * (ibin-1) - 70
         enddo
      else if( isatId .eq. sensor_id_metopSGA1 ) then
         do ibin = 1, NBIN+1
            bin_box(ibin) = 4 * (ibin-1) - 70
         enddo
      else if ( isatId .eq. sensor_id_f16   .or. isatId .eq. sensor_id_f18 .or. &
           isatId .eq. sensor_id_f17   .or. isatId .eq. sensor_id_f19 .or. &
            ...
            do icond = 1, NCOND
               ...
               do isfc = 1, NSFC
                  ...
                  do ibin = 1, NBIN
                     ...
                     bin_logic = ang1 .ge. BIN_BOX(ibin) .and. ang1 .lt. BIN_BOX(ibin+1) .and. &
                          bias1 .ne. MISSING .and. bias1 .ne. FAILING
  ...
  !*************************************************************************************************
    subroutine computeBias3(fileDepen,fileChisq,fileSfc,fileClw,fileRr,fileMirs,fileNwp,fileBias,fileAsym,&
      ...
      if (isatId .eq. sensor_id_metopSGA1 ) NBIN=35
      if (isatId .eq. sensor_id_trmm  ) NBIN=26
!liusy-      if (isatId .eq. sensor_id_gpm   ) NBIN=26
      if (isatId .eq. sensor_id_gpm   ) NBIN=20 !liusy+
      if (isatId .eq. sensor_id_amsre ) NBIN=191
      if (isatId .eq. sensor_id_gcomw1) NBIN=243
      if (isatId .eq. sensor_id_mtma  ) NBIN=26
      if (isatId .eq. sensor_id_mtsa  ) NBIN=26
               ...
               do icond = 1, NCOND
                  ...
                  do isfc = 1, NSFC
                     ...
                     do ibin = 1, NBIN
                        ...
                        bin_logic = ang1 .ge. BIN_BOX(ibin) .and. ang1 .lt. BIN_BOX(ibin+1) &
                             .and. bias1 .ne. MISSING .and. bias1 .ne. FAILING
