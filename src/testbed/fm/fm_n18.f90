...
!old           IF (ANY(qc(2:nqc_1+1) .ne. 0) .and. ANY(qc(nqc_1+2:nqc_1+nqc_2+1) .ne. 0)) qc(1)=1
!          qc(5) corresponds to the third quality flag in rdr2tdr procedure
!          (calibration problem code) and frequently it has nonzero value which means bad.
!          After the degradation of MHS, several gray coloed missing blocks showed up in
!          the NOAAr-18 MIRS outputs. The following two lines are intended to
!          remove the missing blocks. It does not guarantee the quality of the
!          NOAA-18 MiRS outputs. 2019.06.05  Yong-Keun Lee
           IF (ANY(qc(2:      4) .ne. 0) .and. ANY(qc(nqc_1+2:nqc_1+nqc_2+1) .ne. 0)) qc(1)=1
           IF (ANY(qc(6:nqc_1+1) .ne. 0) .and. ANY(qc(nqc_1+2:nqc_1+nqc_2+1) .ne. 0)) qc(1)=1
