...
!
!       06-12-2014     Craig Kenton Smith, AER @ NOAA/STAR
!                      Modified to accomodate FMresol=2 
!                      (ENV res, 2 IMG averaged, nearest neighbor ENV/LAS/UAS)
!
!===============================================================
...
program ModifyNedt_ssmis
  ...
  IF (FMresol.eq.3) THEN
      ...
      fact_uas = 1.
  ENDIF
  IF (FMresol.eq.2) THEN ! Assumes 1 scan line averaged
     !CALL ErrHandl(ErrorType,Err_NotImplemented,'(SSMIS/S Noise-Ampl. @ ENV resolution)')
      fact_img = 1./SQRT(2.)
      fact_env = 1.
      fact_las = 1.
      fact_uas = 1.
  ...
  IF (FMresol.eq.1) THEN
      ...
      fact_uas = 1./sqrt(3.)
  ENDIF
  IF (FMresol.eq.0) THEN
      fact_img = 1./sqrt(36.)
      fact_env = 1./sqrt(18.)
      fact_las = 1./sqrt(12.)
      fact_uas = 1./sqrt(6.)
  ! KLUGE TO MAKE RESULTS IDENTICAL TO IFORT FOR PREVIOUS VERSION
  ! MUST BE COMMENTED OUT FOR SUBSEQUENT RUNS
  !   fact_img = 0.
  !   fact_env = 0.
  !   fact_las = 0.
  !   fact_uas = 0.
