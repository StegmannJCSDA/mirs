...
!
!       06-11-2014      Craig Kenton Smith, AER Inc. @ NOAA/STAR
!                       Added resolution 2 (90 FOVs/scan) by averaging
!                       adjacent IMG measurements and selecting nearest
!                       neighbor ENV/LAS/UAS measuements.  Separated #IMG's
!                       averaged from number of scans averaged for this
!                       resolution only.
!===============================================================
...
program fm_f18
  ...
  INTRINSIC :: ALL,ANY,INT,SUM,TRIM,ABS,COUNT,MOD
  ...
  INTEGER    :: nAvg=0,nAvg_scan=0,nAvg_track=0,iAvg=0
  ...
  FilesLoop: DO ifile=1,nFiles
      ...
      ELSE IF( FMresol .eq. 2 ) THEN !FM process is done @ the ENV resolution
         ...
         nAvg    = nAvg4ENV
         nAvg_scan=nAvg
         nAvg_track=1 ! Set to no averaging of scans
      ...
      ALLOCATE(Cfreq(nChan),pol(nChan),lat(nFovs),lon(nFovs),angle(nFovs),angleFM(nchan),         &
          ...
          lat_img(nFovs_img,nAvg_track),lon_img(nFovs_img,nAvg_track),angle_img(nFovs_img,nAvg_track),        &
          tb_img(nFovs_img,nchan_img,nAvg_track),qc_img(nqc_img,nAvg_track),                                  &
          lat_env(nFovs_env,nAvg_track),lon_env(nFovs_env,nAvg_track),angle_env(nFovs_env,nAvg_track),        &
          tb_env(nFovs_env,nchan_env,nAvg_track),qc_env(nqc_env,nAvg_track),                                  &
          lat_las(nFovs_las,nAvg_track),lon_las(nFovs_las,nAvg_track),angle_las(nFovs_las,nAvg_track),        &
          tb_las(nFovs_las,nchan_las,nAvg_track),qc_las(nqc_las,nAvg_track),                                  &
          lat_uas(nFovs_uas,nAvg_track),lon_uas(nFovs_uas,nAvg_track),angle_uas(nFovs_uas,nAvg_track),        &
          tb_uas(nFovs_uas,nchan_uas,nAvg_track),qc_uas(nqc_uas,nAvg_track),ChanReorder(nChan),               &
          RelAziAngle_img(nFovs_img,nAvg_track),SolZenAngle_img(nFovs_img,nAvg_track),                        &
          RelAziAngle_env(nFovs_env,nAvg_track),SolZenAngle_env(nFovs_env,nAvg_track),                        &
          RelAziAngle_las(nFovs_las,nAvg_track),SolZenAngle_las(nFovs_las,nAvg_track),                        &
          RelAziAngle_uas(nFovs_uas,nAvg_track),SolZenAngle_uas(nFovs_uas,nAvg_track))
      ...
      ScanLLoopFlag: DO iscanLine=1,nScanL
        ...
        DO ifov=1,nFovs
           ...
           ELSE IF( FMresol .eq. 2 ) THEN
              ...
              IF (nAvg_track .eq. 1) THEN
                 lat(ifov)   = lat_env(ifov,1)
                 lon(ifov)   = lon_env(ifov,1)
              ELSE IF (nAvg_track .eq. 2) THEN
                 lat(ifov) = (lat_env(ifov,1)+lat_env(ifov,2))/2.0
                 lon_env1=lon_env(ifov,1)
                 lon_env2=lon_env(ifov,2)
                 IF (ABS(lon_env1-lon_env2) .le. 180.0) then
                    lon(ifov) = (lon_env1+lon_env2)/2.0
                 ELSE
                    if (lon_env1 .lt. 0.0) lon_env1=lon_env1+360.0
                    if (lon_env2 .lt. 0.0) lon_env2=lon_env2+360.0
                 ...
                 ENDIF
              ELSE IF (nAvg_track .eq. 3) THEN
                 lat(ifov)   = lat_env(ifov,2)
                 lon(ifov)   = lon_env(ifov,2)
              ELSE
                 CALL ErrHandl(ErrorType,Err_NotImplemented,'(SSMIS/S FM @ ENV nAvg_track>3)')
              ENDIF
        ...
        IF (ALL(qc(1:nqc) .eq. 0)) THEN 
           DO ifov=1,nFovs
              ...
              ELSE IF( FMresol .eq. 2 ) THEN
                 ...
                 ifov_img = int((ifov-1)*2.)+1  ! Ave 2 IMG and take nearest neighbor ENV/LAS/UAS
                 ifov_env = ifov                ! Exact matchup with ENV centroids.
                 ifov_las = int((ifov-1)/1.5)+1
                 ifov_uas = int((ifov-1)/3.0)+1

                 DO ichan=1,nchan_img
                    tb(iFov,ichan)          = SUM(tb_img(ifov_img:ifov_img+1,ichan,1:nAvg_track))/(nAvg_scan*nAvg_track)
                 ENDDO
                 nchanTot                   = nchan_img
                 DO ichan=1,nchan_env
                    tb(iFov,nchanTot+ichan) = SUM(tb_env(ifov_env,ichan,1:nAvg_track))/(nAvg_track)
                 ENDDO
                 nchanTot                   = nchan_img + nchan_env

                 ! Res2 Option 2:  To account for every 3rd IFOV having bad spatial match between last scanpos IMG 
                 ! and LAS, average two LAS together for ifov=3N+2 only
                 IF (MOD(ifov,3) .ne. 2) then
                    DO ichan=1,nchan_las
                       tb(iFov,nchanTot+ichan) = SUM(tb_las(ifov_las,ichan,1:nAvg_track))/(nAvg_track)
                    ENDDO
                 ELSE
                    DO ichan=1,nchan_las
                       tb(iFov,nchanTot+ichan) = SUM(tb_las(ifov_las:ifov_las+1,ichan,1:nAvg_track))/(2.*nAvg_track)
                    ENDDO
                 ENDIF
                 ...
                 DO ichan=1,nchan_uas
                    tb(iFov,nchanTot+ichan) = SUM(tb_uas(ifov_uas,ichan,1:nAvg_track))/(nAvg_track)
                 ENDDO
                 ...
                 Angle(ifov)                = SUM(Angle_env(iFov,1:nAvg_track))/(nAvg_track)
                 RelAziAngle(ifov)          = SUM(RelAziAngle_env(iFov,1:nAvg_track))/(nAvg_track)
                 SolZenAngle(ifov)          = SUM(SolZenAngle_env(iFov,1:nAvg_track))/(nAvg_track)
                 node                       = node_env
