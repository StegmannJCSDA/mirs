...
if satId eq 'f19' then prefix='mirs_adv_dmsp_'+satId+'_ssmis_orbit_'
if satId eq 'gpm' then prefix='mirs_adv_eos_'+satId+'_gmi_orbit_'
if satId eq 'npp' then prefix='mirs_adv_npoess_'+satId+'_atms_orbit_'
if satId eq 'n20' then prefix='mirs_adv_jpss_'+satId+'_atms_orbit_'
if satId eq 'n21' then prefix='mirs_adv_jpss_'+satId+'_atms_orbit_'
if satId eq 'metopSGA1' then prefix='mirs_adv_poes_'+satId+'_mws_orbit_'

;---Get file list of input files containing data
orbit_files = FILE_SEARCH(qcDir,'QC_MON_' + satId +'_????-??-??.HR', COUNT=nfile)
...
;  orbit_files = FILE_SEARCH(qcDir,'QC_MON_n20_????-??-??.HR', COUNT=nfile)
;endif 
;
;if satId eq 'n21' then begin
;  orbit_files = FILE_SEARCH(qcDir,'QC_MON_n21_????-??-??.HR', COUNT=nfile)
;endif 
;
;if satId eq 'metopSGA1' then begin
;  orbit_files = FILE_SEARCH(qcDir,'QC_MON_metopSGA1_????-??-??.HR', COUNT=nfile)
;endif 
