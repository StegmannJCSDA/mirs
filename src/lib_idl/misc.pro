...
; Junye Chen UMD/ESSIC @ NOAA/NESDIS/STAR, 2016, Add JPSS/N20
; Yong-Keun Lee UMD/ESSIC @ NOAA/NESDIS/STAR, 2020, Add JPSS/N21
;---------------------------------------------------------------------------------------------------
...
;
;===================================================================================================
;plot_vert, Shuyan Liu, 2016-02-23
;===================================================================================================
...
;
;---Shuyan Liu,2016-02-18 Note: For plotting purpose only, I use players_plot and psfc_plot for 
;                               pressure layers and psfc value so that the psfc line can 
;                               plot at the correct level on figure
;             Made changes based on wanchun's original code and did not recorded
;
   players_plot    = fltarr(YDIM+1) 
   players_plot(*) = -999.
   players_plot(YDIM) = yrange(0)
   for irow = YDIM-1,0,-1 do begin
      players_plot(irow) = players_plot(irow+1) - PRESSURE_SPAN/YDIM
   endfor

   start_index    = 100 - YDIM ;---MiRS product has 100 layers
   psfc_plot      = fltarr(XDIM)
   psfc_plot(*)   = -999.

   layers_psfc = -999
   for icol=0, XDIM-1 do begin
      if psfc(icol) gt 0 then begin
         for ilay = 0, 98 do begin
            if psfc(icol) ge PLAYERs(ilay) and psfc(icol) lt PLAYERs(ilay+1) then begin
               layers_psfc = ilay
               layers_psfc = layers_psfc - start_index
            endif
         endfor
      endif

      if layers_psfc lt 0    then  psfc_plot(icol) = -999.
      if layers_psfc ge YDIM then  psfc_plot(icol) = players_plot(YDIM)
      if layers_psfc ge 0 and layers_psfc lt YDIM  then psfc_plot(icol) = players_plot(layers_psfc+1)
      if psfc(icol) lt 0 then psfc_plot(icol) = -999.

      ss = where( FIX(tmp) eq -99, cnt)
      if cnt eq YDIM then psfc_plot(icol) = -999.
   endfor
...
for icol=0, XDIM-1 do begin
  if tmp(icol,irow) eq -99.0  then image(icol,irow) = 252 ;---QC failed, dark grey
  if tmp(icol,irow) eq -999.0 then image(icol,irow) = 254 ;---no data, white
endfor
endfor
...
ss = where( psfc_plot       lt 0.0, cnt) & if( cnt gt 0 ) then psfc_plot(ss)       = !VALUES.F_NAN
Oplot, x, psfc_plot, color=255, thick=2
...
;     
;===================================================================================================
Pro plot_vert_org,tmp,xval,yval,xrange,yrange,xtitle,ytitle,title,map_name,minvalue,maxvalue,Xticks,$
    div,fmt,Yticks,YtickName,x,psfc,color_table,isMirs
...
;     
;===================================================================================================
;---plot_vert2, Shuyan Liu, 2016-02-23
;===================================================================================================
...
;
   ...
   pfreezing_plot      = fltarr(XDIM)
   pfreezing_plot(*)   = -999.

   layers_psfc = -999
   layers_pfreezing = -999
   for icol=0, XDIM-1 do begin
      if psfc(icol) gt 0 then begin
         for ilay = 0, 98 do begin
            if psfc(icol) ge PLAYERs(ilay) and psfc(icol) lt PLAYERs(ilay+1) then begin
      ...
      if pfreezing(icol) gt 0 then begin
         for ilay = 0, 98 do begin
            if pfreezing(icol) ge PLAYERs(ilay) and pfreezing(icol) lt PLAYERs(ilay+1) then begin
               layers_pfreezing = ilay
               layers_pfreezing = layers_pfreezing - start_index
            endif
         endfor
      endif
      if layers_pfreezing lt 0 or layers_pfreezing ge YDIM-1   then pfreezing_plot(icol) = -999.
      if layers_pfreezing ge 0 and layers_pfreezing lt YDIM-1  then pfreezing_plot(icol) = players_plot(layers_pfreezing+1)
      if pfreezing(icol) lt 0 then pfreezing_plot(icol) = -999.

      ss = where(tmp(icol,*) lt 0, cnt)
      if cnt eq YDIM then pfreezing_plot(icol) = -999.
   endfor
...
for irow=0, YDIM-1 do begin 
  ...
  if tmp(icol,irow) eq -99.0  then image(icol,irow) = 252  ;---QC failed, dary grey
  if tmp(icol,irow) eq -999.0 then image(icol,irow) = 254  ;---no data, white
endfor
endfor
...
ss = where( psfc_plot       lt 0.0, cnt) & if( cnt gt 0 ) then psfc_plot(ss)       = !VALUES.F_NAN
ss = where( pfreezing_plot  lt 0.0, cnt) & if( cnt gt 0 ) then pfreezing_plot(ss)  = !VALUES.F_NAN

Oplot, x, psfc_plot,       color=255, thick=2
Oplot, x, pfreezing_plot,  color=246, thick=1
...
;     
;===================================================================================================
Pro plot_vert2_org, tmp,xval,yval,xrange,yrange,xtitle,ytitle,title,map_name,minvalue,maxvalue,Xticks,$
	        div,fmt,Yticks,YtickName,x,psfc,pfreezing,praintop,picebottom,color_table,isMirs
...
position_bar[2]=position[2]-0.1
position_bar[3]=position_bar[1]+0.03
ColorBar,NColors=npos*step,Color=1,Position=position_bar, FORMAT='(I3)', $
...
;   if TQ eq 0 and isMeanOrNot eq 0 and plotreqs eq 1 then begin
;    	plots,[2.5,2.5],[950,700],linestyle=linesty,color=colr,thick=thck
;    	plots,[1.5,2.5],[700,700],linestyle=linesty,color=colr,thick=thck
;    	plots,[1.5,1.5],[700,100],linestyle=linesty,color=colr,thick=thck
;    	;plots,[1.5,1.5],[300,30], linestyle=linesty,color=colr,thick=thck
...
;   if TQ eq 1 and isMeanOrNot eq 0 and plotreqs eq 1 then begin
;    	plots,[20,20],[950,600],linestyle=linesty,color=colr,thick=thck
;    	plots,[20,40],[600,600],linestyle=linesty,color=colr,thick=thck
;    	plots,[40,40],[600,200],linestyle=linesty,color=colr,thick=thck
;    	;plots,[40,40],[400,100],linestyle=linesty,color=colr,thick=thck
...
; History:
;       03-22-2007      Sid Ahmed Boukabara, IMSG Inc @ NOAA/NESDIS/ORA
;       10-28/2016      Junye Chen, UMD/ESSIC @ NOAA/NESDIS/STAR, Add JPSS/N20
;
;===============================================================
...
PRO consts, Consts
   ...
   Consts={                                        $
	  ...
	  STR_SATID_METOPC: 'metopC',              $
	  STR_SATID_METOPSGA1: 'metopSGA1',        $
...
if satId1 eq 'f16' or satId1 eq 'f17' or satId1 eq 'f18' or satId1 eq 'f19' then begin
...
Pro vertical_cross, NCOL,NROW,NLAY, gridfactor, gridDataDir, figsDir, $
  ...
  if ( satId eq 'f16' or satId eq 'f17' or satId eq 'f18' or satId eq 'f19' ) then begin
  ...
  if ( satId eq 'n20' ) then begin
    prefix='mirs_adv_jpss_'+satId+nwpId+'_vert_glb'
    prefix_us='mirs_adv_jpss_'+satId+nwpId+'_vert_us'
  endif
  if ( satId eq 'n21' ) then begin
    prefix='mirs_adv_jpss_'+satId+nwpId+'_vert_glb'
    prefix_us='mirs_adv_jpss_'+satId+nwpId+'_vert_us'
  endif
  ...
  if prod_name eq 'rainp' and isMirs eq 2 then prod_name1='clwp'
  fileGrid=gridDataDir+'GRID_'+satId+nwpId+'_'+yyyymmdd+'_'+prod_name1+'_'+cend+'.dat'
  ...
  ;fix ploting problem of zero plotted as white (no data).
  if (prod_name eq 'clwp' or prod_name eq 'rainp' or prod_name eq 'graupelp') then begin
     ii=where(prod ge 0. and prod lt 0.001, cnt)
     if (cnt gt 0) then prod[ii] = 0.001
  endif
...
Pro vertical_cross_diff, NCOL,NROW,NLAY,gridfactor,satId1,satId2,gridDir1,gridDir2,figsDir,$
  ...
  if ( satId1 eq 'n20' ) then begin
    prefix='mirs_adv_jpss_'+satId1+'_'+satId2+'_vert_glb'
    prefix_us='mirs_adv_jpss_'+satId1+'_'+satId2+'_vert_us'
  endif
  if ( satId1 eq 'n21' ) then begin
    prefix='mirs_adv_jpss_'+satId1+'_'+satId2+'_vert_glb'
    prefix_us='mirs_adv_jpss_'+satId1+'_'+satId2+'_vert_us'
  endif
  ...
  if ( satId1 eq 'n20'   ) then titleSatId='N20/ATMS'
  if ( satId1 eq 'n21'   ) then titleSatId='N21/ATMS'
  if ( satId1 eq 'metopSGA1'   ) then titleSatId='metopSGA1/MWS'
...
;  position=[3.6/xz,3.6/yz,17.1/xz,17.1/yz]
;  position=[1.6/xz,1.8/yz,12.8/xz,16.2/yz]
  ...
  if (iplotStats eq 1) then begin
      ...
      r0=minY
      r=maxY
      xyouts,xs,r0+(0.94*(r-r0)),comm9,charsize=chsz*0.8
      xyouts,xs,r0+(0.90*(r-r0)),comm10,charsize=chsz*0.8
      xyouts,xs,r0+(0.86*(r-r0)),comm11,charsize=chsz*0.8
      xyouts,xs,r0+(0.82*(r-r0)),comm12,charsize=chsz*0.8
      xyouts,xs,r0+(0.78*(r-r0)),comm13,charsize=chsz*0.8
      xyouts,xs,r0+(0.74*(r-r0)),comm14,charsize=chsz*0.8
      xyouts,xs,r0+(0.70*(r-r0)),comm15,charsize=chsz*0.8
  endif
  ;---- create colorbar
  position_bar=fltarr(4)
